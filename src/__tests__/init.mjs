 ...
 *
 * OPTIMIZATION: Only initialize once using a global flag to prevent
 * 71 test files from re-initializing the same polyfills.
 * This reduces init overhead from 71 × setup to just 1 × setup.
 */

// Skip initialization if already done (module is cached, so only first import runs code below)
if (!globalThis.__TEST_INIT_DONE__) {
  globalThis.__TEST_INIT_DONE__ = true;
