...
export function useStockValidation() {
  ...
  const checkAvailability = useCallback(
    async (productId, warehouseId, quantity, unit) => {
      try {
        setLoading(true);
        setError(null);
   ...
   */
  const validateUomCompatibility = useCallback(
    async (productId, requestedUnit) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const getConversionPreview = useCallback(
    async (productId, quantity, fromUnit) => {
      try {
        setLoading(true);
        setError(null);
   ...
   */
  const getShortfallWarning = useCallback(
    async (productId, warehouseId, quantity) => {
...
function convertUnits(quantity, fromUnit, toUnit) {
  ...
  const rate =
    weightConversions[conversionKey] || lengthConversions[conversionKey] || 1;
