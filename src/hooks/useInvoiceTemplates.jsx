...
const useInvoiceTemplates = (initialTemplate = "standard", companySettings = null) => {
  ...
  const [selectedTemplateId, setSelectedTemplateId] = useState(savedPrefs?.templateId || initialTemplate);

  // Custom colors override
  const [customColors, setCustomColors] = useState(savedPrefs?.customColors || null);
  ...
  }, [companySettings?.settings?.invoiceTemplate, companySettings?.settings]); // loadCompanySettings is stable
 ...
 */
export const RecurringInvoiceSettings = ({ settings, onToggle, onUpdate, isDarkMode = false, className = "" }) => {
  return (
    <div className={`space-y-4 ${className}`}>
      {/* Enable toggle */}
      <div className="flex items-center justify-between">
        <div>
          <div className={`font-medium ${isDarkMode ? "text-white" : "text-gray-900"}`}>Recurring Invoice</div>
      ...
      {settings.enabled && (
        ...
        >
          ...
          `}
          >
            ℹ️ Recurring invoices require backend setup. This UI prepares the settings that will be used once the feature
            is enabled.
          </div>
        </div>
      )}
