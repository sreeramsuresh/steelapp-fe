...
const Input = ({ label, error, className = '', type = 'text', isDarkMode, ...props }) => {
  return (
...
const Select = ({ label, options, value, onChange, className = '', isDarkMode }) => {
  return (
    <div className="space-y-1">
      {label && (
        <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-700'}`}>
...
const ProductDetailDrawer = ({ product, isOpen, onClose, isDarkMode, navigate }) => {
  ...
  const InfoRow = ({ icon: Icon, label, value, valueClassName = '' }) => (
    <div className="flex items-start gap-3 py-2">
      <Icon size={16} className={`mt-0.5 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
      <div className="flex-1 min-w-0">
        <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>{label}</p>
        <p className={`text-sm font-medium truncate ${valueClassName || (isDarkMode ? 'text-white' : 'text-gray-900')}`}>
          {value || '—'}
        </p>
      </div>
    </div>
  ...
  return (
    <>
      ...
      />
      {/* Drawer */}
      <div className={`fixed top-0 right-0 h-full w-[400px] z-50 transform transition-transform duration-300 ease-out shadow-2xl ${
        ...
        <div className="p-4 overflow-y-auto h-[calc(100%-130px)]">
          ...
          {/* Pricing Section */}
          <div className={`rounded-lg p-4 mb-4 ${isDarkMode ? 'bg-gray-800/50' : 'bg-gray-50'}`}>
            <h5 className={`text-xs font-semibold uppercase mb-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              Pricing
            </h5>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>Selling Price</p>
                <p className={`text-lg font-bold text-teal-500`}>
                  AED {product.sellingPrice?.toFixed(2) || product.selling_price?.toFixed(2) || '0.00'}
                </p>
              </div>
              <div>
                <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>Cost Price</p>
            ...
            {(product.sellingPrice && product.costPrice) && (
              <div className="mt-3 pt-3 border-t border-dashed ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}">
                <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>Margin</p>
          ...
          <div className={`rounded-lg p-4 mb-4 ${isDarkMode ? 'bg-gray-800/50' : 'bg-gray-50'}`}>
            ...
            <div className="space-y-1">
              ...
              {(product.isImported || product.is_imported) && (
                <div className="space-y-1">
                  <InfoRow icon={Globe} label="Country of Origin" value={product.countryOfOrigin || product.country_of_origin || product.origin_country} />
                  <InfoRow icon={Factory} label="Mill / Manufacturer" value={product.millName || product.mill_name || product.manufacturer} />
                </div>
              )}
            </div>
          </div>
          ...
          <div className={`rounded-lg p-4 ${isDarkMode ? 'bg-gray-800/50' : 'bg-gray-50'}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>Available Stock</p>
                <p className={`text-lg font-bold ${
                  (product.stockQuantity || product.stock_quantity || 0) > 0 ? 'text-green-500' : 'text-red-500'
                }`}>
                  {product.stockQuantity || product.stock_quantity || 0}
                </p>
              </div>
              <div>
                <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>Reserved</p>
                <p className={`text-lg font-semibold ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
        ...
        {/* Footer */}
        <div className={`absolute bottom-0 left-0 right-0 p-4 border-t ${isDarkMode ? 'border-gray-700 bg-[#1E2328]' : 'border-gray-200 bg-white'}`}>
          <button
            onClick={() => {
              onClose();
    ...
    <div className={`p-4 min-h-screen ${isDarkMode ? 'bg-[#121418]' : 'bg-[#FAFAFA]'}`}>
          ...
          <div className="w-full lg:w-[30%] space-y-4 lg:sticky lg:top-4 lg:self-start lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto">
            ...
            <div className={`rounded-xl border p-4 ${
              ...
              <div className="flex items-center justify-between mb-3">
                <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Products with Prices</span>
              ...
              <div className="flex items-center gap-3">
                <div className={`flex-1 flex items-center gap-2 px-2.5 py-1.5 rounded-lg ${isDarkMode ? 'bg-green-900/20' : 'bg-green-50'}`}>
                  <TrendingUp size={14} className="text-green-500" />
                  <span className={`text-sm font-semibold ${isDarkMode ? 'text-green-400' : 'text-green-700'}`}>
                    {stats.increasedPrices}
                  </span>
                  <span className={`text-xs ${isDarkMode ? 'text-green-400/70' : 'text-green-600'}`}>up</span>
                </div>
                <div className={`flex-1 flex items-center gap-2 px-2.5 py-1.5 rounded-lg ${isDarkMode ? 'bg-red-900/20' : 'bg-red-50'}`}>
                  <TrendingDown size={14} className="text-red-500" />
                  <span className={`text-sm font-semibold ${isDarkMode ? 'text-red-400' : 'text-red-700'}`}>
                    {stats.decreasedPrices}
                  </span>
                  <span className={`text-xs ${isDarkMode ? 'text-red-400/70' : 'text-red-600'}`}>down</span>
                </div>
              </div>
                  ...
                  <>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead className={`sticky top-0 ${isDarkMode ? 'bg-[#1E2328]' : 'bg-white'}`}>
                          <tr className={`border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                            <th className={`text-left py-3 px-3 text-xs font-semibold uppercase ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Product</th>
                            <th className={`text-right py-3 px-3 text-xs font-semibold uppercase ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Current Price</th>
                            <th className={`text-right py-3 px-3 text-xs font-semibold uppercase ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>New Price</th>
                            <th className={`text-right py-3 px-3 text-xs font-semibold uppercase ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Change</th>
                          </tr>
                        </thead>
                        <tbody>
                          {filteredProducts.map((product) => {
                            ...
                            const margin = newPrice && costPrice ? calculateMargin(parseFloat(newPrice), costPrice) : null;
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className={`py-3 px-3 text-right text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                                  {formData.currency} {product.sellingPrice?.toFixed(2) || '0.00'}
                                  <div className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                                    Cost: {formData.currency} {costPrice?.toFixed(2) || '0.00'}
                                  </div>
                                </td>
                                <td className="py-3 px-3">
      ...
      {showBulkDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/50" onClick={() => setShowBulkDialog(false)} />
