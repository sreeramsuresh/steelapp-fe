  ...
  const filteredPricelists = useMemo(() => {
    return pricelists.filter(pl => {
      ...
      if (searchTerm) {
        ...
        if (!pl.name?.toLowerCase().includes(search) &&
      ...
      <div className={`rounded-xl border p-6 ${
              ...
              {STATUS_FILTERS.map(filter => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`px-1.5 rounded text-xs ${isActive ? 'bg-white/20' : isDarkMode ? 'bg-gray-600' : 'bg-gray-100'}`}>
                      {count}
                    </span>
                  </button>
                );
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {CURRENCY_FILTERS.map(filter => {
                ...
                const count = filter.id === 'all'
                  ? pricelists.length
                  : pricelists.filter(pl => pl.currency === filter.id).length;
                return (
                  <button
                    key={filter.id}
        ...
        <div className={`flex flex-wrap items-center gap-4 py-2 mb-3 text-xs border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <div className="flex items-center gap-1">
            ...
            <span className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{filteredPricelists.length}</span>
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex-1">
                      ...
                      <p className={`text-sm mb-2 line-clamp-2 min-h-[2.5rem] ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    ...
                    <div className="flex flex-col gap-1 min-h-[6.5rem]">
                      <button
                        ...
                        className={`p-1.5 rounded transition-colors ${
                          isDarkMode ? 'text-teal-400 hover:text-teal-300 hover:bg-gray-700' : 'text-teal-600 hover:bg-gray-100'
                        }`}
                        title="Edit"
                      >
                        <Edit size={16} />
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded transition-colors ${
                          isDarkMode ? 'text-blue-400 hover:text-blue-300 hover:bg-gray-700' : 'text-blue-600 hover:bg-gray-100'
                        }`}
                        title="Copy"
                      >
                        <Copy size={16} />
                      ...
                      {!pricelist.isDefault ? (
                        <button
                          ...
                          className={`p-1.5 rounded transition-colors ${
                            isDarkMode ? 'text-red-400 hover:text-red-300 hover:bg-gray-700' : 'text-red-600 hover:bg-gray-100'
                          }`}
                          title="Delete"
                        >
                          <Trash2 size={16} />
                  ...
                  <div className="mt-auto">
                    <div className="flex justify-between items-center mb-1">
                      <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Products Coverage</span>
                    </div>
                    <div className={`w-full rounded-full h-2 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                      <div
                        className="h-2 rounded-full bg-teal-500 transition-all duration-300"
                        style={{ width: `${Math.min((pricelist.productCount || 0) * 5, 100)}%` }}
                      />
                    </div>
