...
} from "../hooks/useCreditNoteDrafts";
...
const CreditNoteList = ({ preSelectedInvoiceId }) => {
  ...
  const handlePreview = async (creditNote) => {
    try {
      ...
      const fullCreditNote = await creditNoteService.getCreditNote(
        creditNote.id,
      );
  ...
  const handleDownloadPDF = async (creditNote) => {
    ...
    if (!creditNote.id) {
      notificationService.warning(
        "Credit note must be saved before downloading PDF.",
      );
      return;
    }
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {draft.data?.totalCredit > 0 && (
                      <span
                        className={`text-sm font-medium mr-2 ${isDarkMode ? "text-red-400" : "text-red-600"}`}
                      >
                    ...
                    >
                      ...
                      >
                        <div className="font-medium flex items-center">
                          ...
                          <NewBadge
                            createdAt={
                              creditNote.createdAt || creditNote.created_at
                            }
                          />
                        </div>
                      </td>
                      ...
                      <td
                        className={`px-6 py-4 whitespace-nowrap font-medium text-red-600`}
                      >
                        ...
                        <CreditNoteStatusActions
                          ...
                          onOpenRefundModal={() => {
                            ...
                            const method = window.prompt(
                              "Refund method (cash, bank_transfer, cheque):",
                            );
                            if (method) {
                              creditNoteService
