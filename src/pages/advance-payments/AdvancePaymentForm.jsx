...
export function AdvancePaymentForm({ advance, companyId, onSave, onClose }) {
  ...
  useEffect(() => {
    if (formData.currency !== "AED") {
      const amountInBase = parseFloat(formData.amount || 0) * parseFloat(formData.exchangeRate || 1);
      setFormData((prev) => ({
        ...prev,
  ...
  const validateForm = () => {
    ...
    if (formData.paymentMethod === "bank_transfer" && !formData.transactionId && !isEditing) {
      // Warning, not error
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditing && ["FULLY_APPLIED", "REFUNDED", "EXPIRED"].includes(formData.status)) {
  ...
  const isEditingDisabled = isEditing && ["FULLY_APPLIED", "REFUNDED", "EXPIRED"].includes(formData.status);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <details className="border rounded-lg overflow-hidden">
            ...
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <Label className={labelClass}>Advance Number {isEditing && "(readonly)"}</Label>
                  <Input
          ...
          <details open className="border rounded-lg overflow-hidden">
            ...
            >
              <h3 className={`font-semibold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}>Customer Details</h3>
              <ChevronDown className="w-5 h-5" />
            </summary>
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <Select value={formData.customerId} onValueChange={handleCustomerChange} disabled={isEditingDisabled}>
                    <SelectTrigger className={inputClass}>
                ...
                <div>
                  ...
                  <Input value={formData.customerName} disabled={true} className={readonlyClass} />
                </div>
                <div>
                  <Label className={labelClass}>Customer TRN (readonly)</Label>
                  <Input value={formData.customerTrn} disabled={true} className={readonlyClass} />
                </div>
              </div>
            </div>
          </details>
          ...
          <details open className="border rounded-lg overflow-hidden">
            ...
            >
              <h3 className={`font-semibold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}>Payment Details</h3>
              <ChevronDown className="w-5 h-5" />
            </summary>
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="border-t pt-4 space-y-3">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => handleChange("isVatInclusive", e.target.checked)}
                    disabled={isEditingDisabled}
                    className="w-4 h-4"
                  />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label className={labelClass}>VAT Amount (auto-calculated)</Label>
                    <Input value={formData.vatAmount.toFixed(2)} disabled={true} className={readonlyClass} />
                  </div>
                  <div>
                    <Label className={labelClass}>Total Received (auto-calculated)</Label>
                    <Input value={formData.totalReceived.toFixed(2)} disabled={true} className={readonlyClass} />
                  </div>
                </div>
                ...
                {formData.paymentMethod === "cheque" && (
                  <div>
                    ...
                    {validationErrors.chequeNumber && (
                      <p className="text-red-500 text-sm mt-1">{validationErrors.chequeNumber}</p>
                    )}
                  </div>
                )}
                {formData.paymentMethod === "bank_transfer" && (
                  <div>
                    <Label className={labelClass}>Transaction ID (recommended)</Label>
                    <Input
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  {validationErrors.exchangeRate && (
                    <p className="text-red-500 text-sm mt-1">{validationErrors.exchangeRate}</p>
                  )}
                </div>
                <div>
                  <Label className={labelClass}>Amount in AED (auto-calculated)</Label>
                  <Input value={formData.amountInBaseCurrency.toFixed(2)} disabled={true} className={readonlyClass} />
                </div>
              </div>
            </div>
          </details>
            ...
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input value={formData.amountAvailable.toFixed(2)} disabled={true} className={readonlyClass} />
                </div>
                <div>
                  <Label className={labelClass}>Amount Applied</Label>
                  <Input value={formData.amountApplied.toFixed(2)} disabled={true} className={readonlyClass} />
                </div>
                <div>
                      ...
                      <tbody>
                        {formData.applications.map((app, idx) => (
                          ...
                          >
                            ...
                            <td className="p-2 text-right">{parseFloat(app.amountApplied || 0).toFixed(2)}</td>
                            <td className="p-2">
              ...
              {formData.status === "RECEIVED" && (
                <div className="mt-4">
                  <Button type="button" variant="outline" disabled={true} className={inputClass}>
          ...
          <details className="border rounded-lg overflow-hidden">
            ...
            >
              <h3 className={`font-semibold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}>Refund Information</h3>
              <ChevronDown className="w-5 h-5" />
            </summary>
            <div className="p-4 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => handleChange("refundReference", e.target.value)}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => handleChange("amountRefunded", e.target.value)}
                      className={inputClass}
                      disabled={isEditingDisabled}
                    />
                  </div>
          ...
          <details className="border rounded-lg overflow-hidden">
            ...
            >
              <h3 className={`font-semibold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}>VAT Period</h3>
              <ChevronDown className="w-5 h-5" />
            </summary>
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="date" value={formData.vatPeriodStart} disabled={true} className={readonlyClass} />
                </div>
                <div>
                  <Label className={labelClass}>VAT Period End (readonly)</Label>
                  <Input type="date" value={formData.vatPeriodEnd} disabled={true} className={readonlyClass} />
                </div>
              </div>
              <div className="text-sm text-gray-500 italic">
                VAT on this advance is reportable in Form 201 for the period above. The VAT period is determined by the
                payment date.
              </div>
            </div>
          </details>
