...
const CERTIFICATE_TYPES = [
  { value: 'mill_test_certificate', label: 'Mill Test Certificate (MTC)', icon: FileText, color: 'blue' },
  { value: 'certificate_of_origin', label: 'Certificate of Origin (COO)', icon: Award, color: 'green' },
  { value: 'certificate_of_analysis', label: 'Certificate of Analysis (COA)', icon: Beaker, color: 'purple' },
  { value: 'inspection_report', label: 'Inspection Report', icon: FileCheck, color: 'orange' },
  { value: 'quality_certificate', label: 'Quality Certificate', icon: Shield, color: 'teal' },
];

// Verification Status Configuration
const VERIFICATION_STATUS = {
  pending: { label: 'Pending', color: 'yellow', bgLight: 'bg-yellow-100', bgDark: 'bg-yellow-900/30', textLight: 'text-yellow-700', textDark: 'text-yellow-300', icon: Clock },
  verified: { label: 'Verified', color: 'green', bgLight: 'bg-green-100', bgDark: 'bg-green-900/30', textLight: 'text-green-700', textDark: 'text-green-300', icon: CheckCircle },
  rejected: { label: 'Rejected', color: 'red', bgLight: 'bg-red-100', bgDark: 'bg-red-900/30', textLight: 'text-red-700', textDark: 'text-red-300', icon: XCircle },
};
...
const MaterialCertificateList = () => {
  ...
  const loadCertificates = useCallback(async (page = 1) => {
    try {
      ...
      const response = await materialCertificateService.getMaterialCertificates(params);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const dataToSend = {
        ...formData,
        chemical_c: formData.chemical_c ? parseFloat(formData.chemical_c) : null,
        chemical_si: formData.chemical_si ? parseFloat(formData.chemical_si) : null,
        chemical_mn: formData.chemical_mn ? parseFloat(formData.chemical_mn) : null,
        chemical_p: formData.chemical_p ? parseFloat(formData.chemical_p) : null,
        chemical_s: formData.chemical_s ? parseFloat(formData.chemical_s) : null,
        chemical_cr: formData.chemical_cr ? parseFloat(formData.chemical_cr) : null,
        chemical_ni: formData.chemical_ni ? parseFloat(formData.chemical_ni) : null,
        chemical_mo: formData.chemical_mo ? parseFloat(formData.chemical_mo) : null,
        chemical_cu: formData.chemical_cu ? parseFloat(formData.chemical_cu) : null,
        chemical_n: formData.chemical_n ? parseFloat(formData.chemical_n) : null,
      ...
      if (modalMode === 'create') {
        ...
        await materialCertificateService.updateMaterialCertificate(selectedCertificate.id, dataToSend);
  ...
  const handleVerifySubmit = async () => {
    try {
      ...
      await materialCertificateService.updateVerification(verifyingId, status, verifyNotes);
    ...
    <div className={`p-6 min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-4 mb-6 shadow-sm`}>
        <div className="flex flex-col md:flex-row gap-4">
          ...
          >
            ...
            {(filters.certificate_type || filters.verification_status || filters.grade || filters.start_date || filters.end_date) && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {Object.entries(VERIFICATION_STATUS).map(([value, config]) => (
      ...
      {showModal && (modalMode === 'create' || modalMode === 'edit') && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div className={`relative w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-lg shadow-xl ${
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold">
                    {modalMode === 'create' ? 'New Material Certificate' : 'Edit Material Certificate'}
                  </h2>
                  <p className="text-sm text-gray-500">
                    {modalMode === 'create' ? 'Create a new MTC, COO, COA or other certificate' : `Editing ${selectedCertificate?.certificate_number}`}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    {formErrors.certificate_type && (
                      <p className="text-red-500 text-xs mt-1">{formErrors.certificate_type}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {formErrors.certificate_number && (
                      <p className="text-red-500 text-xs mt-1">{formErrors.certificate_number}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleInputChange('mill_name', e.target.value)}
                    ...
                    {formErrors.mill_name && (
                      <p className="text-red-500 text-xs mt-1">{formErrors.mill_name}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1">Import Order</label>
                    <select
                    ...
                    >
                      ...
                      {importOrders.map(order => (
                        <option key={order.id} value={order.id}>
                          {order.import_order_number || order.importOrderNumber} - {order.supplier_name || order.supplierName}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1">Line Item (Product)</label>
                    <select
                      value={formData.import_order_item_id}
                      onChange={(e) => handleInputChange('import_order_item_id', e.target.value)}
                      disabled={!formData.import_order_id || orderItems.length === 0}
                      className={`w-full px-3 py-2 border rounded-lg ${
                        isDarkMode
                          ? 'bg-gray-700 border-gray-600 text-white'
                          : 'bg-white border-gray-300'
                    ...
                    >
                      ...
                      {orderItems.map(item => (
                        <option key={item.id} value={item.id}>
                          {item.product_name || item.productName} - {item.quantity} {item.unit}
                        </option>
                      ))}
                    </select>
                  </div>
              ...
              <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1">
                      Heat Number {formData.certificate_type === 'mill_test_certificate' && <span className="text-red-500">*</span>}
                    </label>
                    <input
                      type="text"
                      value={formData.heat_number}
                      onChange={(e) => handleInputChange('heat_number', e.target.value)}
                    ...
                    {formErrors.heat_number && (
                      <p className="text-red-500 text-xs mt-1">{formErrors.heat_number}</p>
                    )}
                  </div>

                  <div>
                    <label className="block text-sm font-medium mb-1">Coil ID (Optional)</label>
                    <input
                      type="text"
                      value={formData.coil_id}
                      onChange={(e) => handleInputChange('coil_id', e.target.value)}
                  ...
                  {formData.grade === 'OTHER' && (
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleInputChange('grade_other', e.target.value)}
                      ...
                      {formErrors.grade_other && (
                        <p className="text-red-500 text-xs mt-1">{formErrors.grade_other}</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    {formErrors.issue_date && (
                      <p className="text-red-500 text-xs mt-1">{formErrors.issue_date}</p>
                    )}
                  </div>
              ...
              {formData.certificate_type === 'mill_test_certificate' && (
                <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-purple-900/20 border border-purple-800' : 'bg-purple-50 border border-purple-200'}`}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-1">Yield Strength (MPa)</label>
                      <input
                        type="number"
                        step="0.1"
                        value={formData.yield_strength}
                        onChange={(e) => handleInputChange('yield_strength', e.target.value)}
                        placeholder="e.g., 205"
                        className={`w-full px-3 py-2 border rounded-lg ${
                          isDarkMode
                            ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400'
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1">Tensile Strength (MPa)</label>
                      <input
                        type="number"
                        step="0.1"
                        value={formData.tensile_strength}
                        onChange={(e) => handleInputChange('tensile_strength', e.target.value)}
                        placeholder="e.g., 520"
                        className={`w-full px-3 py-2 border rounded-lg ${
                          isDarkMode
                            ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400'
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1">Elongation (%)</label>
                      <input
                        type="number"
                        step="0.1"
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1">Hardness (HRB/HRC)</label>
                      <input
                        type="number"
                        step="0.1"
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Certificate PDF</label>
            ...
            <div className={`sticky bottom-0 flex gap-3 p-6 border-t ${
              ...
              >
                {saving ? 'Saving...' : modalMode === 'create' ? 'Create Certificate' : 'Save Changes'}
              </button>
            </div>
          </div>
        </div>
                ...
                <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/30' : 'bg-gray-50'}`}>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{getGradeLabel(selectedCertificate.grade, selectedCertificate.grade_other)}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    {[
                      ...
                      <div key={key} className="text-center">
                        ...
                        <div className="font-mono font-medium">
                          {selectedCertificate[key] ? selectedCertificate[key].toFixed(3) : '-'}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-purple-900/20 border border-purple-800' : 'bg-purple-50 border border-purple-200'}`}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-xs text-gray-500 mb-1">Yield Strength</div>
                      <div className="text-lg font-medium">
                        {selectedCertificate.yield_strength ? `${selectedCertificate.yield_strength} MPa` : '-'}
                      </div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs text-gray-500 mb-1">Tensile Strength</div>
                      <div className="text-lg font-medium">
                        {selectedCertificate.tensile_strength ? `${selectedCertificate.tensile_strength} MPa` : '-'}
                      </div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs text-gray-500 mb-1">Elongation</div>
                      <div className="text-lg font-medium">
                        {selectedCertificate.elongation ? `${selectedCertificate.elongation}%` : '-'}
                      </div>
                    </div>
                    <div className="text-center">
                ...
                <div className={`p-4 rounded-lg ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <span className="font-medium ml-2">{selectedCertificate.verified_by || '-'}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Date:</span>
                      <span className="font-medium ml-2">{formatDate(selectedCertificate.verified_date)}</span>
                    </div>
                    {selectedCertificate.rejection_reason && (
                      <div className="md:col-span-3">
                        <span className="text-gray-500">Reason:</span>
                        <span className="font-medium ml-2">{selectedCertificate.rejection_reason}</span>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div className={`relative w-full max-w-md rounded-lg shadow-xl ${
            ...
            <div className="p-6">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">
                  {verifyAction === 'verify' ? 'Notes (Optional)' : 'Rejection Reason'}
                  {verifyAction === 'reject' && <span className="text-red-500">*</span>}
                </label>
                <textarea
                  value={verifyNotes}
                  onChange={(e) => setVerifyNotes(e.target.value)}
                  placeholder={verifyAction === 'verify' ? 'Add any verification notes...' : 'Enter reason for rejection...'}
                  rows={4}
                  className={`w-full px-3 py-2 border rounded-lg ${
                    isDarkMode
                      ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400'
