...
const CreditNoteForm = () => {
  ...
  const isPhysicalReturn = useMemo(() => {
    ...
    if (
      ...
      isPhysicalReturnReason(creditNote.reasonForReturn)
    ) {
      return true;
    }
    ...
    if (
      creditNote.reasonForReturn &&
      isFinancialOnlyReason(creditNote.reasonForReturn)
    ) {
      return false;
    }
  ...
  const hasSelectedItems = useMemo(() => {
    return creditNote.items.some(
      (item) => item.selected && item.quantityReturned > 0,
    );
  }, [creditNote.items]);
  ...
  const currentInvoiceId =
    creditNote.invoiceId || searchParams.get("invoiceId");
  ...
  const loadNextCreditNoteNumber = async () => {
    try {
      ...
      const nextNumber =
        response.nextNumber || response.nextNumber || "CN-0001";
    ...
    try {
      ...
      if (normalizedStatus !== "issued") {
        notificationService.warning(
          "Credit notes can only be created for Final Tax Invoices",
        );
        return;
      }
  ...
  const handleQuantityChange = (index, quantity) => {
    ...
    if (qty > item.originalQuantity) {
      notificationService.warning(
        `Cannot return more than ${item.originalQuantity} units`,
      );
      return;
    }
    ...
    if (
      ...
      const originalItemShare =
        item.originalItemAmount / creditNote.invoiceSubtotal;
      const itemsFullDiscount =
        creditNote.invoiceDiscountValue * originalItemShare;
      // Scale by proportion of quantity returned
      const returnRatio =
        item.originalQuantity > 0 ? qty / item.originalQuantity : 0;
  ...
  const recalculateTotals = (items) => {
    ...
    const discountAmount = returnedItems.reduce(
      (sum, item) => sum + (item.discountAmount || 0),
      0,
    );
    const netTaxable = subtotal - discountAmount;
    const vatAmount = returnedItems.reduce(
  ...
  const validateForm = () => {
    ...
    const touchedFieldsSet = new Set([
      "invoiceId",
      "creditNoteDate",
      "reasonForReturn",
      "creditNoteType",
    ...
    const returnedItems = creditNote.items.filter(
      (item) => item.selected && item.quantityReturned > 0,
    );
    ...
    if (isPhysicalReturn) {
      ...
      if (!hasItems && !hasManualAmount) {
        errors.push(
          "Either select items to credit OR enter a manual credit amount",
        );
      ...
      if (hasManualAmount) {
        ...
        if (!creditNote.refundMethod) {
          errors.push(
            "Please specify how the credit will be settled (Settlement Method)",
          );
        ...
        if (
          ...
          METHODS_REQUIRING_REFERENCE.includes(creditNote.refundMethod)
        ) {
          touchedFieldsSet.add("refundReference");

          if (!creditNote.refundReference) {
            const methodLabel =
              REFUND_METHODS.find((m) => m.value === creditNote.refundMethod)
                ?.label || creditNote.refundMethod;
    ...
    returnedItems.forEach((item, idx) => {
      if (item.quantityReturned > item.originalQuantity) {
        errors.push(
          `Item "${item.productName}": Cannot return more than original quantity`,
        );
      }
      // Validate quantity precision for PCS/BUNDLE units
      const precisionResult = validateQuantityPrecision(
  ...
  const handleSave = async (issueImmediately = false) => {
    if (!validateForm()) {
      setTimeout(() => {
        document
          .getElementById("validation-errors-alert")
    ...
    try {
      ...
      if (creditNote.creditNoteType === "RETURN_WITH_QC") {
        ...
        itemsToSave = creditNote.items.filter(
          (item) => item.selected && item.quantityReturned > 0,
        );
      } else if (
      ...
      if (id) {
        ...
        notificationService.success(
          issueImmediately
            ? "Tax document issued successfully"
      ...
      notificationService.error(
        error?.response?.data?.error || "Failed to save credit note",
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              >
                {id
                  ? `Editing ${creditNote.creditNoteNumber}`
        ...
        {!isEditable && (
          ...
          >
            <div className="flex items-start gap-3">
              <AlertTriangle
                className={`flex-shrink-0 h-5 w-5 ${isDarkMode ? "text-amber-400" : "text-amber-600"}`}
              />
              <div className="flex-1">
                <p
                  className={`font-semibold mb-1 ${isDarkMode ? "text-amber-300" : "text-amber-900"}`}
                >
                ...
                >
                  This credit note is{" "}
                  <span className="font-semibold">{creditNote.status}</span> and
                  cannot be edited.
                  {creditNote.status === "issued" && (
                    <>
                      {" "}
                      Once issued, credit notes become tax documents and must
                      remain unchanged per UAE VAT compliance requirements. To
                      make corrections, please use the{" "}
                  ...
                  {["applied", "refunded", "completed"].includes(
                    creditNote.status,
                  ) && (
                    <>
                      {" "}
                      This credit note has financial impact (customer balance or
                      payments affected). Contact the finance team for guidance
                      on corrections.
                    </>
                  )}
                  {["items_received", "items_inspected"].includes(
                    creditNote.status,
                  ) && (
                    <>
                      {" "}
                      Physical goods have been received/inspected. Amounts and
                      items are locked. Contact operations team if corrections
                      are needed.
                    </>
                  )}
                </p>
          ...
          <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="relative">
                    ...
                    {searchResults.length > 0 && (
                      ...
                      >
                        ...
                        {/* Results Count */}
                        <span
                          className={`ml-auto ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                        >
            ...
            {selectedInvoice && (
              ...
              >
                ...
                <div
                  className={`grid grid-cols-2 gap-4 text-sm ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}
                >
                  <div>
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  >
                    ...
                    Select Items to {isPhysicalReturn
                      ? "Return"
                      : "Credit"}{" "}
                    {itemsRequired && (
                      <span className="text-red-500 font-bold">*</span>
                ...
                {!isPhysicalReturn && (
                  <p
                    className={`mb-4 text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                  >
                    💡 For financial-only credit, you can either select items
                    below OR enter a manual credit amount in the sidebar.
                  </p>
                )}
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <div
                              className={`text-right text-sm ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}
                            >
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2 rounded-lg border transition-colors ${
                      ...
                      shouldShowError("creditNoteDate") ||
                ...
                {id && (
                  <div>
                    ...
                    >
                      {CREDIT_NOTE_STATUSES.find(
                        (s) => s.value === creditNote.status,
                  ...
                  >
                    ...
                    <optgroup label="Physical Return (Items Required)">
                      {RETURN_REASONS.filter(
                        (r) => r.category === REASON_CATEGORIES.PHYSICAL_RETURN,
                      ).map((reason) => (
                    ...
                    <optgroup label="Financial Only (No Return)">
                      {RETURN_REASONS.filter(
                        (r) => r.category === REASON_CATEGORIES.FINANCIAL_ONLY,
                      ).map((reason) => (
                        <option key={reason.value} value={reason.value}>
                          {reason.label}
                        </option>
            ...
            {((creditNote.creditNoteType === "ACCOUNTING_ONLY" &&
              ...
              >
                ...
                >
                  Specify how this credit will be applied or refunded to the
                  customer.
                </p>
                <div className="space-y-4">
                  <div>
                    ...
                    <p
                      className={`text-xs mt-1 ${isDarkMode ? "text-gray-500" : "text-gray-500"}`}
                    >
                      Credit Adjustment: Apply to customer account • Offset:
                      Apply to specific invoice • Bank/Cheque/Cash: Direct
                      refund
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {creditNote.refundMethod &&
                        METHODS_REQUIRING_REFERENCE.includes(
                          creditNote.refundMethod,
                        ) && <span className="text-red-500 ml-1">*</span>}
                    </label>
                    <input
            ...
            {creditNote.creditNoteType === "RETURN_WITH_QC" &&
              ["items_inspected", "applied", "refunded", "completed"].includes(
                creditNote.status,
              ) && (
                <div
                  className={`p-6 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-white"} shadow-sm`}
            ...
            {!isPhysicalReturn && (
              <div
                className={`p-6 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-white"} shadow-sm ${
                  invalidFields.has("manualCreditAmount")
                    ? "ring-2 ring-red-500"
                    : ""
                }`}
              >
                ...
                <p
                  className={`mb-4 text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                >
                  Enter a credit amount directly if not selecting specific
                  items.{" "}
                  {!hasSelectedItems && (
                    <span className="text-red-500 font-bold">*</span>
                  )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-2 rounded-lg border transition-colors ${
                        shouldShowError("manualCreditAmount") ||
                        invalidFields.has("manualCreditAmount")
                          ? "border-red-500 ring-1 ring-red-500 focus:ring-red-500 focus:border-red-500"
                          : isDarkMode
                    ...
                    <p
                      className={`mt-1 text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
                    >
                      Use this for goodwill credits, pricing adjustments, or
                      other financial-only credits
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div className="space-y-3">
                    ...
                    <>
                      ...
                      {creditNote.discountAmount > 0 && (
                        <div className="flex justify-between">
                          ...
                          >
                            {formatCurrency(
                              creditNote.subtotal - creditNote.discountAmount,
                            )}
                          </span>
                        </div>
                      )}
                  ...
                  >
                    ...
                    >
                      {formatCurrency(
                        creditNote.totalCredit + creditNote.manualCreditAmount,
                      )}
                    </span>
                  </div>
