...
const STATUS_OPTIONS = [
  { value: 'pending', label: 'Pending', color: 'gray', bgColor: 'bg-gray-100', textColor: 'text-gray-800' },
  { value: 'submitted', label: 'Submitted', color: 'blue', bgColor: 'bg-blue-100', textColor: 'text-blue-800' },
  { value: 'under_review', label: 'Under Review', color: 'yellow', bgColor: 'bg-yellow-100', textColor: 'text-yellow-800' },
  { value: 'assessed', label: 'Assessed', color: 'purple', bgColor: 'bg-purple-100', textColor: 'text-purple-800' },
  { value: 'payment_pending', label: 'Payment Pending', color: 'orange', bgColor: 'bg-orange-100', textColor: 'text-orange-800' },
  { value: 'examination_required', label: 'Examination Required', color: 'pink', bgColor: 'bg-pink-100', textColor: 'text-pink-800' },
  { value: 'cleared', label: 'Cleared', color: 'green', bgColor: 'bg-green-100', textColor: 'text-green-800' },
  { value: 'rejected', label: 'Rejected', color: 'red', bgColor: 'bg-red-100', textColor: 'text-red-800' },
  { value: 'on_hold', label: 'On Hold', color: 'amber', bgColor: 'bg-amber-100', textColor: 'text-amber-800' },
];

// Common HS codes for stainless steel products
const HS_CODES = [
  { code: '7219.11', description: 'Hot-rolled stainless steel coils >10mm thick' },
  { code: '7219.12', description: 'Hot-rolled stainless steel coils 4.75-10mm thick' },
  { code: '7219.13', description: 'Hot-rolled stainless steel coils 3-4.75mm thick' },
  { code: '7219.14', description: 'Hot-rolled stainless steel coils <3mm thick' },
  { code: '7219.21', description: 'Hot-rolled stainless steel sheets >10mm thick' },
  { code: '7219.22', description: 'Hot-rolled stainless steel sheets 4.75-10mm thick' },
  { code: '7219.23', description: 'Hot-rolled stainless steel sheets 3-4.75mm thick' },
  { code: '7219.24', description: 'Hot-rolled stainless steel sheets <3mm thick' },
  { code: '7219.31', description: 'Cold-rolled stainless steel sheets >4.75mm thick' },
  { code: '7219.32', description: 'Cold-rolled stainless steel sheets 3-4.75mm thick' },
  { code: '7219.33', description: 'Cold-rolled stainless steel sheets 1-3mm thick' },
  { code: '7219.34', description: 'Cold-rolled stainless steel sheets 0.5-1mm thick' },
  { code: '7219.35', description: 'Cold-rolled stainless steel sheets <0.5mm thick' },
  { code: '7220.11', description: 'Hot-rolled stainless steel strips >4.75mm thick' },
  { code: '7220.12', description: 'Hot-rolled stainless steel strips <4.75mm thick' },
  { code: '7220.20', description: 'Cold-rolled stainless steel strips' },
  { code: '7221.00', description: 'Stainless steel bars - hot rolled' },
  { code: '7222.11', description: 'Stainless steel bars - circular cross-section' },
  { code: '7222.19', description: 'Stainless steel bars - other cross-sections' },
  { code: '7222.20', description: 'Stainless steel angles, shapes and sections' },
  { code: '7222.30', description: 'Stainless steel bars and rods' },
  { code: '7222.40', description: 'Stainless steel profiles' },
  { code: '7304.41', description: 'Stainless steel tubes and pipes - cold drawn' },
...
const validateCooExpiry = (issueDate) => {
  ...
  if (daysDiff > 120) {
    return { valid: false, message: `COO expired. Valid for 4 months. Issued ${daysDiff} days ago.` };
  }
  const daysRemaining = 120 - daysDiff;
  return { valid: true, message: `COO valid. ${daysRemaining} days remaining.` };
};
...
const CustomsDocumentList = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${statusConfig.bgColor} ${statusConfig.textColor}`}>
        {statusConfig.label}
      </span>
    );
  };
  ...
  return (
    <div className={`min-h-screen p-6 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      {showInfoPanel && (
        <div className={`mb-6 rounded-lg p-6 ${isDarkMode ? 'bg-blue-900/20 border border-blue-800' : 'bg-blue-50 border border-blue-200'}`}>
          <div className="flex justify-between items-start mb-4">
            <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-blue-300' : 'text-blue-800'}`}>
              UAE Customs Rules Reference
            </h3>
            <button onClick={() => setShowInfoPanel(false)} className="text-gray-500 hover:text-gray-700">
              <X size={20} />
            </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <h4 className={`font-medium mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Standard Duty Rate</h4>
            ...
            <div>
              ...
              <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Duty exemption with valid COO</p>
            </div>
          </div>
          <div className={`mt-4 pt-4 border-t ${isDarkMode ? 'border-blue-800' : 'border-blue-200'}`}>
            <h4 className={`font-medium mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Calculation Formula</h4>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className={`relative z-10 w-full max-w-4xl mx-4 rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {formErrors.document_type && (
                    <p className="text-red-500 text-sm mt-1">{formErrors.document_type}</p>
                  )}
                </div>
                  ...
                  >
                    ...
                    {importOrders.map(order => (
                      <option key={order.id} value={order.id}>
                        {order.importOrderNumber || order.import_order_number} - {order.supplierName || order.supplier_name}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={modalMode === 'view'}
                    className={`w-full px-3 py-2 border rounded-lg ${
                      formErrors.declaration_date ? 'border-red-500' : isDarkMode ? 'border-gray-600' : 'border-gray-300'
                    } ${isDarkMode ? 'bg-gray-700 text-white' : 'bg-white'} ${
                      modalMode === 'view' ? 'opacity-60 cursor-not-allowed' : ''
                    }`}
                  />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      {formErrors.cif_value && (
                        <p className="text-red-500 text-sm mt-1">{formErrors.cif_value}</p>
                      )}
                    </div>
                    ...
                    {/* Total Payable */}
                    <div>
                      <label className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-green-400' : 'text-green-700'}`}>
                        Total Payable (AED)
                      </label>
                      <input
                        type="text"
                        ...
                        className={`w-full px-3 py-2 border rounded-lg opacity-60 cursor-not-allowed font-bold ${
                          isDarkMode ? 'bg-green-900/30 border-green-600 text-green-400' : 'bg-green-100 border-green-300 text-green-700'
                        }`}
                      />
                    </div>
                  </div>
      ...
      {showCalculator && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-black bg-opacity-50" onClick={() => setShowCalculator(false)} />
          <div className={`relative z-10 w-full max-w-lg mx-4 rounded-xl shadow-2xl ${
            isDarkMode ? 'bg-gray-800' : 'bg-white'
          }`}>
            {/* Calculator Header */}
            <div className={`flex justify-between items-center px-6 py-4 border-b ${
              ...
              <button onClick={() => setShowCalculator(false)} className="p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded">
                <X size={24} />
              </button>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setCalculatorData(prev => ({ ...prev, cif_value: e.target.value }))}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setCalculatorData(prev => ({ ...prev, hs_code: e.target.value }))}
                    className={`w-full px-3 py-2 border rounded-lg ${
                      isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'
                    }`}
                  >
                ...
                <div>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setCalculatorData(prev => ({ ...prev, gcc_origin: e.target.checked }))}
                      className="w-4 h-4 rounded border-gray-300 text-teal-600 focus:ring-teal-500"
                    />
                    <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      GCC Origin (0% duty)
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCalculatorData(prev => ({ ...prev, duty_rate: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCalculatorData(prev => ({ ...prev, vat_rate: e.target.value }))}
                      className={`w-full px-3 py-2 border rounded-lg ${
                        isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'
                      }`}
                    />
                ...
                {calculatorResults && (
                  <div className={`mt-4 p-4 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                    ...
                    {calculatorData.gcc_origin && (
                      <div className={`mt-3 p-2 rounded ${isDarkMode ? 'bg-green-900/30 text-green-400' : 'bg-green-100 text-green-700'}`}>
                        <p className="text-sm">
