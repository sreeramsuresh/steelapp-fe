...
const DOCUMENT_TYPES = [
  ...
  { value: 'documentary_collection', label: 'Documentary Collection (DC)', icon: FileText },
...
const STATUS_OPTIONS = [
  { value: 'draft', label: 'Draft', color: 'gray', bgColor: 'bg-gray-100', textColor: 'text-gray-800' },
  { value: 'issued', label: 'Issued', color: 'blue', bgColor: 'bg-blue-100', textColor: 'text-blue-800' },
  { value: 'confirmed', label: 'Confirmed', color: 'purple', bgColor: 'bg-purple-100', textColor: 'text-purple-800' },
  { value: 'amended', label: 'Amended', color: 'yellow', bgColor: 'bg-yellow-100', textColor: 'text-yellow-800' },
  { value: 'utilized', label: 'Utilized', color: 'green', bgColor: 'bg-green-100', textColor: 'text-green-800' },
  { value: 'expired', label: 'Expired', color: 'red', bgColor: 'bg-red-100', textColor: 'text-red-800' },
  { value: 'cancelled', label: 'Cancelled', color: 'gray', bgColor: 'bg-gray-100', textColor: 'text-gray-600' },
];
...
const DOCUMENT_REQUIREMENTS = [
  ...
  { id: 'certificate_of_origin', label: 'Certificate of Origin', required: false },
  { id: 'mill_test_certificate', label: 'Mill Test Certificate', required: false },
  { id: 'insurance_certificate', label: 'Insurance Certificate', required: false },
  { id: 'inspection_certificate', label: 'Inspection Certificate', required: false },
  ...
  if (daysUntilExpiry < 0) {
    return (
      <div className="flex items-center gap-1">
        ...
        <span className="text-red-500 text-xs">({Math.abs(daysUntilExpiry)}d ago)</span>
      </div>
    );
  }
  ...
  useEffect(() => {
    if (editingRecord) {
      setFormData({
        ...getInitialFormState(),
        ...editingRecord,
        document_requirements: editingRecord.document_requirements || getInitialFormState().document_requirements,
        // For amendment, increment amendment number
        amendment_number: isAmendment ? (editingRecord.amendment_number || 0) + 1 : editingRecord.amendment_number || 0,
        amendment_date: isAmendment ? new Date().toISOString().split('T')[0] : editingRecord.amendment_date || '',
        amendment_details: isAmendment ? '' : editingRecord.amendment_details || '',
      });
    } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
      ...
      >
        ...
        <div className="px-6 py-4 max-h-[calc(100vh-200px)] overflow-y-auto">
          ...
          {/* Amendment Banner */}
          {isAmendment && (
            <div className={`mb-4 p-3 rounded-lg ${isDarkMode ? 'bg-yellow-900/30 border-yellow-700' : 'bg-yellow-50 border-yellow-300'} border`}>
              <div className="flex items-center gap-2">
                <RefreshCw size={16} className="text-yellow-600" />
                <span className="font-medium">Creating Amendment #{formData.amendment_number}</span>
              </div>
              <p className="text-sm mt-1 text-yellow-700 dark:text-yellow-400">
                Original document will be marked as amended. Please specify the changes in Amendment Details.
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {errors.reference_number && (
                <p className="text-red-500 text-xs mt-1">{errors.reference_number}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.issuing_bank && (
                <p className="text-red-500 text-xs mt-1">{errors.issuing_bank}</p>
              )}
            </div>
            ...
            <div>
              <label className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                Beneficiary (Seller/Supplier) <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
              ...
              />
              {errors.beneficiary && (
                <p className="text-red-500 text-xs mt-1">{errors.beneficiary}</p>
              )}
            </div>
            ...
            {showAdvanced && (
              <div className="mt-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {DOCUMENT_REQUIREMENTS.map(doc => (
                      <label key={doc.id} className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={formData.document_requirements[doc.id] || false}
                          onChange={(e) => handleDocRequirementChange(doc.id, e.target.checked)}
                ...
                <div>
                  ...
                  {formData.file && (
                    <p className="text-sm text-teal-600 mt-1">Selected: {formData.file.name}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="mt-6 space-y-4">
              ...
              <div>
                ...
                {errors.amendment_details && (
                  <p className="text-red-500 text-xs mt-1">{errors.amendment_details}</p>
                )}
              </div>
            </div>
          )}
...
const TradeFinanceList = () => {
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ['Reference', 'Type', 'Issuing Bank', 'Beneficiary', 'Amount', 'Currency', 'Issue Date', 'Expiry Date', 'Status'].join(','),
  ...
  const getLinkedOrderNumber = (importOrderId) => {
    ...
    return order ? (order.import_order_number || order.importOrderNumber || `#${importOrderId}`) : `#${importOrderId}`;
  };
    ...
    <div className={`p-6 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Manage Letters of Credit, Bank Guarantees, and other trade finance instruments
          </p>
        </div>
        ...
        <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-yellow-600">{summaryStats.expiringSoon}</p>
            </div>
          </div>
        </div>
        ...
        <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-red-600">{summaryStats.expired}</p>
            </div>
          </div>
        </div>
        ...
        <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{formatCurrency(summaryStats.totalAmount, 'USD')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-4 mb-6 shadow-sm`}>
        <div className="flex flex-wrap items-center gap-4">
          ...
          {/* Clear Filters */}
          {(filters.search || filters.document_type || filters.status || filters.expiry_filter || filters.start_date || filters.end_date) && (
            <button
              ...
              <tbody className={`${isDarkMode ? 'bg-gray-800 divide-gray-700' : 'bg-white divide-gray-200'} divide-y`}>
                {filteredRecords.map((record) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <DocumentTypeBadge type={record.document_type} isDarkMode={isDarkMode} />
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className="text-sm">{record.issuing_bank || '-'}</span>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className="text-sm">{record.beneficiary || '-'}</span>
                    </td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className="text-sm">{formatDate(record.issue_date)}</span>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
