...
import { formatCurrency, formatDateForInput, calculateItemAmount } from '../../utils/invoiceUtils';
...
const VendorBillForm = () => {
  ...
  useEffect(() => {
    localStorage.setItem('vendorBillFormPreferences', JSON.stringify(formPreferences));
  }, [formPreferences]);
  ...
  }, [bill.subtotal, bill.freightCharges, bill.customsDuty, bill.insuranceCharges, bill.handlingCharges, bill.otherCharges, bill.items]);
  ...
  const handleItemChange = (index, field, value) => {
    ...
    if (field === 'productId' && value) {
      ...
      if (product) {
        ...
        item.missingWeightWarning = (item.pricingBasis === 'PER_MT' || item.pricingBasis === 'PER_KG')
          && quantityUom === 'PCS'
          && !item.unitWeightKg;
      }
    } else {
    ...
    if (['quantity', 'unitPrice', 'vatRate', 'unitWeightKg', 'pricingBasis'].includes(field) || field === 'productId') {
      const qty = parseFloat(item.quantity) || 0;
      ...
      if (field === 'unitWeightKg' || field === 'pricingBasis') {
        item.missingWeightWarning = (pricingBasis === 'PER_MT' || pricingBasis === 'PER_KG')
          && quantityUom === 'PCS'
          && !unitWeightKg;
      }
    }
  ...
  const recalculateAllItems = (newVatRate) => {
    const updatedItems = bill.items.map((item) => {
      ...
      const amount = calculateItemAmount(qty, price, pricingBasis, unitWeightKg, quantityUom);
  ...
  const handleQuickAddProduct = useCallback((product) => {
    const newItem = {
      ...
      description: product.displayName || product.display_name || product.uniqueName || product.unique_name || product.description || '',
  ...
  const togglePinProduct = async (e, productId) => {
    ...
    try {
      if (pinnedProductIds.includes(productId)) {
        ...
        localStorage.setItem('vendorBillPinnedProducts', JSON.stringify(pinnedProductIds.filter(pid => pid !== productId)));
      } else {
        ...
        localStorage.setItem('vendorBillPinnedProducts', JSON.stringify(newPinned));
      }
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    bill.items.forEach((item, index) => {
      if (item.missingWeightWarning) {
        errors.push(`Item ${index + 1}: Unit weight is missing for "${item.description}". This product has weight-based pricing (${item.pricingBasis}) but no unit weight. Please contact admin to add unit weight to the product master.`);
      }
    });
  ...
  return (
    <div className={`h-full overflow-auto ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      {/* Sticky Header */}
      <header className={`sticky top-0 z-20 border-b shadow-sm ${
            ...
            <div className="flex gap-2 items-start relative">
              ...
              {bill.grnId && (
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${
                  isDarkMode ? 'bg-teal-900/30 text-teal-300 border border-teal-700' : 'bg-teal-50 text-teal-700 border border-teal-200'
                }`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleProcurementChannelChange(e.target.value)}
                    className={`w-full px-4 py-2 rounded-lg border ${
                      isDarkMode
                        ? 'border-gray-600 bg-gray-700 text-white'
                        : 'border-gray-300 bg-white text-gray-900'
                  ...
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    {bill.procurementChannel === 'IMPORTED' ? 'Reverse charge VAT applies' : 'Standard input VAT'}
                  </p>
                </div>
                ...
                {bill.procurementChannel === 'IMPORTED' && (
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const container = availableContainers.find(c => c.id === parseInt(e.target.value));
                        setBill((prev) => ({
                          ...prev,
                          importContainerId: e.target.value ? parseInt(e.target.value) : null,
                          importContainerNumber: container?.containerNumber || '',
                        }));
                      }}
                      className={`w-full px-4 py-2 rounded-lg border ${
                        isDarkMode
                      ...
                      <div key={product.id} className="relative group">
                        ...
                        >
                          <div className="font-medium truncate">
                            {product.displayName || product.display_name || product.uniqueName || product.unique_name || product.name}
                          </div>
                  ...
                  >
                    <div className="grid grid-cols-12 gap-4">
                      ...
                      {item.missingWeightWarning && (
                        <div className={`col-span-12 p-2 rounded-md border ${isDarkMode ? 'bg-amber-900/30 border-amber-600' : 'bg-amber-50 border-amber-200'}`}>
                      ...
                      <div className={`col-span-12 mt-2 pt-2 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                            ...
                            <div className={`px-2 py-1.5 rounded border text-xs ${
                              item.weightVarianceKg
                                ? item.weightVarianceKg > 0
                                  ? isDarkMode ? 'border-green-600 bg-green-900/20 text-green-400' : 'border-green-300 bg-green-50 text-green-700'
                                  : item.weightVarianceKg < 0
                                    ? isDarkMode ? 'border-red-600 bg-red-900/20 text-red-400' : 'border-red-300 bg-red-50 text-red-700'
                                    : isDarkMode ? 'border-gray-600 bg-gray-700/50 text-gray-400' : 'border-gray-300 bg-gray-100 text-gray-500'
                                : isDarkMode ? 'border-gray-600 bg-gray-700/50 text-gray-400' : 'border-gray-300 bg-gray-100 text-gray-500'
                            }`}>
                              {item.weightVarianceKg
              ...
              <div className={`space-y-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                    ...
                    {bill.procurementChannel === 'IMPORTED' ? 'Import' : 'Local'}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center pt-2">
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setBill(prev => ({ ...prev, triggerBatchCreation: e.target.checked }))}
      ...
      {showGRNMatchModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className={`w-full max-w-2xl max-h-[80vh] overflow-auto rounded-xl shadow-2xl ${
            ...
            <div className={`sticky top-0 px-6 py-4 border-b ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <p className={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Link this vendor bill to a Goods Receipt Note for 3-way matching (PO - GRN - Bill)
              </p>
            </div>
                        ...
                        <div>
                          ...
                          {grn.items && grn.items.length > 0 && (
                            <div className={`text-xs mt-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                              ...
                              Total: {formatCurrency(grn.totalAmount || 0)}
                            </div>
                          )}
                        </div>
