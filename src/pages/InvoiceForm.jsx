...
const InvoiceForm = ({ onSave }) => {
  ...
  const handleStatusChange = (newStatus) => {
    console.log('handleStatusChange called:', { newStatus, id, currentInvoiceId: invoice.id });
    const currentStatus = invoice.status || 'draft';

    // For new invoices (no id), allow any status selection freely WITHOUT confirmation
    // Just update the status directly - no side effects, no popups, no invoice number changes
    // Invoice number will be properly generated by backend when saving
    if (!id) {
      console.log('New invoice - updating status directly to:', newStatus);
      setInvoice((prev) => {
        console.log('Previous invoice state:', prev);
        const newState = {
          ...prev,
          status: newStatus
        };
      ...
      console.log('Status update completed, returning');
      return;
    }
    ...
    // Apply status change directly for edit mode
