...
const SkeletonCard = ({ isDarkMode, height = 'h-40' }) => (
  <div className={`animate-pulse rounded-xl border p-6 ${height} ${
...
const ExportOrderDetails = () => {
  ...
  const orderData = useMemo(() => {
    ...
    return {
      ...
      orderNumber: order.exportOrderNumber || order.export_order_number || order.orderNumber || order.order_number || 'N/A',
    ...
    return (
      <div className={`p-6 min-h-screen ${isDarkMode ? 'bg-[#121418]' : 'bg-[#FAFAFA]'}`}>
        <div className={`text-center p-12 rounded-2xl border ${
          ...
          <p className={`mb-6 max-w-md mx-auto ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            {error}
          </p>
  ...
  const subtotalAed = calculateAEDValue(orderData.subtotal, orderData.exchangeRate);
          ...
          {availableTransitions.length > 0 && (
            <div className="relative">
              ...
              >
                <RefreshCw size={18} />
                Update Status
                <ChevronDown size={16} className={`transform transition-transform ${statusDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              {statusDropdownOpen && (
                <>
                  <div
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {orderData.customerTrn && (
                <div>
                  ...
                  <p className={`font-medium font-mono flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                    <Hash size={14} />
                    {orderData.customerTrn}
                    <button
          ...
          <div className={`rounded-xl border overflow-hidden ${
            ...
            {expandedSections.vatCompliance && (
              <div className="px-4 pb-4">
                ...
                <div className={`border-l-4 border-green-500 p-4 rounded-r-lg ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className={`font-mono px-3 py-1.5 rounded inline-block mt-1 ${
                    ...
                    <div className={`mt-4 p-3 rounded-lg flex items-start gap-3 ${
                      ...
                      <div>
                        ...
                        <p className={`text-sm mt-1 ${isDarkMode ? 'text-green-400' : 'text-green-700'}`}>
                          This export qualifies for 0% VAT under UAE VAT Law Article 45.
                          Ensure all export documentation (customs declaration, BOL, proof of export)
                          is retained for FTA audit purposes.
                        </p>
                      </div>
                    </div>
                  )}
                    ...
                    {expandedSections.gcc && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div>
                          ...
                          <p className={`font-semibold flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                            <span className="text-lg">{getCountryFlag(orderData.gccCountryCode || orderData.destinationCountryCode)}</span>
                        ...
                        {orderData.customerGccVatId && (
                          <div>
                            ...
                            <p className={`font-mono font-medium flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                              {orderData.customerGccVatId}
                              <button
                    ...
                    orderData.items.map((item, index) => {
                      ...
                      return (
                        <tr key={item.id || index} className={`${isDarkMode ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}`}>
                          ...
                          <td className={`px-4 py-3 text-sm text-center ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                            {unit}
                          </td>
                          <td className={`px-4 py-3 text-sm text-right ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
            ...
            <div className={`border-t pt-4 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
              <div className="flex flex-wrap gap-2 mb-4">
                {DOCUMENT_TABS.map((tab) => {
                  ...
                  const docCount = orderData.documents.filter(d =>
                    d.category === tab.id || (!d.category && tab.id === 'export'),
                  ).length;
              ...
              <div className="space-y-2">
                {orderData.documents.filter(doc =>
                  doc.category === activeDocTab || (!doc.category && activeDocTab === 'export'),
                ).length === 0 ? (
                  ...
                  return (
                    <div key={statusKey} className="flex flex-col items-center relative">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
                        ...
                        <Icon size={14} className={
                          isCurrent
                            ? 'text-white'
                            : isActive
                              ? isDarkMode ? config.textDark : config.textLight
                              : isDarkMode ? 'text-gray-500' : 'text-gray-400'
                        } />
                      </div>
                      ...
                      {index < STATUS_ORDER.length - 1 && (
                        <div className={`absolute top-4 left-8 w-full h-0.5 -translate-y-1/2 ${
                          index < currentIndex
                            ? 'bg-teal-500'
                            : isDarkMode ? 'bg-gray-700' : 'bg-gray-200'
            ...
            <div className="space-y-3">
              ...
              <div className={`pt-3 border-t ${isDarkMode ? 'border-blue-700' : 'border-blue-200'}`}>
                <p className={`text-xs ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`}>
                  UAE VAT Law Article 45 - Exports of goods outside GCC implementing states are zero-rated.
                  Ensure export evidence (customs declaration, bill of lading, proof of export) is retained.
                </p>
              </div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                {pendingStatus && (() => {
                  ...
                  return (
                    <>
                      <div className={`p-2 rounded-full ${isDarkMode ? pendingConfig.bgDark : pendingConfig.bgLight}`}>
                        <PendingIcon size={20} className={isDarkMode ? pendingConfig.textDark : pendingConfig.textLight} />
                      </div>
                      <div>
