...
const AccountStatementList = ({ preSelectedCustomerId, preSelectedCustomerName }) => {
  ...
  const fetchStatements = useCallback(async () => {
    try {
      ...
      const statements = Array.isArray(response)
        ? response
        : Array.isArray(response.data)
          ? response.data
          : Array.isArray(response.accountStatements)
            ? response.accountStatements
      ...
      setTotalPages(pagination.totalPages || pagination.total_pages || Math.ceil((statements.length || 0) / 10) || 1);
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-6">
          ...
          >
            <div className="py-4">
              <div className="text-2xl font-bold text-teal-600">{statements.length}</div>
              <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Total Statements</p>
            </div>
          </div>
          <div
            className={`text-center border rounded-2xl shadow-sm ${
          ...
          >
            <div className="py-4">
              ...
              <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Total Outstanding</p>
            </div>
          </div>
        </div>
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        ...
                        <NewBadge createdAt={statement.createdAt || statement.created_at} />
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className={`rounded-xl max-w-md w-full ${isDarkMode ? "bg-[#1E2328]" : "bg-white"}`}>
            ...
            <div className="p-6">
              ...
              <p className={`mt-2 text-sm ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
                ℹ️ The statement will be archived (not permanently deleted) to maintain audit trail and comply with
