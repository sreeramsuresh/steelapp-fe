...
const POTab = ({ canManage }) => {
  ...
  const handleDownloadReceipt = async (payment, paymentIndex) => {
    ...
    if (!po) { alert('Unable to generate receipt. Missing PO information.'); return; }
    const companyInfo = JSON.parse(localStorage.getItem('companySettings') || '{}');
    setDownloadingReceiptId(payment.id);
    try {
      const poData = { invoiceNumber: po.poNo || po.poNumber, total: po.poValue || 0, payments: po.payments || [], customer: po.vendor || {} };
      const result = await generatePaymentReceipt(payment, poData, companyInfo, paymentIndex);
      if (!result.success) { alert(`Error generating receipt: ${result.error}`); }
    } catch (error) { console.error('Error downloading receipt:', error); alert('Failed to generate receipt.'); }
    finally { setDownloadingReceiptId(null); }
  };

  const handlePrintReceipt = async (payment, paymentIndex) => {
    const po = drawer.item;
    if (!po) { alert('Unable to print receipt. Missing PO information.'); return; }
    const companyInfo = JSON.parse(localStorage.getItem('companySettings') || '{}');
    setPrintingReceiptId(payment.id);
    try {
      const poData = { invoiceNumber: po.poNo || po.poNumber, total: po.poValue || 0, payments: po.payments || [], customer: po.vendor || {} };
      const result = await printPaymentReceipt(payment, poData, companyInfo, paymentIndex);
      if (!result.success) { alert(`Error printing receipt: ${result.error}`); }
    } catch (error) { console.error('Error printing receipt:', error); alert('Failed to print receipt.'); }
    finally { setPrintingReceiptId(null); }
  };

  const exportPOs = async () => {
    try {
      const params = { search: filters.q || undefined, status: filters.status === 'all' ? undefined : filters.status, start_date: filters.start || undefined, end_date: filters.end || undefined, vendor: filters.vendor || undefined, min_balance: filters.minBal || undefined, max_balance: filters.maxBal || undefined };
      const blob = await payablesService.exportDownload('pos', params, 'csv');
      downloadBlob(blob, 'pos.csv'); return;
    } catch (e) { console.warn('Backend export failed, falling back to client CSV'); }
    const headers = ['PO #','Vendor','PO Date','Due Date','Currency','PO Value','Paid To-Date','Balance','Status'];
    const rows = items.map(r => [r.poNo || r.poNumber, getVendorName(r), r.poDate || r.date, r.dueDate, r.currency || 'AED', getPOValue(r), getPaid(r), getBalance(r), r.status]);
                  ...
                  <td className="px-4 py-2 text-right" onClick={()=>openDrawer(row)}>
                    <div>
                      ...
                      {(row.payments && row.payments.filter(p => !p.voided).length > 0) && (
                        <div className="text-xs opacity-70">{row.payments.filter(p => !p.voided).length} payment{row.payments.filter(p => !p.voided).length !== 1 ? 's' : ''}</div>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-2" onClick={()=>openDrawer(row)}>
          ...
          <div className={`w-full max-w-md h-full overflow-auto ${isDarkMode ? 'bg-[#1E2328] text-white' : 'bg-white text-gray-900'} shadow-xl`}> 
            ...
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><div className="opacity-70">Due Date</div><div>{formatDate(drawer.item.dueDate)}</div></div>
                          ...
                          <div className="text-right flex items-center gap-2">
                            ...
                            {!p.voided && (
                              <div className="flex gap-1">
                                <button onClick={() => handlePrintReceipt(p, paymentIndex)} disabled={isPrinting}
                                  ...
                                  title="Print payment receipt"><Printer size={14} /></button>
                                <button onClick={() => handleDownloadReceipt(p, paymentIndex)} disabled={isDownloading}
                                  ...
                                  title="Download payment receipt"><Download size={14} /></button>
                              </div>
                            )}
                          </div>
                        </div>
              ...
              {canManage && drawer.item.balance > 0 ? (
                ...
                <div className="p-3 rounded border border-green-300 bg-green-50 text-green-700 text-sm flex items-center gap-2">
                  <CheckCircle size={18} /><span className="font-medium">Purchase Order Fully Paid</span>
                </div>
              ) : (
                    ...
                    return (
                      <>
                        <button className="px-3 py-2 rounded border" onClick={async()=>{
                          try { const blob = await payablesService.downloadPOVoucher(drawer.item.id, last.id); downloadBlob(blob, `voucher-${drawer.item.poNo || drawer.item.poNumber}-${last.id}.pdf`); }
                          catch(e) { notificationService.error('Failed to download voucher'); }
                        }}>Download voucher</button>
                        <button className="px-3 py-2 rounded border" onClick={async()=>{
                          try { await payablesService.emailPOVoucher(drawer.item.id, last.id); notificationService.success('Voucher emailed to vendor'); }
                          catch(e) { notificationService.error('Failed to email voucher'); }
