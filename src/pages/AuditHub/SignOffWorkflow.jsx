...
export default function SignOffWorkflow() {
  ...
  const { selectedDataset } = useAuditHub();
  ...
  const handleSignOff = async (stage) => {
    ...
    try {
      const newSignOff = await auditHubService.signOff(
        user.companyId,
        datasetId,
        stage,
        commentText
  ...
  const stages = [
    { name: 'PREPARED', label: 'Prepared', icon: CheckCircle, description: 'Initial verification' },
    { name: 'REVIEWED', label: 'Reviewed', icon: CheckCircle, description: 'Senior review' },
    { name: 'LOCKED', label: 'Locked', icon: Lock, description: 'Final approval' }
  ];
      ...
      <div className="bg-white dark:bg-slate-800 shadow-sm border-b border-slate-200 dark:border-slate-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-400">
            Multi-stage approval for period closing
          </p>
        </div>
      </div>
