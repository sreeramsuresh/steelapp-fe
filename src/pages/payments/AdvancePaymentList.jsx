...
const AdvancePaymentList = () => {
  ...
  const handleRefund = async (payment) => {
    ...
    const refundMethod = window.prompt(
      "Refund method (bank_transfer, cheque, cash):",
      "bank_transfer",
    );
  ...
  const handleDownloadReceipt = async (payment) => {
    try {
      await advancePaymentService.downloadReceipt(
        payment.id,
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            >
              UAE VAT Article 26: Payments received before invoice issuance
              create an immediate tax point. VAT must be declared in the period
              received.
            </p>
          </div>
          <button
            ...
            >
              ...
              {hasActiveFilters && (
                <span className="ml-1 px-1.5 py-0.5 text-xs bg-teal-600 text-white rounded-full">
                  {
                    [statusFilter, customerFilter, startDate, endDate].filter(
                      Boolean,
                    ).length
                  }
                </span>
                ...
                >
                  {payments.map((payment) => (
                    <tr
                      ...
                      onClick={() =>
                        navigate(`/payments/advance/${payment.id}`)
                      }
                    >
                      <td
