...
const Select = ({ label, children, error, className = '', required = false, ...props }) => {
  const { isDarkMode } = useTheme();
...
const getEmirateFromPort = (portCode) => {
  return EMIRATE_PORT_MAP[portCode] || { code: 'DXB', name: 'Dubai', port: 'Unknown' };
};
...
const UAE_DESIGNATED_ZONES = [
  ...
  { code: 'KIZAD', name: 'Khalifa Industrial Zone', emirate: 'Abu Dhabi', port: 'AEAUH' },
  { code: 'AFZA', name: 'Ajman Free Zone', emirate: 'Ajman', port: 'AEAJM' },
  { code: 'RAK_FTZ', name: 'RAK Free Trade Zone', emirate: 'Ras Al Khaimah', port: 'AERAK' },
  { code: 'HFZA', name: 'Hamriyah Free Zone', emirate: 'Sharjah', port: 'AESHJ' },
];

// Movement types for VAT treatment (UAE VAT Law Article 51)
const MOVEMENT_TYPES = [
  { value: 'mainland', label: 'Direct to Mainland (5% VAT)', vat_treatment: 'standard' },
  { value: 'dz_entry', label: 'Entry to Designated Zone (0% VAT)', vat_treatment: 'zero_rated' },
  { value: 'dz_to_mainland', label: 'DZ to Mainland (5% VAT on exit)', vat_treatment: 'deferred' },
  { value: 'dz_to_dz', label: 'DZ to DZ Transfer (0% VAT)', vat_treatment: 'zero_rated' },
];
...
const COMMON_PORTS = [
  ...
  { value: 'AEJEA', label: 'Jebel Ali, UAE', country: 'UAE', has_designated_zone: true, zone_code: 'JAFZA' },
  { value: 'AESHJ', label: 'Sharjah, UAE', country: 'UAE', has_designated_zone: true, zone_code: 'SAIF' },
  { value: 'AEAUH', label: 'Abu Dhabi, UAE', country: 'UAE', has_designated_zone: true, zone_code: 'KIZAD' },
  { value: 'AEKLF', label: 'Khor Fakkan, UAE', country: 'UAE', has_designated_zone: false, zone_code: null },
  // China Ports
...
const ImportOrderForm = () => {
  ...
  }, [order.items, order.freight_cost, order.insurance_cost, order.exchange_rate, order.customs_duty_rate, order.vat_rate, order.other_charges, order.movement_type, calculateItemTotal]);
  ...
  const handleSupplierChange = useCallback((supplierId) => {
    ...
    setOrder(prev => ({
      ...
      supplier_vat_status: supplier?.country === 'UAE'
        ? (supplier?.trn_number ? 'uae_registered' : 'non_vat_registered')
  ...
  const validateForm = useCallback(() => {
    ...
    if (order.status !== 'draft') {
      ...
      if ((order.status === 'completed' || order.status === 'customs') && !order.customs_assessment_date) {
        newErrors.customs_assessment_date = 'Customs assessment date required for VAT tax point determination (UAE VAT Law Article 25)';
      }
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              <Input
                ...
                helperText={order.status !== 'draft' ? 'Required for confirmed orders' : 'Tax Registration Number for VAT-registered importers'}
              />
              {order.importer_trn && order.importer_trn.length > 0 && (
                <div className={`text-xs mt-1 ${validateTRN(order.importer_trn).valid ? 'text-green-500' : 'text-amber-500'}`}>
        ...
        <Card title="Supplier & Trade Terms" icon={Building2}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Select
              ...
              onChange={(e) => handleFieldChange('supplier_vat_status', e.target.value)}
            >
            ...
            {(order.supplier_vat_status === 'uae_registered' || order.supplier_vat_status === 'gcc_registered') && (
              <div>
                <Input
                  ...
                  placeholder={order.supplier_vat_status === 'uae_registered' ? '100XXXXXXXXXX (15 digits)' : 'GCC Tax Registration Number'}
                  error={errors.supplier_trn}
                  required={order.status !== 'draft'}
                />
                {order.supplier_trn && order.supplier_vat_status === 'uae_registered' && (
                  <div className={`text-xs mt-1 ${validateTRN(order.supplier_trn).valid ? 'text-green-500' : 'text-amber-500'}`}>
          ...
          <div className={`mt-4 pt-4 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Input
                ...
                required={order.status === 'customs' || order.status === 'completed'}
              />
              <Input
                label="BOE Number"
            ...
            {calculations.isDesignatedZone && (
              <div className={`mt-3 p-3 rounded-lg text-xs ${isDarkMode ? 'bg-green-900/30 border border-green-700' : 'bg-green-50 border border-green-200'}`}>
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className={isDarkMode ? 'text-green-200' : 'text-green-700'}>
                      ...
                      VAT will apply when goods move from the designated zone to mainland UAE.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    VAT ({calculations.effectiveVatRate}%) {calculations.isDesignatedZone ? '(Zero-Rated)' : '- Reverse Charge'}
                  </span>
                ...
                <div className={`mt-3 p-3 rounded-lg text-xs ${isDarkMode ? 'bg-indigo-900/30 border border-indigo-700' : 'bg-indigo-50 border border-indigo-200'}`}>
                  <p className={`font-semibold mb-2 ${isDarkMode ? 'text-indigo-300' : 'text-indigo-800'}`}>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between">
                      ...
                      <span className={`font-medium ${isDarkMode ? 'text-indigo-100' : 'text-indigo-900'}`}>{formatAED(calculations.goodsImportedValue)}</span>
                    </div>
                    {!calculations.isDesignatedZone && (
                      <>
                        <div className="flex justify-between">
                          <span className={isDarkMode ? 'text-indigo-200' : 'text-indigo-700'}>Box 9: Reverse Charge (Output)</span>
                          <span className={`font-medium ${isDarkMode ? 'text-indigo-100' : 'text-indigo-900'}`}>{formatAED(calculations.reverseChargeOutput)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className={isDarkMode ? 'text-indigo-200' : 'text-indigo-700'}>Box 15: Reverse Charge (Input)</span>
                          <span className={`font-medium ${isDarkMode ? 'text-indigo-100' : 'text-indigo-900'}`}>{formatAED(calculations.reverseChargeInput)}</span>
                        </div>
                      </>
                    )}
                    {calculations.isDesignatedZone && (
                      <div className={`text-xs ${isDarkMode ? 'text-indigo-300' : 'text-indigo-600'}`}>
                        Zero-rated entry - No VAT boxes applicable until goods move to mainland
                      </div>
                    )}
                  </div>
                </div>
                ...
                {!calculations.isDesignatedZone && (
                  <div className={`mt-3 p-3 rounded-lg text-xs ${isDarkMode ? 'bg-blue-900/30 border border-blue-700' : 'bg-blue-50 border border-blue-200'}`}>
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <ul className={`space-y-1 ${isDarkMode ? 'text-blue-200' : 'text-blue-700'}`}>
                          <li>• <strong>VAT-Registered:</strong> No VAT payment at customs. Declare in Form 201 Box 9 (Output) & Box 15 (Input).</li>
                          <li>• <strong>Non-Registered:</strong> Pay {formatAED(calculations.vatAmount)} at customs clearance.</li>
                          <li>• <strong>Net Effect:</strong> Zero VAT cash outflow for registered businesses.</li>
                        </ul>
                      </div>
                    </div>
                  </div>
