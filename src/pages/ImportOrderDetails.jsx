...
const ImportOrderDetails = () => {
  ...
  const confirmStatusUpdate = async () => {
    try {
      await importOrderService.updateStatus(
        id,
        pendingStatus,
        statusUpdateNotes,
      );
  ...
  const cifValue =
    order.cifValue ||
    order.cif_value ||
    parseFloat(subtotal) + parseFloat(freightCost) + parseFloat(insuranceCost);
  const customsDuty =
    order.customsDuty || order.customs_duty || cifValue * 0.05;
  const vat = order.vat || cifValue * 0.05;
  const grandTotal =
    order.grandTotal ||
    order.grand_total ||
    order.total ||
    cifValue + customsDuty + vat;
  const grandTotalAED =
    order.grandTotalAed || order.grand_total_aed || grandTotal * exchangeRate;
                    ...
                    {availableTransitions.map((nextStatus) => {
                      ...
                      return (
                        ...
                        >
                          <NextIcon
                            size={18}
                            className={nextConfig.textLight}
                          />
                          {nextConfig.label}
                        </button>
                      );
