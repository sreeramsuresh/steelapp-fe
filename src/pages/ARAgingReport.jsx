...
export default function ARAgingReport() {
  ...
  const [pageInfo, setPageInfo] = useState({ total: 0, page: 1, limit: 50, totalPages: 0 });
  ...
  const fetchARAgingData = useCallback(async () => {
    try {
      ...
      if (response) {
        ...
        setPageInfo(response.pageInfo || { total: 0, page: 1, limit: pageSize, totalPages: 0 });
      }
    } catch (error) {
        ...
        {totals && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
              ...
              <p className={`text-2xl font-bold ${totals.overduePercentage > 30 ? 'text-red-500' : isDarkMode ? 'text-white' : 'text-gray-900'}`}>
          ...
          <table className="w-full">
            <thead className={isDarkMode ? 'bg-gray-750' : 'bg-gray-50'}>
              <tr>
                <th className={`px-4 py-3 text-left text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase tracking-wider`}>
                  <button onClick={() => handleSort('customerCode')} className="flex items-center gap-1 hover:text-blue-500">
                    Code <ArrowUpDown size={12} />
                  </button>
                </th>
                <th className={`px-4 py-3 text-left text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase tracking-wider`}>
                  <button onClick={() => handleSort('customerName')} className="flex items-center gap-1 hover:text-blue-500">
                    Customer <ArrowUpDown size={12} />
                  </button>
                </th>
                <th className={`px-4 py-3 text-right text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase tracking-wider`}>
                  Current
                </th>
                <th className={`px-4 py-3 text-right text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase tracking-wider`}>
                  1-30 Days
                ...
                <th className={`px-4 py-3 text-right text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase tracking-wider`}>
                  <button onClick={() => handleSort('totalAr')} className="flex items-center gap-1 hover:text-blue-500 ml-auto">
                    Total AR <ArrowUpDown size={12} />
                  </button>
                </th>
                <th className={`px-4 py-3 text-center text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase tracking-wider`}>
                  <button onClick={() => handleSort('dsoDays')} className="flex items-center gap-1 hover:text-blue-500 mx-auto">
            ...
            <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
              {loading ? (
                ...
                <tr>
                  <td colSpan={10} className={`px-4 py-8 text-center text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                ...
                sortedData.map((row) => (
                  ...
                  >
                    ...
                    <td className={`px-4 py-3 text-sm text-right ${row.aging1To30 > 0 ? 'text-yellow-600' : isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      {formatCurrency(row.aging1To30)}
                    </td>
                    <td className={`px-4 py-3 text-sm text-right ${row.aging31To60 > 0 ? 'text-orange-600' : isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      {formatCurrency(row.aging31To60)}
                    </td>
                    <td className={`px-4 py-3 text-sm text-right ${row.aging61To90 > 0 ? 'text-red-500' : isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      {formatCurrency(row.aging61To90)}
                    </td>
                    <td className={`px-4 py-3 text-sm text-right ${row.aging90Plus > 0 ? 'text-red-700 font-bold' : isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      {formatCurrency(row.aging90Plus)}
                    </td>
                    <td className={`px-4 py-3 text-sm text-right font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                    ...
                    <td className={`px-4 py-3 text-center`}>
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full border ${getGradeColor(row.creditGrade)}`}>
                        {row.creditGrade || 'N/A'}
                      </span>
                    </td>
                  </tr>
        ...
        {pageInfo.totalPages > 1 && (
          <div className={`px-4 py-3 border-t ${isDarkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50'} flex items-center justify-between`}>
            <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              Showing {((page - 1) * pageSize) + 1} to {Math.min(page * pageSize, pageInfo.total)} of {pageInfo.total} customers
            </div>
            <div className="flex gap-2">
              <button
