...
const DeliveryNoteDetails = () => {
  ...
  const getTotalDeliveredPercentage = () => {
    ...
    const totalOrdered = deliveryNote.items.reduce(
      (sum, item) => sum + item.orderedQuantity,
      0,
    );
    const totalDelivered = deliveryNote.items.reduce(
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <p
                  className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
                >
                  {deliveryNote.deliveryAddress?.street ||
                    deliveryNote.customerDetails?.address?.street}
                  <br />
                  {deliveryNote.deliveryAddress?.city ||
                    deliveryNote.customerDetails?.address?.city}{' '}
                  {deliveryNote.deliveryAddress?.poBox ||
                    deliveryNote.customerDetails?.address?.poBox}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
                >
                  {deliveryNote.items?.filter((item) => item.isFullyDelivered)
                    .length || 0}
                </span>
              </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex justify-between">
                ...
                <span
                  className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
                >
                  {deliveryNote.items?.filter((item) => item.stockDeducted)
                    .length || 0}{' '}
                  / {deliveryNote.items?.length || 0}
                </span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
                >
                  {deliveryNote.items?.filter(
                    (item) =>
                      !item.stockDeducted && item.allocationStatus !== 'failed',
                  ).length || 0}
                </span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={`text-sm font-medium ${
                    deliveryNote.items?.filter(
                      (item) => item.allocationStatus === 'failed',
                    ).length > 0
                      ? isDarkMode
                        ? 'text-red-400'
                        : 'text-red-600'
              ...
              {deliveryNote.items?.some(
                (item) => item.stockDeducted && item.stockDeductedAt,
              ) && (
                <>
                  <hr
                    className={`my-2 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}
                    ...
                    >
                      {formatDate(
                        deliveryNote.items
                          ...
                          .sort(
                            (a, b) =>
                              new Date(b.stockDeductedAt) -
                              new Date(a.stockDeductedAt),
                          )[0]?.stockDeductedAt,
                      )}
                    </p>
                  </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p
                      className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
                    >
                      Remaining quantity: {partialDialog.item.remainingQuantity}{' '}
                      {partialDialog.item.unit}
                    </p>
                  </div>
                  <div>
                    <label
                    ...
                    <p
                      className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
                    >
                      Maximum: {partialDialog.item.remainingQuantity}{' '}
                      {partialDialog.item.unit}
                    </p>
                  </div>
                </div>
