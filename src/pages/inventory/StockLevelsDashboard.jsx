  ...
  return (
    <div className={`p-0 sm:p-4 min-h-[calc(100vh-64px)] overflow-auto ${isDarkMode ? 'bg-[#121418]' : 'bg-[#FAFAFA]'}`}>
      ...
      <div className={`p-0 sm:p-6 mx-0 rounded-none sm:rounded-2xl border overflow-hidden ${
              ...
              <div className="flex flex-wrap gap-4 items-center">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    id="includeZero"
                    checked={includeZero}
                    onChange={(e) => { setIncludeZero(e.target.checked); setPage(1); }}
                    className="w-4 h-4 text-teal-600 rounded focus:ring-teal-500"
                  />
        ...
        {/* Error display */}
        {error && (
          <div className={`mb-4 p-4 rounded-lg border mx-4 sm:mx-0 ${
            ...
            <table className="w-full">
              <thead className={isDarkMode ? 'bg-[#2E3B4E]' : 'bg-gray-50'}>
                <tr>
                  ...
                  <th className={`px-6 py-3 text-center text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    Status
                  </th>
                  <th className={`px-6 py-3 text-right text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    Value
              ...
              <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                {stockLevels.map((item) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-right ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                      {item.quantityOnHand?.toFixed(2)} {item.unit}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-right ${isDarkMode ? 'text-yellow-400' : 'text-yellow-600'}`}>
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-right ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
