...
it('Modal Save button creates item and closes', async () => {
  ...
  await userEvent.type(
    within(modal).getByLabelText('Name'),
    'New Customer'
  );
...
it('Delete button triggers confirmation then deletes', async () => {
  ...
  render(<ItemList items={[{ id: 1, name: 'Item A' }]} onDelete={mockDelete} />);

  const deleteButton = findButtonByRole('Delete', { within: screen.getByText('Item A').closest('tr') });
...
it('Delete button only visible to admins', async () => {
  ...
  rerender(
    <InvoiceActions permissions={{ canDelete: true }} />
  );
