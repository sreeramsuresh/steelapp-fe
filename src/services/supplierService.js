...
export const supplierService = {
  ...
  async getSupplier(id) {
    try { return await apiClient.get(`/suppliers/${id}`); } catch { return ls.all().find(s => s.id === id); }
  },

  async createSupplier(data) {
    try { return await apiClient.post('/suppliers', data); } catch {
  ...
  async updateSupplier(id, data) {
    try { return await apiClient.put(`/suppliers/${id}`, data); } catch {
  ...
  async deleteSupplier(id) {
    try { return await apiClient.delete(`/suppliers/${id}`); } catch { ls.remove(id); return { success: true }; }
  },
};
