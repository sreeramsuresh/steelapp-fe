...
describe("allocationService", () => {
  ...
  describe("Error Handling", () => {
    test("should handle network errors on batch fetch", async () => {
      ...
      await expect(allocationService.getAvailableBatches(1, 1)).rejects.toThrow("Network error");
    });

    test("should handle allocation API errors", async () => {
      apiClient.post.mockRejectedValueOnce(new Error("Invalid product or warehouse"));

      await expect(allocationService.allocateFIFO(999, 999, 100)).rejects.toThrow("Invalid product or warehouse");
    });

    test("should handle server errors (5xx)", async () => {
      apiClient.get.mockRejectedValueOnce(new Error("Internal Server Error"));

      await expect(allocationService.getAvailableBatches(1, 1)).rejects.toThrow("Internal Server Error");
    });

    test("should handle authorization errors", async () => {
      apiClient.post.mockRejectedValueOnce(new Error("Unauthorized"));

      await expect(allocationService.allocateFIFO(1, 1, 50)).rejects.toThrow("Unauthorized");
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    test("should handle response with data property vs direct response", async () => {
      ...
      const mockResponseWithData = {
        data: [{ id: 1, batchNumber: "BATCH-001" }],
      };
