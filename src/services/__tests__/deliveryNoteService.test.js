...
describe("deliveryNoteService", () => {
  ...
  describe("create()", () => {
    ...
    test("should validate required fields on create", async () => {
      ...
      apiClient.post.mockRejectedValueOnce(new Error("Missing required fields"));

      await expect(deliveryNoteService.create(invalidData)).rejects.toThrow("Missing required fields");
    });
  });
  ...
  describe("update()", () => {
    ...
    test("should handle update errors", async () => {
      ...
      await expect(deliveryNoteService.update(1, { driverName: "John" })).rejects.toThrow(
        "Cannot update delivered note"
      );
    });
  });
  ...
  describe("updateDelivery()", () => {
    ...
    test("should prevent exceeding ordered quantity", async () => {
      ...
      apiClient.patch.mockRejectedValueOnce(new Error("Cannot deliver more than ordered quantity"));

      await expect(deliveryNoteService.updateDelivery(1, 10, deliveryData)).rejects.toThrow(
  ...
  describe("updateStatus()", () => {
    test("should update delivery note status", async () => {
      ...
      expect(result.status).toBe("DELIVERED");
      expect(result.stockDeducted).toBe(true);
      expect(apiClient.patch).toHaveBeenCalledWith("/delivery-notes/1/status", {
        status: "DELIVERED",
    ...
    test("should update status with empty notes", async () => {
      ...
      expect(apiClient.patch).toHaveBeenCalledWith("/delivery-notes/1/status", { status: "CANCELLED", notes: "" });
    });

    test("should prevent invalid status transitions", async () => {
      apiClient.patch.mockRejectedValueOnce(new Error("Cannot cancel delivered note"));

      await expect(deliveryNoteService.updateStatus(1, "CANCELLED")).rejects.toThrow("Cannot cancel delivered note");
    });
    ...
    test("should handle status transition to HOLD", async () => {
      ...
      expect(apiClient.patch).toHaveBeenCalledWith("/delivery-notes/1/status", {
        status: "HOLD",
        notes: "On hold pending inspection",
      });
    });
  });
  ...
  describe("delete()", () => {
    ...
    test("should prevent deleting delivered note", async () => {
      apiClient.delete.mockRejectedValueOnce(new Error("Cannot delete delivered note"));

      await expect(deliveryNoteService.delete(1)).rejects.toThrow("Cannot delete delivered note");
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    test("should handle network errors in create", async () => {
      ...
      await expect(deliveryNoteService.create({ invoiceId: 1 })).rejects.toThrow("Network error");
    });

    test("should handle network errors in updateStatus", async () => {
      apiClient.patch.mockRejectedValueOnce(new Error("Server error"));

      await expect(deliveryNoteService.updateStatus(1, "DELIVERED")).rejects.toThrow("Server error");
    });
