...
describe("invoiceService", () => {
  ...
  describe("getInvoices", () => {
    ...
    test("should handle non-paginated response for backward compatibility", async () => {
      const mockInvoices = [{ id: 1, invoiceNumber: "INV-001", total: 52500 }];
  ...
  describe("Search & Filtering", () => {
    ...
    test("should search invoices eligible for credit note", async () => {
      const mockResponse = [{ id: 1, invoiceNumber: "INV-001", customerName: "ABC Corp" }];
      apiClient.get.mockResolvedValueOnce(mockResponse);

      const result = await invoiceService.searchForCreditNote("ABC");

      expect(result).toHaveLength(1);
      expect(apiClient.get).toHaveBeenCalledWith("/invoices/search-for-credit-note", { q: "ABC" });
    });
    ...
    test("should filter invoices by date range", async () => {
      ...
      await invoiceService.getInvoicesByDateRange("2026-01-01", "2026-01-31");
  ...
  describe("Error Handling", () => {
    test("should handle API errors in getInvoices", async () => {
      ...
      await expect(invoiceService.getInvoices()).rejects.toThrow("Network error");
    });

    test("should handle errors in createInvoice", async () => {
      apiClient.post.mockRejectedValueOnce(new Error("Validation failed"));

      await expect(invoiceService.createInvoice({})).rejects.toThrow("Validation failed");
    });

    test("should handle errors in issueInvoice", async () => {
      apiClient.post.mockRejectedValueOnce(new Error("Cannot issue locked invoice"));

      await expect(invoiceService.issueInvoice(1)).rejects.toThrow("Cannot issue locked invoice");
    });
    ...
    test("should handle errors in updateInvoiceStatus", async () => {
      ...
      await expect(invoiceService.updateInvoiceStatus(1, "invalid")).rejects.toThrow("Invalid status transition");
    });
  });
});
