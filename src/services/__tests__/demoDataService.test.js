...
describe("demoDataService", () => {
  ...
  describe("initializeDemoProducts", () => {
    ...
    it("should show success notification when at least one product created", async () => {
      productService.createProduct.mockResolvedValueOnce({ id: 1 }).mockRejectedValue(new Error("Failed"));

      await demoDataService.initializeDemoProducts();

      expect(notificationService.success).toHaveBeenCalledWith(expect.stringContaining("Initialized"));
    });
