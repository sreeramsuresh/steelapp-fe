...
export const dashboardService = {
    ...
    try {
      ...
      const totalCustomers = parseInt(
        dashboardData?.metrics?.totalCustomers ||
          ...
          (Array.isArray(customerStats?.customers)
            ? customerStats.customers.length
            : 0) ||
          0,
      );
      const totalProducts = parseInt(
        dashboardData?.metrics?.totalProducts ||
          ...
          (Array.isArray(productStats?.products)
            ? productStats.products.length
            : 0) ||
          0,
      );
  ...
  async getProductAnalytics(options = {}) {
    ...
    try {
      ...
      const [productPerformance, productList, inventorySummary] =
        await Promise.all([
      ...
      const analytics = {
        ...
        slowMoving,
        summary: {
          totalProductsSold: topProducts.reduce(
          ...
          avgMargin:
            topProducts.length > 0
              ? topProducts.reduce((sum, p) => sum + p.margin, 0) /
                topProducts.length
              : 0,
        },
        isMockData: false,
  ...
  async getAgentPerformance(options = {}) {
    ...
    try {
      ...
      const [commissionDashboard, commissionAgents, commissionTransactions] =
        await Promise.all([
      ...
      const performance = {
        ...
        summary: {
          ...
          activeAgents:
            parseInt(
              dashboardMetrics.activeAgents || dashboardMetrics.active_agents,
            ) || enrichedAgents.filter((a) => a.isActive).length,
          totalTeamRevenue: safeNum(
            dashboardMetrics.totalCommissions ||
              dashboardMetrics.total_commissions,
          ),
          totalCommissionPaid: safeNum(
            dashboardMetrics.paidThisPeriod ||
              dashboardMetrics.paid_this_period,
          ),
          totalCommissionPending: safeNum(
            dashboardMetrics.pendingPayout || dashboardMetrics.pending_payout,
          ),
          avgConversionRate: 0, // Would need more data
        },
        topAgents: (
          dashboardMetrics.topAgents ||
          dashboardMetrics.top_agents ||
          []
        ).map((ta) => ({
        ...
        trendData: (
          dashboardMetrics.trendData ||
          dashboardMetrics.trend_data ||
          []
        ).map((td) => ({
          period: td.period,
  ...
  async getInventoryHealth(options = {}) {
    ...
    try {
      ...
      const [inventorySummary, lowStockItems, inventoryInsights] =
        await Promise.all([
          inventoryService.getInventorySummary().catch((err) => {
            console.warn(
              '[dashboardService] getInventorySummary failed:',
          ...
          totalItems:
            parseInt(
              inventorySummary?.totalItems ||
                inventoryInsights?.overview?.total_products,
            ) || 0,
          totalValue: safeNum(
            inventorySummary?.totalValue ||
              inventoryInsights?.overview?.total_value,
          ),
          ...
          outOfStockCount:
            parseInt(
              inventorySummary?.outOfStockCount ||
                inventoryInsights?.overview?.out_of_stock_count,
            ) || 0,
        },
        lowStockItems: Array.isArray(lowStockItems)
          ? lowStockItems.slice(0, 10)
          : (inventoryInsights?.low_stock_items || []).slice(0, 10),
        turnoverRate: safeNum(
          inventoryInsights?.overview?.turnover_rate ||
            inventorySummary?.turnoverRate,
        ),
  ...
  async getVATMetrics(options = {}) {
    ...
    try {
      ...
      const vatMetrics = await vatService
      ...
      const vatData = {
        ...
        compliance: {
          ...
          effectiveVATRate:
            totalSales > 0
              ? ((totalVatCollected / totalSales) * 100).toFixed(2)
              : 0,
        },
        alerts: [],
        history: [],
  ...
  async getCustomerInsights(options = {}) {
    ...
    try {
      ...
      const [customerAnalysis, customerList, recentInvoices] =
        await Promise.all([
      ...
      invoices.forEach((inv) => {
        const customerId =
          inv.customerId || inv.customer_id || inv.customer?.id;
        if (customerId) {
          ...
          const invDate = new Date(
            inv.invoiceDate || inv.invoice_date || inv.date || inv.createdAt,
          );
          if (
            !customerInvoiceMap[customerId].lastOrderDate ||
      ...
      const enrichedCustomers = customers.map((customer) => {
        ...
        const avgOrderValue =
          invoiceCount > 0 ? history.totalRevenue / invoiceCount : 0;
        ...
        if (history.lastOrderDate) {
          daysInactive = Math.floor(
            (now - history.lastOrderDate) / (1000 * 60 * 60 * 24),
          );
        } else {
          daysInactive = 90;
        }
        ...
        if (history.totalOutstanding > 0) {
          const outstandingRatio =
            history.totalOutstanding / (history.totalRevenue || 1);
        ...
        return {
          ...
          creditUsed: safeNum(
            customer.currentCredit || customer.current_credit,
          ),
        };
      });
      ...
      const inactiveCount = enrichedCustomers.filter(
        (c) => c.daysInactive > 60,
      ).length;
      const churnRate =
      ...
      const result = {
        ...
        avgCLV: Math.round(
          enrichedCustomers.reduce((sum, c) => sum + c.clv, 0) /
            (enrichedCustomers.length || 1),
        ),
        totalOutstanding: Math.round(
          enrichedCustomers.reduce((sum, c) => sum + c.outstanding, 0),
        ),
  ...
  async getNetProfit(options = {}) {
    ...
    try {
      ...
      const response = await analyticsService.getNetProfit(
        options.params || {},
      );
      ...
      const netProfitData = {
        ...
        operatingExpenses: safeNum(
          response?.operating_expenses || response?.operatingExpenses || 0,
        ),
        netProfit: safeNum(response?.net_profit || response?.netProfit),
        grossMarginPercent: safeNum(
          response?.gross_margin_percent || response?.grossMarginPercent,
        ),
        netMarginPercent: safeNum(
          response?.margin_percentage ||
            response?.marginPercentage ||
            response?.net_margin_percent,
        ),
      ...
      if (
        !netProfitData.grossProfit &&
        netProfitData.revenue &&
        netProfitData.cogs
      ) {
      ...
      if (!netProfitData.grossMarginPercent && netProfitData.revenue > 0) {
        netProfitData.grossMarginPercent =
          (netProfitData.grossProfit / netProfitData.revenue) * 100;
      }
      if (!netProfitData.netMarginPercent && netProfitData.revenue > 0) {
        netProfitData.netMarginPercent =
          (netProfitData.netProfit / netProfitData.revenue) * 100;
      }
      ...
      const apAgingData = {
        buckets: (response?.buckets || []).map((bucket) => ({
          label:
            bucket.label ||
            bucket.range ||
            `${bucket.min_days || 0}-${bucket.max_days || 30} Days`,
        ...
        criticalSuppliers: (
          response?.critical_suppliers ||
          response?.criticalSuppliers ||
          []
        ).map((supplier) => ({
          name: supplier.name || supplier.supplier_name,
          amount: safeNum(supplier.amount || supplier.total_overdue),
          daysOverdue:
            parseInt(supplier.days_overdue || supplier.daysOverdue) || 0,
        })),
        isMockData: false,
        fetchedAt: new Date().toISOString(),
      };
  ...
  async getCashFlow(options = {}) {
    ...
    try {
      ...
      const transformPeriodData = (data) => ({
        ...
        outflows: safeNum(
          data?.outflow || data?.outflows || data?.total_outflow,
        ),
        netCashFlow: safeNum(
          data?.net_cash_flow || data?.netCashFlow || data?.net,
        ),
  ...
  async getStockTurnover(options = {}) {
    ...
    try {
      ...
      const response = await analyticsService.getStockTurnover(
        options.params || {},
      );
      ...
      const stockTurnoverData = {
        products: (
          ...
          data:
            ...
            item.monthlyData ||
            Array(6).fill(
              safeNum(item.turnover_ratio || item.turnoverRatio || 1),
            ),
        })),
        months,
        overallEfficiency:
          safeNum(
            response?.overall_efficiency || response?.overallEfficiency,
          ) ||
          Math.min(
            100,
            Math.round(
              safeNum(
                response?.turnover_ratio || response?.turnoverRatio || 2,
              ) * 25,
            ),
          ),
        avgTurnover: safeNum(
          response?.turnover_ratio ||
            response?.turnoverRatio ||
            response?.avg_turnover,
        ),
        daysOfInventory:
          parseInt(response?.days_of_inventory || response?.daysOfInventory) ||
          0,
      ...
      if (
        !stockTurnoverData.bestPerformer &&
        stockTurnoverData.products.length > 0
      ) {
      ...
      const warehouseData = {
        ...
        summary: {
          totalCapacity: warehouses.reduce(
            (sum, w) => sum + safeNum(w.capacity),
            0,
          ),
          totalUsed: warehouses.reduce(
            (sum, w) => sum + safeNum(w.inventoryCount || 0),
            0,
          ),
          avgUtilization:
            warehouses.length > 0
