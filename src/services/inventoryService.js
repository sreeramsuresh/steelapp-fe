...
const toServer = (item = {}) => ({
  ...
  // ERP fields
  quantity_on_hand:
    typeof item.quantityOnHand === "number" ? item.quantityOnHand : parseFloat(item.quantityOnHand) || 0,
  quantity_reserved:
    typeof item.quantityReserved === "number" ? item.quantityReserved : parseFloat(item.quantityReserved) || 0,
