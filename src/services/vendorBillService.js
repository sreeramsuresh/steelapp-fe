...
const vendorBillService = {
  ...
  async approve(id, notes = "") {
    try {
      const response = await apiClient.post(`/vendor-bills/${id}/approve`, {
        notes,
      });
      return transformVendorBillFromServer(response);
    } catch (error) {
  ...
  async reject(id, reason = "") {
    try {
      const response = await apiClient.post(`/vendor-bills/${id}/reject`, {
        reason,
      });
      return transformVendorBillFromServer(response);
    } catch (error) {
  ...
  async search(searchTerm, filters = {}) {
    try {
      ...
      return Array.isArray(bills)
        ? bills.map(transformVendorBillFromServer)
        : [];
    } catch (error) {
