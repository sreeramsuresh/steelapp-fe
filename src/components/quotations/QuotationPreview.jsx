...
const QuotationPreview = ({ quotation, company, onClose }) => {
  ...
  const customerDetails = quotation.customerDetails || quotation.customer_details || {};
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-teal-100 mt-1">
                    {quotation.quotationNumber || quotation.quotation_number || 'QTN-DRAFT'}
                  </p>
                </div>
            ...
            <div className={`p-6 ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="font-medium">{customerDetails.name || 'Customer Name'}</p>
                  ...
                  {customerDetails.vatNumber && (
                    <p className="text-sm mt-1">VAT: {customerDetails.vatNumber}</p>
                  )}
                </div>
                <div className="text-right">
                  <div className="mb-2">
                  ...
                  <div className="mb-2">
                    <span className={`inline-block px-2 py-1 text-xs font-medium rounded ${getStatusColor(quotation.status)}`}>
                  ...
                  {(quotation.warehouseName || quotation.warehouse_name) && (
                    <div>
                      ...
                      <span className="font-medium">{quotation.warehouseName || quotation.warehouse_name}</span>
                    </div>
                  )}
                </div>
              </div>
                      ...
                      {items.length > 0 ? (
                        items.map((item, index) => (
                          <tr key={index} className={`border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                            ...
                            <td className="px-4 py-3 text-right">{item.quantity || 0} {item.unit || 'pcs'}</td>
                            ...
                            <td className="px-4 py-3 text-right font-medium">{formatCurrency(item.netAmount || item.net_amount || item.amount || 0)}</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
              ...
              <div className={`border rounded-lg p-4 ${isDarkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50'}`}>
                ...
                {(quotation.packingCharges || quotation.freightCharges || quotation.otherCharges) && (
              ...
              {/* Terms & Notes */}
              {(quotation.notes || quotation.termsAndConditions || quotation.terms_and_conditions) && (
                <div className={`mt-6 pt-4 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                  {quotation.notes && (
                    <div className="mb-4">
                      <h4 className={`text-sm font-semibold ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mb-1`}>Notes:</h4>
                      <p className="text-sm whitespace-pre-wrap">{quotation.notes}</p>
                    </div>
                  )}
                  {(quotation.termsAndConditions || quotation.terms_and_conditions) && (
                    <div>
                      <h4 className={`text-sm font-semibold ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mb-1`}>Terms & Conditions:</h4>
                      <p className="text-sm whitespace-pre-wrap">{quotation.termsAndConditions || quotation.terms_and_conditions}</p>
                    </div>
                  )}
                </div>
              )}
