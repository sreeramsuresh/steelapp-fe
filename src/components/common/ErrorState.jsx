import { AlertTriangle, FileQuestion, Home, LogOut, RefreshCw, ServerOff, ShieldOff, WifiOff } from "lucide-react";
...
const ErrorState = ({
  ...
  const getColorScheme = () => {
    if (status >= 500 || !status) {
      return {
        ...
        buttonBg: isDarkMode ? "bg-red-600 hover:bg-red-500" : "bg-red-600 hover:bg-red-700",
      };
    }
    if (status === 403 || status === 401) {
      return {
        ...
        buttonBg: isDarkMode ? "bg-amber-600 hover:bg-amber-500" : "bg-amber-600 hover:bg-amber-700",
      };
    }
    if (status === 404) {
      return {
        ...
        buttonBg: isDarkMode ? "bg-gray-600 hover:bg-gray-500" : "bg-gray-600 hover:bg-gray-700",
      };
    }
    // Default fallback
    return {
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-[400px] p-8 ${colors.bg} rounded-lg ${className}`}>
      {/* Icon */}
      ...
      {/* Title */}
      <h2 className={`text-xl font-semibold mb-2 ${colors.titleColor}`}>{title}</h2>

      {/* Message */}
      <p className={`text-center max-w-md mb-6 ${colors.textColor}`}>{message}</p>

      {/* Status code (if available) */}
      {status && (
        <p className={`text-sm mb-4 ${isDarkMode ? "text-gray-500" : "text-gray-400"}`}>Error Code: {status}</p>
      )}
...
ErrorState.propTypes = {
  ...
  icon: PropTypes.oneOf(["WifiOff", "ServerOff", "AlertTriangle", "ShieldOff", "FileQuestion", "LogOut"]),
