...
import { SETTINGS_NAV_ITEMS } from "../config/settingsNavItems";
import { useTheme } from "../contexts/ThemeContext";
import { authService } from "../services/axiosAuthService";

const SettingsSidebar = ({ isOpen, onToggle }) => {
  ...
  const isActiveRoute = (path) => {
    ...
    if (path !== "/app/settings" && location.pathname.startsWith(path)) return true;
    return false;
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <div>
            <div className="font-bold text-lg leading-tight">SETTINGS</div>
      ...
      <div className="flex-1 relative min-h-0">
        ...
        {/* Scrollable content */}
        <nav ref={scrollContainerRef} className="absolute inset-0 overflow-y-auto py-2 no-scrollbar">
          {SETTINGS_NAV_ITEMS.map((section, sectionIndex) => {
              ...
              <div key={section.section || `section-${sectionIndex}`}>
                ...
                <div className="space-y-1">
                  {visibleItems.map((item, itemIndex) => {
                    ...
                    return (
                      <div key={item.name || `item-${itemIndex}`} className="px-2">
                        <Link
                          to={item.path}
