    ...
    <Box>
      ...
      <Paper sx={{ mb: 2 }}>
        <Tabs value={activeTab} onChange={(e, v) => setActiveTab(v)}>
          <Tab icon={<ReportIcon />} label="Reconciliation Report" iconPosition="start" />
      ...
      <TabPanel value={activeTab} index={0}>
        ...
        {/* Error Alert */}
        {reconciliationError && (
          <Alert severity="error" sx={{ mb: 2 }} onClose={() => setReconciliationError(null)}>
            {reconciliationError}
          </Alert>
        )}
              ...
              <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="h6">{reconciliationData.warehouseName}</Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="h6">{reconciliationData.items?.length || 0}</Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="h6">{formatQuantity(reconciliationData.totalSystemValue)}</Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                <Card sx={{ bgcolor: reconciliationData.discrepancyCount > 0 ? 'warning.light' : 'success.light' }}>
                  <CardContent>
                    <Typography variant="subtitle2" color="text.secondary">
                      Discrepancies
                    </Typography>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      ...
                      <Typography variant="h6">{reconciliationData.discrepancyCount}</Typography>
                    </Box>
                  </CardContent>
                </Card>
              </Grid>
                ...
                <TableBody>
                  {(reconciliationData.items || []).length === 0 ? (
                    ...
                    reconciliationData.items.map((item, idx) => {
                      ...
                      return (
                        <TableRow key={idx} hover sx={{ bgcolor: hasDiscrepancy ? 'warning.light' : 'inherit' }}>
                          <TableCell>{item.productName}</TableCell>
                          <TableCell>{item.productSku || '-'}</TableCell>
                          <TableCell align="right">{formatQuantity(item.systemQuantity)}</TableCell>
                          <TableCell align="right">{formatQuantity(item.lastPhysicalCount)}</TableCell>
                          ...
                          <TableCell>{formatDate(item.lastCountDate)}</TableCell>
                          <TableCell>
                            <Chip
                              size="small"
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
            <FormControl size="small" sx={{ minWidth: 200 }}>
              ...
              <Select
                value={selectedWarehouseId}
                label="Warehouse"
                onChange={(e) => { setSelectedWarehouseId(e.target.value); setAuditPage(0); }}
              >
                    ...
                    <TableRow key={entry.id} hover>
                      ...
                      <TableCell align="right">
                        <Typography
                          ...
                          fontWeight="medium"
                        >
                          {isIncrease ? '+' : '-'}{formatQuantity(Math.abs(change))}
                        </Typography>
                      </TableCell>
                      <TableCell align="right">{formatQuantity(entry.balanceBefore)}</TableCell>
                      <TableCell align="right">{formatQuantity(entry.balanceAfter)}</TableCell>
                      <TableCell>
