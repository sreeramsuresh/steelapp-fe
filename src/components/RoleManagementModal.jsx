...
import { X, Lock, Plus, Edit2, Trash2, Shield, AlertCircle } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      ...
      >
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          {/* Create/Edit Form */}
          {showCreateForm ? (
            <div className={`mb-6 p-4 rounded-lg border ${isDarkMode ? 'border-gray-700 bg-gray-750' : 'border-gray-200 bg-gray-50'}`}>
              <h3 className="text-lg font-medium mb-4">
                {editingRole ? 'Edit Role' : 'Create New Role'}
              </h3>

              {editingRole?.isSystemRole && (
                <div className={`mb-4 p-3 rounded-lg flex items-start gap-2 ${isDarkMode ? 'bg-blue-900/20 text-blue-300' : 'bg-blue-50 text-blue-800'}`}>
                  <AlertCircle size={20} className="flex-shrink-0 mt-0.5" />
                  <div className="text-sm">
                    <strong>System Role:</strong> You can edit the description and director status, but cannot change the role name.
                  </div>
                </div>
              )}
              ...
              <div className="space-y-4">
                {/* Display Name */}
                <div>
                  <label htmlFor="displayName" className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    Display Name *
                  </label>
                  <input
                    id="displayName"
                    ...
                    className={`w-full px-3 py-2 border rounded-lg transition-colors ${
                      validationErrors.displayName ? 'border-red-500' : isDarkMode ? 'border-gray-600' : 'border-gray-300'
                    } ${
                      isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'
              ...
              {roles.map((role) => (
                <div
                  ...
                  className={`p-4 rounded-lg border transition-all ${
                    isDarkMode ? 'border-gray-700 bg-gray-750 hover:bg-gray-700' : 'border-gray-200 bg-white hover:bg-gray-50'
                  }`}
                >
