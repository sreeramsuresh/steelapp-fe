...
const SearchResults = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const invoices = Array.isArray(inv?.invoices)
          ? inv.invoices
          : Array.isArray(inv)
            ? inv
            : [];
        const customers = Array.isArray(cust?.customers)
          ? cust.customers
          : Array.isArray(cust)
            ? cust
            : [];
  ...
  return (
    <div
      className={`p-4 md:p-6 min-h-[calc(100vh-64px)] ${isDarkMode ? "bg-[#121418]" : "bg-[#FAFAFA]"}`}
    >
      <div
      ...
      >
        <div className="mb-4 md:mb-6 flex items-center gap-3">
          ...
          <h1
            className={`text-xl md:text-2xl font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}
          >
            Search results
          </h1>
          {q && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            ...
            >
              <SectionHeader
                ...
                toAll={
                  q ? `/invoices?search=${encodeURIComponent(q)}` : undefined
                }
            ...
            >
              <SectionHeader
                ...
                toAll={
                  q ? `/customers?search=${encodeURIComponent(q)}` : undefined
                }
                isDarkMode={isDarkMode}
              />
