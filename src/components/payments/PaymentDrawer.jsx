...
const PaymentDrawer = ({
  ...
  const getPaymentStatusColor = () => {
    switch (computedStatus) {
      ...
      default:
        return isDarkMode ? "bg-red-900/30 text-red-400 border-red-700" : "bg-red-100 text-red-800 border-red-300";
    }
  };
    ...
    <div className="fixed inset-0 z-[1100] flex" data-testid="payment-drawer">
      ...
      >
        ...
        <div className="p-4 border-b flex items-center justify-between">
          <div>
            <div className="font-semibold text-lg">{invoice.invoiceNo || invoice.invoiceNumber}</div>
            <div className="text-sm opacity-70">{invoice.customer?.name || ""}</div>
          </div>
        ...
        {otherSessions && otherSessions.length > 0 && (
          ...
          >
            <span>⚠️</span>
            <span>
              Currently being edited by <strong>{[...new Set(otherSessions.map((s) => s.userName))].join(", ")}</strong>
        ...
        <div className="p-4 space-y-4">
                  ...
                  {[...(invoice.payments || [])]
                    ...
                    .map((p, idx) => {
                      ...
                      return (
                        <div
                          key={p.id || idx}
                          className={`${isDarkMode ? "bg-gray-900/50" : "bg-white"} ${isVoided ? "opacity-70" : ""}`}
                        >
                          {/* Main Payment Row */}
                          <div className="grid grid-cols-12 gap-2 px-3 py-3 items-center text-sm">
                            ...
                            >
                              ...
                              <span className="truncate">{paymentMode?.label || methodValue}</span>
                            </div>
                                ...
                                <>
                                  ...
                                  {onVoidPayment && isDropdownOpen && (
                                    <div
                                      ...
                                      onKeyDown={(e) => {
                                        if (e.key === "Escape") onVoidDropdownToggle(null);
                                      }}
                                      role="menu"
                                      tabIndex={-1}
                                    >
                                      ...
                                      >
                                        <input
                                          ...
                                          onChange={(e) => onVoidCustomReasonChange(e.target.value)}
                          ...
                          {(p.notes || p.receiptNumber) && (
                            <div className={`px-3 pb-2 -mt-1 ${isDarkMode ? "text-gray-500" : "text-gray-500"}`}>
                              {p.receiptNumber && (
                                <div
                                  className={`text-xs font-semibold ${isDarkMode ? "text-teal-400" : "text-teal-600"}`}
                                >
          ...
          {/* Add Payment Section */}
          {outstanding > 0 ? (
            <AddPaymentForm outstanding={outstanding} onSave={onAddPayment} onCancel={onClose} isSaving={isSaving} />
          ) : invoiceAmount === 0 ? (
            <div
            ...
            >
              <CheckCircle size={18} />
              <span className="font-medium">No Payment Required (Zero Invoice)</span>
            </div>
          ) : (
            <>
              {!canManage && (
