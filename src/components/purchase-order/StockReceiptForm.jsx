...
const StockReceiptForm = ({
  ...
  useEffect(() => {
    if (poItems && poItems.length > 0) {
      ...
      poItems.forEach((item) => {
        const pending =
          parseFloat(item.pendingQuantity) || parseFloat(item.quantity) - parseFloat(item.receivedQuantity || 0);
  ...
  const allSelected =
    ...
    receivableItems
      .filter((i) => {
        const pending = parseFloat(i.pendingQuantity) || parseFloat(i.quantity) - parseFloat(i.receivedQuantity || 0);
        return pending > 0;
      })
        ...
        >
            ...
            <div className={`${cardBg} border ${cardBorder} rounded-2xl p-4`}>
              ...
              {grnStatus !== "approved" && (
                <div className="flex items-center gap-2">
                  <button type="button" onClick={handleApproveGRN}
              ...
              <div className="flex items-start gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
                ...
                <div>
                  ...
                  <p className={`text-xs ${textMuted} mt-1`}>
                    No items with linked products found. Stock movements can only be created for items that are linked
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className={tableHeaderBg}>
                          ...
                          <th className={`p-3 border-b ${tableBorder} text-right ${textMuted} font-medium`}>Ordered</th>
                          <th className={`p-3 border-b ${tableBorder} text-right ${textMuted} font-medium`}>
                            Received
                          </th>
                          <th className={`p-3 border-b ${tableBorder} text-right ${textMuted} font-medium`}>Pending</th>
                          <th className={`p-3 border-b ${tableBorder} text-center ${textMuted} font-medium`}>Status</th>
                          <th
                              ...
                              <td className={`p-3 border-b ${tableBorder}`}>
                                {!isComplete ? (
                                  <div className="flex items-center gap-2 justify-end">
                                    ...
                                    />
                                    <button type="button" onClick={() => handleSetMaxQuantity(item.id)}
                                      disabled={!isSelected}
                              ...
                              <td className={`p-3 border-b ${tableBorder}`}>
                                {!isComplete && isSelected ? (
                                  ...
                                  >
                                    ...
                                    <option value="damage_weight_loss">Damage/Weight Loss</option>
                                    <option value="measurement_difference">Measurement Difference</option>
                                  </select>
                                ) : (
                                  <span className={`text-center block ${textMuted}`}>-</span>
                                )}
          ...
          <div className={`flex items-center justify-between gap-3 p-4 border-t ${modalBorder}`}>
            <div className={`text-xs ${textMuted}`}>
              {hasSelectedItems ? (
                <span>{Object.values(selectedItems).filter(Boolean).length} item(s) selected</span>
              ) : (
