...
const POStockMovements = ({ purchaseOrderId, poNumber: _poNumber, defaultExpanded = true }) => {
  ...
  const fetchMovements = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const result = await stockMovementService.getByPurchaseOrder(purchaseOrderId);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="p-1 hover:bg-gray-100 rounded transition-colors">
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm mt-1 opacity-75">
                  Stock movements will be created automatically when items are received.
                </p>
              </div>
            </div>
          ) : (
            <>
                    ...
                    {movements.map((movement) => {
                      ...
                      return (
                        <TableRow key={movement.id} className="hover:bg-gray-50">
                          <TableCell>
                            <span className="text-xs text-gray-600">{formatDate(movement.movementDate)}</span>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <span className="text-sm">{movement.warehouseName || movement.warehouseCode || "-"}</span>
                          </TableCell>
                          <TableCell>
                            <span
              ...
              <div className="flex justify-end gap-6 mt-4 pt-4 border-t border-gray-200">
                <div className="text-right">
                  ...
                  <p className="text-lg font-semibold text-green-600">+{formatQuantity(totals.totalIn, "KG")}</p>
                </div>
              </div>
            </>
          )}
