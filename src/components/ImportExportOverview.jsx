  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const [importData, exportData, shippingData, certData] =
          await Promise.all([
            importOrderService
              .getImportOrders({ limit: 100 })
              .catch(() => ({ orders: [], pagination: {} })),
            exportOrderService
              .getExportOrders({ limit: 100 })
              .catch(() => ({ orders: [], pagination: {} })),
            shippingDocumentService
              .getShippingDocuments({ limit: 100 })
              .catch(() => ({ documents: [], pagination: {} })),
