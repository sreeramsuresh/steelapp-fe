...
describe("BatchAllocator", () => {
  ...
  describe("FIFO Allocation Logic", () => {
    ...
    it("should allocate across multiple batches if needed", () => {
      const { container } = renderWithProviders(
        <BatchAllocator {...defaultProps} quantity={200} />,
      );
      expect(container).toBeInTheDocument();
    });
    ...
    it("should handle exact quantity match", () => {
      const { container } = renderWithProviders(
        <BatchAllocator {...defaultProps} quantity={150} />,
      );
      expect(container).toBeInTheDocument();
    });
    ...
    it("should prevent over-allocation", () => {
      const { container } = renderWithProviders(
        <BatchAllocator {...defaultProps} quantity={500} />,
      );
      expect(container).toBeInTheDocument();
    });
  });
  ...
  describe("Batch Selection", () => {
    ...
    it("should disable allocation for unavailable batches", () => {
      const unavailableBatches = [
        { ...mockBatches[0], availableQuantity: 0 },
      ];
  ...
  describe("Expiry Date Handling", () => {
    ...
    it("should sort by expiry date when enabled", () => {
      const { container } = renderWithProviders(
        <BatchAllocator {...defaultProps} sortByExpiry={true} />,
      );
      expect(container).toBeInTheDocument();
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle batch loading errors", () => {
      mockStockBatchService.listBatches.mockRejectedValue(
        new Error("API Error"),
      );
      const { container } = renderWithProviders(
        <BatchAllocator {...defaultProps} />,
    ...
    it("should display insufficient stock warning", () => {
      ...
      const { container } = renderWithProviders(
        <BatchAllocator {...defaultProps} quantity={100} />,
      );
      expect(container).toBeInTheDocument();
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle zero quantity", () => {
      const { container } = renderWithProviders(
        <BatchAllocator {...defaultProps} quantity={0} />,
      );
      expect(container).toBeInTheDocument();
    });
    ...
    it("should handle very large quantities", () => {
      const { container } = renderWithProviders(
        <BatchAllocator {...defaultProps} quantity={999999} />,
      );
      expect(container).toBeInTheDocument();
    });
  });
