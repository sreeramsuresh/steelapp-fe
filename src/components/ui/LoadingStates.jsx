 ...
 * Card skeleton
 */
export const CardSkeleton = ({ showImage = false, lines = 3, isDarkMode = false, className = '' }) => (
  <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} ${className}`}>
    {showImage && <Skeleton width="full" height="32" rounded="lg" isDarkMode={isDarkMode} className="mb-4" />}
...
export const ProgressBar = ({
  progress = 0, showPercentage = true, size = 'md', color = 'teal', isDarkMode = false, className = '',
}) => {
  const heights = { sm: 'h-1', md: 'h-2', lg: 'h-3' };
  const colors = { teal: 'bg-teal-500', blue: 'bg-blue-500', green: 'bg-green-500' };
  ...
  return (
    <div className={className}>
      ...
      {showPercentage && (
        <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{Math.round(progress)}%</p>
      )}
    </div>
  );
};
 ...
 * Saving indicator
 */
export const SavingIndicator = ({ status = 'idle', lastSaved = null, isDarkMode = false, className = '' }) => {
  const configs = {
    idle: { icon: null, text: lastSaved ? `Last saved ${lastSaved}` : 'Not saved', color: isDarkMode ? 'text-gray-500' : 'text-gray-400' },
    saving: { icon: <Spinner size="xs" color={isDarkMode ? 'white' : 'gray'} />, text: 'Saving...', color: isDarkMode ? 'text-gray-400' : 'text-gray-500' },
    saved: {
      ...
      text: 'Saved', color: 'text-green-500',
    },
    error: {
      icon: <svg className="w-3 h-3 text-red-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>,
      text: 'Error saving', color: 'text-red-500',
    },
  };
...
export default { Spinner, PulsingDots, Skeleton, TableRowSkeleton, CardSkeleton, PageLoader, InlineLoader, ProgressBar, SavingIndicator };
