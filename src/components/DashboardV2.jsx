...
// Lazy load widgets
const LazyRevenueKPIWidget = lazy(
  () => import('./dashboard/widgets/financial/RevenueKPIWidget'),
);
const LazyARAgingWidget = lazy(
  () => import('./dashboard/widgets/financial/ARAgingWidget'),
);
const LazyGrossMarginWidget = lazy(
  () => import('./dashboard/widgets/financial/GrossMarginWidget'),
);
const LazyCashFlowWidget = lazy(
  () => import('./dashboard/widgets/financial/CashFlowWidget'),
);
const LazyDSOWidget = lazy(
  () => import('./dashboard/widgets/financial/DSOWidget'),
);
const LazyCreditUtilizationWidget = lazy(
  () => import('./dashboard/widgets/financial/CreditUtilizationWidget'),
);
const LazyProfitSummaryWidget = lazy(
  () => import('./dashboard/widgets/financial/ProfitSummaryWidget'),
);
const LazyAPAgingWidget = lazy(
  () => import('./dashboard/widgets/financial/APAgingWidget'),
);
const LazyInventoryHealthWidget = lazy(
  () => import('./dashboard/widgets/inventory/InventoryHealthWidget'),
);
const LazyFastMovingWidget = lazy(
  () => import('./dashboard/widgets/inventory/FastMovingWidget'),
);
const LazySlowMovingWidget = lazy(
  () => import('./dashboard/widgets/inventory/SlowMovingWidget'),
);
const LazyReorderAlertsWidget = lazy(
  () => import('./dashboard/widgets/inventory/ReorderAlertsWidget'),
);
const LazyStockTurnoverWidget = lazy(
  () => import('./dashboard/widgets/inventory/StockTurnoverWidget'),
);
const LazyWarehouseUtilizationWidget = lazy(
  () => import('./dashboard/widgets/inventory/WarehouseUtilizationWidget'),
);
const LazyTopProductsWidget = lazy(
  () => import('./dashboard/widgets/product/TopProductsWidget'),
);
const LazyCategoryPerformanceWidget = lazy(
  () => import('./dashboard/widgets/product/CategoryPerformanceWidget'),
);
const LazyGradeAnalysisWidget = lazy(
  () => import('./dashboard/widgets/product/GradeAnalysisWidget'),
);
const LazyVATCollectionWidget = lazy(
  () => import('./dashboard/widgets/vat/VATCollectionWidget'),
);
const LazyVATReturnStatusWidget = lazy(
  () => import('./dashboard/widgets/vat/VATReturnStatusWidget'),
);
const LazyLeaderboardWidget = lazy(
  () => import('./dashboard/widgets/sales-agent/LeaderboardWidget'),
);
const LazyAgentScorecardWidget = lazy(
  () => import('./dashboard/widgets/sales-agent/AgentScorecardWidget'),
);
const LazyConversionFunnelWidget = lazy(
  () => import('./dashboard/widgets/sales-agent/ConversionFunnelWidget'),
);
const LazyCustomerPortfolioWidget = lazy(
  () => import('./dashboard/widgets/sales-agent/CustomerPortfolioWidget'),
);
const LazyCollectionPerformanceWidget = lazy(
  () => import('./dashboard/widgets/sales-agent/CollectionPerformanceWidget'),
);
const LazyCustomerSegmentsWidget = lazy(
  () => import('./dashboard/widgets/customer/CustomerSegmentsWidget'),
);
const LazyNewCustomerWidget = lazy(
  () => import('./dashboard/widgets/customer/NewCustomerWidget'),
);
...
const ChangeIndicator = ({ positive, children }) => (
  <div
    className={`flex items-center gap-1 text-sm font-medium mt-1 ${positive ? 'text-green-500' : 'text-red-500'}`}
  >
    {children}
  </div>
);
...
const DashboardV2 = () => {
  ...
  const fetchWidgetData = useCallback(async () => {
    try {
      ...
      const inventoryHealth = await dashboardService
        .getInventoryHealth()
        .catch(() => null);
      if (inventoryHealth) {
        setWidgetData((prev) => ({
          ...
          inventoryHealth: {
            ...
            breakdown:
              inventoryHealth.warehouseUtilization?.length > 0
                ...
                summary: {
                  ...
                  avgTurnover:
                      inventoryHealth.topMoving.reduce(
                        (sum, i) =>
                          sum +
                          (parseFloat(i.turnover_ratio || i.turnoverRatio) ||
                            0),
                        0,
      ...
      // Fetch VAT metrics
      const vatMetrics = await dashboardService
        .getVATMetrics()
        .catch(() => null);
      if (vatMetrics) {
        setWidgetData((prev) => ({
      ...
      const productAnalytics = await dashboardService
        .getProductAnalytics()
        .catch(() => null);
      if (productAnalytics) {
        setWidgetData((prev) => ({
      ...
      const customerInsights = await dashboardService
        .getCustomerInsights()
        .catch(() => null);
      if (customerInsights) {
        setWidgetData((prev) => ({
          ...
          customerSegments: {
            ...
            totalRevenue:
              customerInsights.topCustomers?.reduce(
          ...
          newCustomers: {
            ...
            summary: {
              ...
              previousPeriodCount: Math.max(
                0,
                (customerInsights.newCustomersThisMonth || 0) - 2,
              ),
              target: Math.max(
                15,
                (customerInsights.newCustomersThisMonth || 0) + 5,
              ),
              ...
              avgFirstOrderValue: customerInsights.avgCLV
                ? customerInsights.avgCLV / 12
                : 0,
              conversionRate: 68,
              retentionRate: 100 - (customerInsights.churnRate || 0),
              previousRetentionRate:
                100 - (customerInsights.churnRate || 0) - 3,
            },
            acquisitionSources: [
              {
                source: 'Referral',
                count: Math.ceil(
                  (customerInsights.newCustomersThisMonth || 0) * 0.4,
                ),
                value: 0,
                percent: 40,
              },
              {
                ...
                count: Math.ceil(
                  (customerInsights.newCustomersThisMonth || 0) * 0.3,
                ),
                value: 0,
                percent: 30,
              },
              {
                source: 'Exhibition',
                count: Math.ceil(
                  (customerInsights.newCustomersThisMonth || 0) * 0.2,
                ),
                value: 0,
                percent: 20,
              },
              {
                source: 'Website',
                count: Math.ceil(
                  (customerInsights.newCustomersThisMonth || 0) * 0.1,
                ),
                value: 0,
                percent: 10,
              },
            ...
            recentCustomers:
              customerInsights.topCustomers?.slice(0, 4).map((c, idx) => ({
                ...
                source: ['Referral', 'Cold Outreach', 'Exhibition', 'Website'][
                  idx % 4
                ],
      ...
      const agentPerformance = await dashboardService
        .getAgentPerformance()
        .catch(() => null);
      if (agentPerformance) {
        setWidgetData((prev) => ({
      ...
      const stockTurnover = await dashboardService
        .getStockTurnover()
        .catch(() => null);
      if (stockTurnover) {
        setWidgetData((prev) => ({
      ...
      const warehouseUtilization = await dashboardService
  ...
  return (
    <div
      className={`p-4 md:p-6 lg:p-8 min-h-screen ${isDarkMode ? 'bg-[#121418]' : 'bg-[#FAFAFA]'}`}
    >
      {/* Header */}
      <div
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            <h1
              className={`text-3xl font-semibold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
            >
              Dashboard
            </h1>
          ...
          <div className="flex items-center gap-2">
            {isRefreshing && (
              <div className="animate-spin h-4 w-4 border-b border-teal-500 rounded-full" />
            )}
            <button
      ...
      {/* Tab Navigation */}
      <div
        className={`flex gap-2 mb-6 overflow-x-auto pb-2 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}
      >
        {TABS.map((tab) => (
          <button
            key={tab.id}
            ...
            <StatsCard variant="default">
              <div className="p-4 h-full flex flex-col justify-between">
                <div className="flex items-start justify-between">
                  <div>
                    <p
                      className={`text-sm uppercase tracking-wide ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
                    >
                      Total Revenue
                    </p>
                    <h3
                ...
                <ChangeIndicator positive={stats.revenueChange >= 0}>
                  ...
                  {Math.abs(stats.revenueChange || 0).toFixed(1)}% from last
                  month
                </ChangeIndicator>
              </div>
            </StatsCard>
                ...
                <WidgetErrorBoundary widgetName="Warehouse Utilization">
                  <Suspense fallback={<WidgetSkeleton variant="chart" />}>
                    <LazyWarehouseUtilizationWidget
                      ...
                      onRefresh={() =>
                        handleWidgetRefresh('warehouseUtilization')
                      }
                    />
                  </Suspense>
                </WidgetErrorBoundary>
