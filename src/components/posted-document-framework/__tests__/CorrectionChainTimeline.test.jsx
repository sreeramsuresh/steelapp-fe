...
describe("CorrectionChainTimeline", () => {
  it("returns null when no nodes", () => {
    const { container } = renderWithTheme(
      <CorrectionChainTimeline nodes={[]} edges={[]} />
    );
  ...
  it("renders nodes in order", () => {
    ...
    renderWithTheme(
      <CorrectionChainTimeline nodes={nodes} edges={edges} />
    );
  ...
  it("shows computed summary when provided", () => {
    ...
    renderWithTheme(
      <CorrectionChainTimeline nodes={nodes} edges={[]} computed={computed} />
    );
  ...
  it("renders node status badge", () => {
    const nodes = [
      { id: "inv-1", number: "INV-001", type: "invoice", amount: 100, sign: 1, status: "issued" },
    ];
  ...
  it("renders clickable links in live mode with onNavigate", () => {
    ...
    const nodes = [
      { id: "inv-1", number: "INV-001", type: "invoice", amount: 100, sign: 1, docId: 42 },
    ];
