            ...
            <div className={`p-6 ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}>
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="font-medium">{statement.customerName || statement.customer_name || "Customer Name"}</p>
                  {(statement.customerCompany || statement.customer_company) && (
                    <p className="text-sm">{statement.customerCompany || statement.customer_company}</p>
                  )}
                </div>
                <div className="text-right">
                  <div className="mb-2">
                  ...
                  <div className="mb-2">
                    <span className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Generated: </span>
                    <span className="font-medium">
                      {formatDate(statement.createdAt || statement.created_at || new Date())}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className={`mb-6 p-4 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Opening Balance</p>
                    <p className={`text-lg font-semibold ${openingBalance >= 0 ? "text-gray-600" : "text-green-600"}`}>
                      {formatCurrency(Math.abs(openingBalance))}
                    </p>
                  </div>
                  <div className="text-center">
                    <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Total Invoiced</p>
                    <p className="text-lg font-semibold text-orange-600">{formatCurrency(totalInvoices)}</p>
                  </div>
                  <div className="text-center">
                  ...
                  <div className="text-center">
                    <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Closing Balance</p>
                    <p className={`text-lg font-bold ${closingBalance > 0 ? "text-red-600" : "text-green-600"}`}>
                      {formatCurrency(Math.abs(closingBalance))}
                      <span className="text-xs ml-1">
                        {closingBalance > 0 ? "(Due)" : closingBalance < 0 ? "(Credit)" : ""}
                      </span>
                    </p>
                  </div>
                </div>
              ...
              {statement.transactions && statement.transactions.length > 0 ? (
                <div className="mb-6">
                  ...
                  >
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {statement.transactions.slice(0, 10).map((txn, index) => (
                          <tr
                            key={txn.id || txn.name || `txn-${index}`}
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-right font-medium">{formatCurrency(txn.balance || 0)}</td>
                          </tr>
                        ))}
                        {statement.transactions.length > 10 && (
                          <tr className={`border-t ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}>
                            <td colSpan="6" className="px-4 py-3 text-center text-gray-500">
                              ... and {statement.transactions.length - 10} more transactions
                            </td>
                          </tr>
                        )}
                      </tbody>
                ...
                <div className={`mb-6 p-8 text-center rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}>
              ...
              <div className={`text-center text-sm ${isDarkMode ? "text-gray-500" : "text-gray-400"}`}>
                <p>This is a preview. Download the PDF for the complete statement with all transactions.</p>
              </div>
            </div>
          </div>
        </div>
