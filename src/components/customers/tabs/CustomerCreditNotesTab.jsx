...
export default function CustomerCreditNotesTab({ customerId }) {
  ...
  const fetchCreditNotes = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get(
        `/credit-notes?customerId=${customerId}`,
      );
  ...
  const summaryStats = {
    ...
    totalAmount: filteredCreditNotes.reduce(
    ...
    totalRemaining: filteredCreditNotes.reduce(
      (sum, cn) => sum + (parseFloat(cn.remainingBalance) || 0),
      0,
    ),
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`${cardBg} border ${borderColor} rounded-lg p-4`}>
          ...
          <p className={`text-2xl font-bold ${primaryText}`}>
            {formatCurrency(summaryStats.totalApplied)}
          </p>
        </div>
      ...
      <div className={`${cardBg} border ${borderColor} rounded-lg p-4`}>
        <div className="flex flex-wrap gap-4">
          ...
          <div className="flex-1 min-w-[200px]">
            <label
              htmlFor="credit-note-status-filter"
