...
import { AlertTriangle, CheckCircle, Clock, DollarSign, FileText, RefreshCw } from "lucide-react";
...
export default function CustomerCreditNotesTab({ customerId }) {
  ...
  const paginatedCreditNotes = filteredCreditNotes.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  if (error) {
    return (
      ...
      >
        ...
        <button type="button" onClick={fetchCreditNotes}
    ...
    <div className="space-y-6">
      ...
      <div className={`${cardBg} border ${borderColor} rounded-lg p-4`}>
        <div className="flex flex-wrap gap-4">
          ...
          <div className="flex-1 min-w-[200px]">
            <label
              htmlFor="credit-note-date-range-filter"
                ...
                paginatedCreditNotes.map((creditNote) => {
                  ...
                  return (
                    <tr key={creditNote.id} className={hoverBg}>
                      ...
                      <td className={`px-4 py-3 whitespace-nowrap ${secondaryText}`}>{formatDate(creditNote.date)}</td>
        ...
        {totalPages > 1 && (
          <div className={`px-4 py-3 border-t ${borderColor} flex items-center justify-between`}>
            <div className={`text-sm ${secondaryText}`}>
              ...
              {Math.min(currentPage * itemsPerPage, filteredCreditNotes.length)} of {filteredCreditNotes.length} credit
              notes
            </div>
