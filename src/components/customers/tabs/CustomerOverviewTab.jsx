...
export default function CustomerOverviewTab({ customer }) {
  ...
  const totalOutstanding = agingCurrent + aging1To30 + aging31To60 + aging61To90 + aging90Plus;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className={`${cardBg} border ${borderColor} rounded-lg p-6 shadow-sm`}>
        ...
        {/* Customer Name & Code */}
        <div className="mb-4">
          <h3 className={`text-2xl font-bold ${primaryText} mb-2`}>{customer.name || "N/A"}</h3>
          <span
        ...
        <div className="mb-4">
          ...
          >
            {customer.status === "active" ? <CheckCircle className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
        ...
        {/* Payment Terms */}
        <div className="mb-4">
          <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>Payment Terms</div>
          <p className={`${secondaryText} flex items-center gap-2`}>
            <Clock className="w-4 h-4" />
            {customer.paymentTermsDays || customer.payment_terms_days || customer.paymentTerms
        ...
        <div className="space-y-3 mb-4">
          <div className={`block text-xs uppercase tracking-wide ${mutedText}`}>Contact Information</div>
        ...
        {customer.trnNumber && (
          <div className="mb-4">
            <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>Tax Registration Number</div>
            <p className={`${secondaryText} font-mono text-sm`}>{customer.trnNumber}</p>
          </div>
        )}
      ...
      <div className={`${cardBg} border ${borderColor} rounded-lg p-6 shadow-sm`}>
        ...
        {/* Credit Limit */}
        <div className="mb-4">
          <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>Credit Limit</div>
          <p className={`text-2xl font-bold ${primaryText}`}>{formatCurrency(creditLimit)}</p>
        </div>

        {/* Credit Used */}
        <div className="mb-4">
          <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>Credit Used</div>
          <p className={`text-xl font-semibold ${secondaryText}`}>{formatCurrency(creditUsed)}</p>
        </div>

        {/* Available Credit */}
        <div className="mb-4">
          <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>Available Credit</div>
          <p className={`text-xl font-semibold ${getAvailableCreditColor(availableCredit, creditLimit)}`}>
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            <div className={`text-xs uppercase tracking-wide ${mutedText}`}>Credit Utilization</div>
            <span className={`text-sm font-semibold ${getUtilizationColor(utilizationPercent)}`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          {customer.creditGrade && (
            <div>
              <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-2`}>Credit Grade</div>
              <span
          ...
          {customer.creditScore !== undefined && customer.creditScore !== null && (
            <div>
              <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-2`}>Credit Score</div>
              <div className="flex items-baseline gap-1">
                <span className={`text-2xl font-bold ${primaryText}`}>{customer.creditScore}</span>
      ...
      <div className={`${cardBg} border ${borderColor} rounded-lg p-6 shadow-sm`}>
        ...
        {/* Total Outstanding */}
        <div className="mb-4">
          <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>Total Outstanding</div>
          <p className={`text-2xl font-bold ${primaryText}`}>{formatCurrency(totalOutstanding)}</p>
        </div>

        {/* Overdue Amount */}
        <div className="mb-4">
          <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>Overdue Amount</div>
          <div className="flex items-baseline gap-2">
            <p className={`text-xl font-semibold ${totalOverdue > 0 ? "text-red-500" : secondaryText}`}>
              {formatCurrency(totalOverdue)}
            </p>
            {totalOverdue > 0 && <span className={`text-sm ${mutedText}`}>({overduePercent.toFixed(1)}%)</span>}
          </div>
        </div>
        ...
        {customer.dsoDays !== undefined && customer.dsoDays !== null && (
          <div className="mb-4">
            <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>
              Days Sales Outstanding (DSO)
            </div>
            <p className={`text-lg font-semibold ${secondaryText}`}>{customer.dsoDays} days</p>
          </div>
        )}

        {/* Oldest Invoice Age */}
        <div className="mb-6">
          <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-1`}>Oldest Invoice Age</div>
          <p
            className={`text-sm font-medium ${
              aging90Plus > 0 ? "text-red-500" : aging61To90 > 0 ? "text-orange-500" : secondaryText
            }`}
          >
            {getOldestInvoiceAge()}
          </p>
        ...
        <div className="space-y-3">
          <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-2`}>Aging Breakdown</div>

          {/* Current */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${agingBgColors.current}`} />
              <span className={`text-sm ${secondaryText}`}>Current</span>
            </div>
            <span className={`text-sm font-semibold ${agingColors.current}`}>{formatCurrency(agingCurrent)}</span>
          </div>

          {/* 1-30 days */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${agingBgColors["1-30"]}`} />
              <span className={`text-sm ${secondaryText}`}>1-30 days</span>
            </div>
            <span className={`text-sm font-semibold ${agingColors["1-30"]}`}>{formatCurrency(aging1To30)}</span>
          </div>

          {/* 31-60 days */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${agingBgColors["31-60"]}`} />
              <span className={`text-sm ${secondaryText}`}>31-60 days</span>
            </div>
            <span className={`text-sm font-semibold ${agingColors["31-60"]}`}>{formatCurrency(aging31To60)}</span>
          </div>

          {/* 61-90 days */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${agingBgColors["61-90"]}`} />
              <span className={`text-sm ${secondaryText}`}>61-90 days</span>
            </div>
            <span className={`text-sm font-semibold ${agingColors["61-90"]}`}>{formatCurrency(aging61To90)}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className={`text-sm font-semibold ${agingColors["90+"]}`}>{formatCurrency(aging90Plus)}</span>
          </div>
