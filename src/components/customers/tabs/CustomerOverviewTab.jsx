    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className={`${cardBg} border ${borderColor} rounded-lg p-6 shadow-sm`}>
        ...
        <div className="mb-4">
          ...
          <span className={`inline-block px-3 py-1 text-sm font-mono rounded ${
        ...
        {/* Status Badge */}
        <div className="mb-4">
          <span className={`inline-flex items-center gap-2 px-3 py-1 text-sm font-medium uppercase tracking-wider rounded-full ${
        ...
        <div className="mb-4">
          ...
          <p className={`${secondaryText} flex items-center gap-2`}>
            ...
            Net {customer.paymentTermsDays || customer.payment_terms_days || customer.paymentTerms || 'N/A'} days
          </p>
        </div>
          ...
          {customer.address && (
            <div className={`flex items-start gap-2 ${secondaryText}`}>
              ...
              <span className="text-sm">
                {[customer.address.street, customer.address.city, customer.address.state, customer.address.postalCode, customer.address.country].filter(Boolean).join(', ')}
              </span>
            </div>
          )}
        </div>
      ...
      <div className={`${cardBg} border ${borderColor} rounded-lg p-6 shadow-sm`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          {customer.creditGrade && (
            <div>
              <div className={`block text-xs uppercase tracking-wide ${mutedText} mb-2`}>
                Credit Grade
              </div>
              <span className={`inline-block px-4 py-2 text-lg font-bold border rounded-lg ${getGradeColor(customer.creditGrade)}`}>
                {customer.creditGrade}
              </span>
            </div>
          )}
          ...
          {customer.creditScore !== undefined && customer.creditScore !== null && (
            <div>
              ...
              {/* Score Bar */}
              <div className={`mt-2 w-full h-2 rounded-full ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} overflow-hidden`}>
                <div 
      ...
      <div className={`${cardBg} border ${borderColor} rounded-lg p-6 shadow-sm`}>
        ...
        <div className="mb-6">
          ...
          <p className={`text-sm font-medium ${
            aging90Plus > 0 ? 'text-red-500' : 
              aging61To90 > 0 ? 'text-orange-500' : 
                secondaryText
          }`}>
            {getOldestInvoiceAge()}
          </p>
