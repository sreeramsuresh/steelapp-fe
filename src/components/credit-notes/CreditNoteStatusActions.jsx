...
const CreditNoteStatusActions = ({
  ...
  const loadAllowedTransitions = async () => {
    try {
      setLoading(true);
      const response =
        await creditNoteService.getAllowedTransitions(creditNoteId);
      setAllowedTransitions(
        response.allowed_transitions || response.allowedTransitions || [],
      );
    } catch (error) {
    ...
    try {
      ...
      switch (targetStatus) {
        case "issued":
          ...
          break;
        case "items_received":
          result = await creditNoteService.markItemsReceived(
            creditNoteId,
            extraData,
          );
          break;
        case "applied":
          result = await creditNoteService.applyCreditNote(
            creditNoteId,
            extraData.notes,
          );
          break;
        case "completed":
          result = await creditNoteService.completeCreditNote(
            creditNoteId,
            extraData.notes,
          );
          break;
        case "cancelled":
          result = await creditNoteService.cancelCreditNote(
            creditNoteId,
      ...
      notificationService.success(
        `Credit note ${targetStatus.replace("_", " ")} successfully`,
      );
      if (onStatusChange) {
      ...
      notificationService.error(
        error.message ||
          `Failed to ${targetStatus.replace("_", " ")} credit note`,
      );
    } finally {
