...
const ScrapItemsList = ({ creditNoteId = null, showFilters = true }) => {
  ...
  const fetchScrapItems = async () => {
    try {
      ...
      if (creditNoteId) {
        response =
          await creditNoteService.getScrapItemsByCreditNote(creditNoteId);
      } else {
    ...
    >
      ...
      {filteredItems.length === 0 ? (
        ...
        >
          ...
          <p>
            {searchTerm
              ? 'No scrap items match your search'
              : 'No scrap items recorded'}
          </p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            >
              {filteredItems.map((item, index) => (
                ...
                >
                  ...
                  <td
                    className={`py-3 px-3 text-center ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}
                  >
                    {item.quantity}
                  </td>
                  <td className="py-3 px-3">
                    ...
                    >
                      {SCRAP_REASON_LABELS[item.scrapReason] ||
                        item.scrapReason ||
                        'Unknown'}
                    </span>
                  </td>
                  {!creditNoteId && (
                    <td
                      className={`py-3 px-3 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}
                    >
                      {item.creditNoteNumber || '-'}
                    </td>
                  )}
                  <td
                    className={`py-3 px-3 text-right ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}
                  >
            ...
            {filteredItems.length > 0 && (
              <tfoot>
                <tr
                  className={`border-t-2 ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}`}
                >
                  <td
                  ...
                  <td
                    className={`py-3 px-3 text-right font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
                  >
                    {formatCurrency(
                      filteredItems.reduce(
                        (sum, item) => sum + (item.scrapValue || 0),
                        0,
                      ),
                    )}
                  </td>
