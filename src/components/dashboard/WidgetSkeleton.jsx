...
const WidgetSkeleton = ({ variant = 'card', size = 'md' }) => {
  ...
  const skeletonBar = (width, height = 'h-4') => (
    <div
      className={`${height} ${width} rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}
    />
  );
      ...
      <div className="mt-4 space-y-3">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex items-center gap-3">
            <div
              className={`w-8 h-8 rounded-full ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}
            />
  ...
  const renderTableSkeleton = () => (
    ...
    >
      ...
      <div
        className={`flex gap-4 pb-2 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}
      >
