...
class WidgetErrorBoundary extends Component {
  ...
  componentDidCatch(error, errorInfo) {
    ...
    console.error(`[Widget Error] ${this.props.widgetName || "Unknown Widget"} crashed:`, error, errorInfo);
    ...
    if (hasError) {
      ...
      return (
        ...
        >
          ...
          >
            <AlertTriangle size={24} className={isDarkMode ? "text-red-400" : "text-red-500"} />
          </div>

          <h4 className={`text-base font-semibold mb-2 text-center ${isDarkMode ? "text-white" : "text-gray-900"}`}>
            {widgetName || "Widget"} could not load
          </h4>

          <p className={`text-sm text-center mb-4 max-w-xs ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
