  ...
  return (
    <div data-testid="bar-chart">
      <ResponsiveContainer width="100%" height={height}>
        <BarChart data={data} layout={layout} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
          {showGrid && <CartesianGrid strokeDasharray="3 3" stroke={themeColors.grid} vertical={false} />}
          <XAxis
          ...
          {showLegend && <Legend wrapperStyle={{ color: themeColors.textPrimary }} />}
          {multiSeries ? (
            ...
            <Bar dataKey={dataKey} fill={chartColors[0]} barSize={barSize} radius={[4, 4, 0, 0]}>
              {data?.map((entry) => (
                <Cell
                  key={`${dataKey}-${entry.name || entry.value}`}
                  fill={entry.color || chartColors[data.indexOf(entry) % chartColors.length]}
                />
              ))}
            </Bar>
          )}
      ...
      <ResponsiveContainer width="100%" height={height}>
        <PieChart>
          ...
          >
            {data?.map((entry) => (
              <Cell
                key={`pie-${entry.name || entry.value}`}
                fill={entry.color || chartColors[data.indexOf(entry) % chartColors.length]}
              />
            ))}
          </Pie>
    ...
    <div data-testid="radar-chart">
      <ResponsiveContainer width="100%" height={height}>
        <RadarChart data={data} cx="50%" cy="50%" outerRadius="80%">
          ...
          <PolarAngleAxis dataKey={angleKey} tick={{ fill: themeColors.textSecondary, fontSize: 12 }} />
          <PolarRadiusAxis tick={{ fill: themeColors.textSecondary, fontSize: 10 }} tickFormatter={formatter} />
          <Tooltip content={<CustomTooltip isDarkMode={isDarkMode} formatter={formatter} />} />
          {showLegend && <Legend wrapperStyle={{ color: themeColors.textPrimary }} />}
          {multiSeries ? (
            multiSeries.map((series, index) => (
            ...
            <Radar name={dataKey} dataKey={dataKey} stroke={chartColors[0]} fill={chartColors[0]} fillOpacity={0.3} />
          )}
        </RadarChart>
      </ResponsiveContainer>
    </div>
...
export const DonutChartWrapper = (props) => <PieChartWrapper {...props} innerRadius="60%" />;
