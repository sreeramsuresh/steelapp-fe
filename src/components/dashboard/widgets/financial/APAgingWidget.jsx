import { AlertTriangle, Clock, Info, RefreshCw, TrendingDown } from "lucide-react";
import { useEffect, useState } from "react";
import { useTheme } from "../../../../contexts/ThemeContext";

// Fallback mock data for AP Aging (used only when no API data is available)
...
const APAgingWidget = ({ data: propData, onRefresh, loading: externalLoading }) => {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className={`text-lg font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>AP Aging</h3>
        </div>
        <div className="text-center py-8">
          <Clock size={48} className={`mx-auto mb-4 opacity-50 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`} />
          <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>No payables data available</p>
        </div>
      </div>
    );
  }
      ...
      <div className={`pt-4 border-t ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}>
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="text-right">
            ...
            <p className="text-lg font-bold text-red-500">{formatCurrency(data.overdueAP)}</p>
          </div>
        </div>
