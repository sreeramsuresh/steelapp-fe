...
const MOCK_CASH_FLOW = {
  mtd: {
    ...
    netCashFlow: 165000,
    trend: [
      { day: 'W1', inflow: 120000, outflow: 85000 },
      { day: 'W2', inflow: 135000, outflow: 95000 },
      { day: 'W3', inflow: 110000, outflow: 78000 },
      { day: 'W4', inflow: 120000, outflow: 62000 },
    ],
  },
  qtd: {
    inflows: 1450000,
    ...
    netCashFlow: 470000,
    trend: [
      { day: 'M1', inflow: 450000, outflow: 310000 },
      { day: 'M2', inflow: 520000, outflow: 350000 },
      { day: 'M3', inflow: 480000, outflow: 320000 },
    ],
  },
  ytd: {
    inflows: 5800000,
    ...
    netCashFlow: 1700000,
    trend: [
      { day: 'Q1', inflow: 1400000, outflow: 1000000 },
      { day: 'Q2', inflow: 1500000, outflow: 1050000 },
      { day: 'Q3', inflow: 1450000, outflow: 1030000 },
      { day: 'Q4', inflow: 1450000, outflow: 1020000 },
    ],
  },
};

const CashFlowWidget = ({ data: propData, onRefresh }) => {
  const { isDarkMode } = useTheme();
  const [loading, setLoading] = useState(false);
  const [period, setPeriod] = useState('mtd');
  const [data, setData] = useState(propData || MOCK_CASH_FLOW);
  ...
  const handleRefresh = async () => {
    ...
    try {
      if (onRefresh) {
        ...
        setData(freshData || MOCK_CASH_FLOW);
      }
    } finally {
      setLoading(false);
    }
  ...
  const maxValue = Math.max(
    ...currentData.trend.map(d => Math.max(d.inflow, d.outflow))
  );
  ...
  return (
    <div className={`rounded-xl border p-4 sm:p-6 transition-all duration-300 hover:shadow-lg ${
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        ...
        <div className={`p-3 rounded-lg ${
          ...
          <p className={`text-sm sm:text-base font-bold text-green-600`}>
            {formatCurrency(currentData.inflows)}
          </p>
        </div>
        ...
        <div className={`p-3 rounded-lg ${
          ...
          <p className={`text-sm sm:text-base font-bold text-red-600`}>
            {formatCurrency(currentData.outflows)}
          </p>
        </div>
        ...
        <div className={`p-3 rounded-lg ${
          ...
          <p className={`text-sm sm:text-base font-bold ${
            currentData.netCashFlow >= 0 ? 'text-blue-600' : 'text-orange-600'
          }`}>
      ...
      <div className="relative h-32 mt-4">
        ...
        <div className="ml-12 h-full flex items-end gap-2">
          {currentData.trend.map((item, index) => {
            const inflowHeight = maxValue > 0 ? (item.inflow / maxValue) * 100 : 0;
            const outflowHeight = maxValue > 0 ? (item.outflow / maxValue) * 100 : 0;

            return (
              <div key={index} className="flex-1 flex flex-col items-center group relative">
                {/* Tooltip */}
                <div className={`hidden group-hover:block absolute -top-20 left-1/2 -translate-x-1/2 px-3 py-2 rounded-lg text-xs z-10 shadow-lg ${
      ...
      {/* Legend */}
      <div className="flex justify-center gap-4 mt-3 pt-3 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}">
