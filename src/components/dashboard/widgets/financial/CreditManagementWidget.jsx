    ...
    return (
      ...
      >
        ...
        >
          ...
          <span className="text-xs mt-1 opacity-70">Set credit limits on customers to track utilisation</span>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex justify-center mb-4">
        <div className="relative w-40 h-20 overflow-hidden">
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center">
            <p className={`text-2xl font-bold ${utilizationColors.text}`}>{data.utilizationPercent || 0}%</p>
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Utilized</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}>
          <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Total Limit</span>
          <p className={`text-sm font-bold ${isDarkMode ? "text-white" : "text-gray-900"}`}>
            {formatCurrency(data.totalCreditLimit)}
          </p>
        </div>
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}>
          <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Used</span>
          <p className={`text-sm font-bold ${utilizationColors.text}`}>{formatCurrency(data.totalCreditUsed)}</p>
        </div>
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-green-900/20" : "bg-green-50"}`}>
          ...
          <p className="text-sm font-bold text-green-500">{formatCurrency(data.availableCredit)}</p>
        </div>
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}>
          <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Trend</span>
          <p
            ...
            {data.atRiskAccounts.map((account) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-red-500 font-medium">{formatCurrency(account.overdue)} overdue</span>
                </div>
                {/* Mini progress bar */}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 text-xs">
              <span className="flex items-center gap-1">
                ...
                <span className={isDarkMode ? "text-gray-400" : "text-gray-500"}>{data.summary.highRisk} high</span>
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-yellow-500"></span>
                <span className={isDarkMode ? "text-gray-400" : "text-gray-500"}>{data.summary.mediumRisk} med</span>
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-green-500"></span>
                <span className={isDarkMode ? "text-gray-400" : "text-gray-500"}>{data.summary.lowRisk} low</span>
              </span>
            </div>
          </div>
        </div>
