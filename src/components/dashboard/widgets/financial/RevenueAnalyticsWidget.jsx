import { BarChart3, Info, PieChart, RefreshCw } from "lucide-react";
...
const RevenueAnalyticsWidget = ({ data: propData, onRefresh }) => {
  ...
  const renderPieChart = () => {
    ...
    const gradientStops = segments.map((seg) => `${seg.color} ${seg.startPercent}% ${seg.endPercent}%`).join(", ");
      ...
      <div className="flex items-center gap-6">
        ...
        <div
          ...
          style={{
            ...
            boxShadow: isDarkMode ? "inset 0 0 0 3px rgba(30, 35, 40, 1)" : "inset 0 0 0 3px rgba(255, 255, 255, 1)",
          }}
        >
          {/* Center hole for donut effect */}
          <div
            className={`absolute inset-6 sm:inset-8 rounded-full flex items-center justify-center ${
          ...
          {currentData.map((item, index) => (
            <div key={item.name} className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                />
                <span className={`text-xs ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>{item.name}</span>
              </div>
    ...
    >
      ...
      >
        ...
        <div className="text-right">
          <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Categories</span>
          <p className={`text-lg font-bold text-indigo-500`}>{currentData.length}</p>
        </div>
      </div>
    </div>
  );
