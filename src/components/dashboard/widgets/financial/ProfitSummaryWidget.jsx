import { ArrowRight, DollarSign, Info, RefreshCw, TrendingDown, TrendingUp } from "lucide-react";
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}>
          <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Gross Margin</span>
        ...
        {/* Net Margin */}
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}>
          <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Net Margin</span>
          <div className="flex items-baseline gap-2">
            <span className={`text-xl font-bold ${isDarkMode ? "text-white" : "text-gray-900"}`}>
              {netMarginPercent.toFixed(1)}%
            </span>
            {netProfitChange !== 0 && (
              <span className={`text-xs flex items-center ${netProfitChange > 0 ? "text-green-500" : "text-red-500"}`}>
                {netProfitChange > 0 ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
      ...
      <div className="space-y-2">
        {waterfallItems.map((item, index) => {
          ...
          const isSubtotalOrTotal = item.type === "subtotal" || item.type === "total";
            ...
            <div key={item.label} className="group">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  ...
                  {item.type === "negative" && <span className="text-xs text-red-500">-</span>}
                </div>
                <span
              ...
              {index < waterfallItems.length - 1 && index !== 1 && (
                <div className="flex justify-center py-1">
                  <ArrowRight size={14} className={isDarkMode ? "text-gray-600" : "text-gray-300"} />
                </div>
              )}
            </div>
          );
      ...
      <div className={`mt-4 pt-4 border-t ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}>
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Net Profit</span>
