...
const KPICard = ({
  ...
  const isNearTarget =
    colorScheme === "warning" ||
    (target && value >= target * 0.8 && value < target);
  const isBelowTarget =
    colorScheme === "error" || (target && value < target * 0.8);
  ...
  return (
    <div
      className={`p-3 sm:p-4 rounded-xl border transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 ${
        isDarkMode
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <div
            className={`w-8 h-8 rounded-lg flex items-center justify-center ${getBgColor()}`}
          >
      ...
      <div className="flex items-baseline gap-2 mb-1">
        ...
        >
          {typeof value === "number"
            ? value.toFixed(unit === "%" || unit === "" ? 1 : 0)
            : value}
          <span
            className={`text-sm font-normal ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
          >
            {unit}
          </span>
        </span>
      ...
      <div className="flex items-center justify-between">
        ...
        {target !== undefined && (
          <div
            className={`flex items-center gap-1 text-xs ${isDarkMode ? "text-gray-500" : "text-gray-400"}`}
          >
            <Target size={12} />
            <span>
              Target: {target}
...
const FinancialKPICards = ({ data: propData, onRefresh }) => {
  ...
  const getGrossMarginColor = () => {
    ...
    if (data.grossMargin.value >= data.grossMargin.target * 0.9)
      return "warning";
    return "error";
  };
