...
import { Building2, Info, MapPin, PieChart, RefreshCw, Users } from "lucide-react";
...
const CustomerSegmentsWidget = ({ data: propData, onRefresh, onViewSegment, onViewDetails, isLoading = false }) => {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className={`text-lg font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>Customer Segments</h3>
        </div>
        <div className="text-center py-8">
          <PieChart size={48} className={`mx-auto mb-4 opacity-50 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`} />
          <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>No segmentation data available</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="relative w-24 h-24 flex-shrink-0">
          <svg aria-label="icon" viewBox="0 0 100 100" className="w-full h-full transform -rotate-0">
            ...
            <circle cx="50" cy="50" r="20" fill={isDarkMode ? "#1E2328" : "#FFFFFF"} />
          </svg>
          {/* Center text */}
        ...
        {activeSegments.slice(0, 3).map((segment, _idx) => (
          ...
          >
            ...
            <div className="flex items-center justify-between text-xs">
              <span className={isDarkMode ? "text-gray-400" : "text-gray-500"}>{segment.count} customers</span>
              <span className={isDarkMode ? "text-gray-400" : "text-gray-500"}>{segment.percent}% of total</span>
            </div>
      ...
      {/* Action Button */}
      {onViewDetails && (
        <button type="button" onClick={() => onViewDetails(segmentData)}
