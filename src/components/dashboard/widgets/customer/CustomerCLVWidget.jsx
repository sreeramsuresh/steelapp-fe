...
import { Crown, Info, RefreshCw, TrendingDown, TrendingUp } from "lucide-react";
...
const CustomerCLVWidget = ({ data: propData, onRefresh, onViewCustomer, onViewDetails, isLoading = false }) => {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className={`text-lg font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>Customer CLV</h3>
        </div>
        <div className="text-center py-8">
          <Crown size={48} className={`mx-auto mb-4 opacity-50 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`} />
          <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>No CLV data available</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Lifetime Value Analysis</p>
          </div>
        </div>
      ...
      <div className={`p-4 rounded-lg mb-4 ${isDarkMode ? "bg-[#2E3B4E]" : "bg-gray-50"}`}>
        <div className="flex items-center justify-between">
          <div>
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Average Customer CLV</p>
          ...
          <div
            className={`flex items-center gap-1 px-2 py-1 rounded text-sm font-medium ${
              parseFloat(clvChange) >= 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
            }`}
          >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-right flex items-center gap-2">
                    ...
                    {trendInfo.Icon && <trendInfo.Icon size={16} className={trendInfo.color} />}
                  </div>
                </div>
              </div>
            );
          ...
          {segments.map((segment, idx) => (
            <div key={idx} className={`p-3 rounded-lg ${isDarkMode ? "bg-[#2E3B4E]" : "bg-gray-50"}`}>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className={isDarkMode ? "text-gray-400" : "text-gray-500"}>{segment.percent}% of customers</span>
              </div>
