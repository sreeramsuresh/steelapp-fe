...
const NewCustomerWidget = ({
  ...
  const [customerData, setCustomerData] = useState(
    propData || MOCK_NEW_CUSTOMER_DATA,
  );
  ...
  const getSourceIcon = (source) => {
    const sourceConfig = customerData.acquisitionSources.find(
      (s) => s.source === source,
    );
  ...
  const growthPercent = (
    ((summary.newCustomersCount - summary.previousPeriodCount) /
      summary.previousPeriodCount) *
    100
  ).toFixed(1);
  const targetPercent = (
    (summary.newCustomersCount / summary.target) *
    100
  ).toFixed(0);
      ...
      <div className="mb-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {acquisitionSources.map((source, idx) => {
            ...
            const Icon =
              source.icon || SOURCE_CONFIG_MAP[source.source]?.icon || Target;
