...
const VATCollectionWidget = ({ data = null, onRefresh = null, onViewDetails = null, isLoading = false }) => {
  ...
  const outputVATChange = calculateChange(currentData.outputVAT, previousData.outputVAT);
  const inputVATChange = calculateChange(currentData.inputVAT, previousData.inputVAT);
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>{currentData.period}</p>
          </div>
        </div>
      ...
      <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div
              className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium ${
                outputVATChange >= 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
              }`}
            >
              {outputVATChange >= 0 ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />}
        ...
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-[#2E3B4E]" : "bg-gray-50"}`}>
          <div className="flex items-center justify-between">
            ...
            <div
              className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium ${
                inputVATChange >= 0 ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"
              }`}
            >
              {inputVATChange >= 0 ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />}
          ...
          <div className="flex items-center justify-between">
            ...
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center ${
                isNetPayable ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"
              }`}
            >
              {isNetPayable ? <TrendingUp size={20} /> : <TrendingDown size={20} />}
            </div>
          </div>
        </div>
      </div>
