...
const SlowMovingWidget = ({ data, onNavigate, onProductClick, onAction }) => {
  ...
  const formatDate = (dateString) => {
    ...
    return date.toLocaleDateString('en-AE', { month: 'short', day: 'numeric', year: '2-digit' });
  };
  ...
  return (
    <div className={`rounded-xl border p-4 ${
      ...
      {/* Value Locked Alert */}
      {summary && (
        <div className={`p-3 rounded-lg mb-4 ${isDarkMode ? 'bg-red-500/10 border border-red-500/20' : 'bg-red-50 border border-red-100'}`}>
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <div className={`text-right`}>
                  ...
                  <p className={`text-xs ${getDaysColor(product.daysInStock)}`}>
                    {product.daysInStock}d in stock
                  </p>
                </div>
              </div>
