  ...
  return (
    <div className="relative" style={{ width: size, height: size / 2 + 20 }}>
      <svg aria-label="icon" width={size} height={size / 2 + 10} className="overflow-visible">
        ...
        <text x={size / 2} y={size / 2 + 12} textAnchor="middle" className="text-xs" fill={getColor(value)}>
          {getLabel(value)}
        </text>
      </svg>
    </div>
...
const InventoryHealthWidget = ({ data, onNavigate }) => {
  ...
  const hasData =
    healthData && (healthData.totalItems > 0 || healthData.totalValue > 0) && healthData.healthScore !== undefined;
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Overall stock status</p>
            </div>
          </div>
        </div>
        <div
        ...
        >
          ...
          <span className="text-xs mt-1 opacity-70">Inventory health will appear once stock is added to warehouses</span>
        </div>
      </div>
    );
  }
  ...
  const _getCategoryHealthColor = (health) => {
    switch (health) {
      ...
      default:
        return isDarkMode ? "bg-gray-500/20 text-gray-400" : "bg-gray-100 text-gray-600";
    }
  };
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className={`text-sm font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>Inventory Health</h3>
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Overall stock status</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              ...
              <span className={`text-xs ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>
                Low Stock:{" "}
                <span className="font-semibold text-red-500">{safeNumber(healthData.alerts?.lowStock, 0)}</span>
              </span>
            </div>
            <div className="flex items-center gap-1">
              ...
              <span className={`text-xs ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>
                Overstock:{" "}
                <span className="font-semibold text-amber-500">{safeNumber(healthData.alerts?.overstock, 0)}</span>
              </span>
            </div>
          </div>
        </div>
