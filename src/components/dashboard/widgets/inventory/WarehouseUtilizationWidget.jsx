import { ArrowRightLeft, MapPin, RefreshCw, Warehouse } from "lucide-react";
...
const WarehouseUtilizationWidget = ({
  data,
  onNavigate,
  onWarehouseClick,
  onTransfer,
    ...
    if (data?.warehouses && data.warehouses.length > 0) {
      ...
      const correctedData = {
        ...
        summary: {
          ...data.summary,
          avgUtilization:
            data.summary && data.summary.totalCapacity > 0
              ? ((data.summary.totalUsed || 0) / data.summary.totalCapacity) * 100
              : data.summary?.avgUtilization || 0,
        },
      };
      setWarehouseData(correctedData);
    } else {
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Capacity across locations</p>
          </div>
        </div>
          ...
          <div className="text-right">
            ...
            <p
              className={`text-lg font-bold ${
                warehouseData.summary.avgUtilization >= 80
                  ? "text-amber-500"
                  : warehouseData.summary.avgUtilization >= 60
                    ? "text-green-500"
                    : "text-blue-500"
              }`}
            >
          ...
          <div className="space-y-2">
            {warehouseData.transfers.map((transfer, index) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className={`text-xs ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>{transfer.qty} MT</p>
                  <span
