    ...
    <div className={`rounded-xl border p-4 ${
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="relative">
          ...
          <Calendar size={12} className={`absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none ${
          ...
          <div
            ...
            className={`p-3 rounded-lg cursor-pointer transition-all ${
              hoveredCategory === category.name
                ? isDarkMode ? 'bg-[#2E3B4E]' : 'bg-gray-100'
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getCategoryBgClass(category.name)} flex items-center justify-center`}>
              ...
              <div className="text-right">
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  <span className={`text-xs flex items-center gap-0.5 ${
                    ...
                    {category.growth >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />}
      ...
      <div className={`mt-4 pt-3 border-t grid grid-cols-3 gap-4 ${
        ...
        <div className="text-center">
          <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Avg Margin</p>
          <p className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
            {(categories.reduce((sum, c) => sum + c.margin, 0) / categories.length).toFixed(1)}%
          </p>
        </div>
        <div className="text-center">
          <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Total Volume</p>
          <p className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
