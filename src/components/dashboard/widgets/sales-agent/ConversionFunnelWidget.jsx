...
import { CheckCircle, FileText, Filter, Info, Phone, RefreshCw, TrendingDown, TrendingUp } from "lucide-react";
...
const ConversionFunnelWidget = ({ data: propData, onRefresh, onViewDetails, isLoading = false }) => {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className={`text-lg font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>Conversion Funnel</h3>
        </div>
        <div className="text-center py-8">
          <Filter size={48} className={`mx-auto mb-4 opacity-50 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`} />
          <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>No funnel data available</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>{funnelData.period}</p>
          </div>
        </div>
      ...
      <div className={`p-4 rounded-lg mb-4 ${isDarkMode ? "bg-[#2E3B4E]" : "bg-gray-50"}`}>
        <div className="flex items-center justify-between">
          <div>
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Overall Conversion Rate</p>
          ...
          <div
            className={`flex items-center gap-1 px-2 py-1 rounded text-sm font-medium ${
              conversionChange >= 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
            }`}
          >
