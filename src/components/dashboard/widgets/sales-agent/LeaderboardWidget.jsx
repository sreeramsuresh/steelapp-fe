...
const LeaderboardWidget = ({
  ...
  const getMedalIcon = (rank) => {
    if (rank === 1) return { icon: '1st', color: 'from-yellow-400 to-yellow-600', shadow: 'shadow-yellow-500/30' };
    if (rank === 2) return { icon: '2nd', color: 'from-gray-300 to-gray-500', shadow: 'shadow-gray-400/30' };
    if (rank === 3) return { icon: '3rd', color: 'from-amber-500 to-amber-700', shadow: 'shadow-amber-500/30' };
    return null;
  };
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="w-8 flex-shrink-0">
                  {medal ? (
                    <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${medal.color} ${medal.shadow} shadow-lg flex items-center justify-center`}>
