...
const AgentScorecardWidget = ({ data: propData, onRefresh, onViewDetails, isLoading = false }) => {
  ...
  const getRankBadge = (rank) => {
    ...
    if (rank === 2) return { icon: "2nd", color: "bg-gray-400", textColor: "text-gray-400" };
    if (rank === 3)
      return {
        icon: "3rd",
        color: "bg-amber-600",
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className={`text-lg font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>Agent Scorecard</h3>
        </div>
        <div className="text-center py-8">
          <User size={48} className={`mx-auto mb-4 opacity-50 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`} />
          <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>No agent data available</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Performance Overview</p>
          </div>
        </div>
      ...
      <div className={`p-4 rounded-lg mb-4 ${isDarkMode ? "bg-[#2E3B4E]" : "bg-gray-50"}`}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1">
            <p className={`font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>{selectedAgent.name}</p>
            <div className="flex items-center gap-2">
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${rankBadge.color} text-white`}>
          ...
          <div className="text-right">
            ...
            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>of target</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>vs Last Period</span>
          <div
            className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium ${
              parseFloat(growthPercent) >= 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
            }`}
          >
            {parseFloat(growthPercent) >= 0 ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-[#2E3B4E]" : "bg-gray-50"}`}>
          <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>Deals Closed</p>
          <p className={`text-lg font-bold ${isDarkMode ? "text-white" : "text-gray-900"}`}>{selectedAgent.deals}</p>
        </div>
        <div className={`p-3 rounded-lg ${isDarkMode ? "bg-[#2E3B4E]" : "bg-gray-50"}`}>
          <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>New Customers</p>
      ...
      {/* Action Button */}
      {onViewDetails && (
        <button type="button" onClick={() => onViewDetails(selectedAgent)}
