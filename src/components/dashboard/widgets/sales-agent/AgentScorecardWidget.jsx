...
const AgentScorecardWidget = ({
  ...
  const growthPercent = selectedAgent && selectedAgent.lastPeriodAchieved
    ? (((selectedAgent.achieved - selectedAgent.lastPeriodAchieved) / selectedAgent.lastPeriodAchieved) * 100).toFixed(1)
    : 0;
  ...
  const getRankBadge = (rank) => {
    if (rank === 1) return { icon: '1st', color: 'bg-yellow-500', textColor: 'text-yellow-500' };
    if (rank === 2) return { icon: '2nd', color: 'bg-gray-400', textColor: 'text-gray-400' };
    if (rank === 3) return { icon: '3rd', color: 'bg-amber-600', textColor: 'text-amber-600' };
    return { icon: `${rank}th`, color: 'bg-gray-500', textColor: 'text-gray-500' };
  };
