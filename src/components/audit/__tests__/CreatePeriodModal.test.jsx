...
describe("CreatePeriodModal", () => {
  it("returns null when not open", () => {
    const { container } = render(<CreatePeriodModal isOpen={false} onClose={vi.fn()} onCreatePeriod={vi.fn()} />);
    expect(container.innerHTML).toBe("");
  });

  it("renders modal when open", () => {
    render(<CreatePeriodModal isOpen={true} onClose={vi.fn()} onCreatePeriod={vi.fn()} />);
  ...
  it("calls onClose when Cancel clicked", () => {
    const onClose = vi.fn();
    render(<CreatePeriodModal isOpen={true} onClose={onClose} onCreatePeriod={vi.fn()} />);
    screen.getByText("Cancel").click();
    expect(onClose).toHaveBeenCalledTimes(1);
  });

  it("shows Creating... when isLoading", () => {
    render(<CreatePeriodModal isOpen={true} onClose={vi.fn()} onCreatePeriod={vi.fn()} isLoading={true} />);
