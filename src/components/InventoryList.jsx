...
const InventoryList = () => {
  ...
  const fetchInventory = useCallback(async () => {
    try {
      ...
      const total = response.pagination?.total || response.total || response.pagination?.totalCount || data.length;
  ...
  const handleSelectProduct = (product) => {
    ...
    setFormData((prev) => ({
      ...
      description: product.specifications || product.description || prev.description,
      // Pricing information
    ...
    return (
      ...
      >
        <div className="flex justify-center items-center min-h-[400px]">
          ...
          <span className={`ml-4 ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>Loading inventory...</span>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      >
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className={`divide-y ${isDarkMode ? "divide-gray-700" : "divide-gray-200"}`}>
              {filteredInventory.map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`text-sm ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>{item.size}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`text-sm ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>{item.thickness}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {(() => {
                  ...
                  <td className="px-6 py-4">
                    <div className={`text-sm ${isDarkMode ? "text-white" : "text-gray-900"}`}>
                      ...
                      {item.coilNumber && (
                        <div className="flex items-center gap-1 text-xs text-gray-500">Coil: {item.coilNumber}</div>
                      )}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`text-sm font-medium ${isDarkMode ? "text-white" : "text-gray-900"}`}>
                      {item.quantityReserved > 0 ? (
                        ...
                        <span className="text-gray-400">0.000 {(item.unit || "KG").toUpperCase()}</span>
                      )}
                    </div>
                  </td>
          ...
          {inventory.length > 0 && (
            <div
              className={`flex items-center justify-between px-6 py-4 border-t ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}
            >
