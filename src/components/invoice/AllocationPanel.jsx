import { AlertTriangle, CheckCircle, Lock, Package, RefreshCw, Ship, Unlock } from "lucide-react";
...
const AllocationPanel = ({
  ...
  if (allocations.length === 0) {
    ...
    return (
      ...
      >
        <p className="text-sm text-center">No batch allocations found. Contact warehouse team if this is unexpected.</p>
      </div>
    );
  }
          ...
          <TableBody>
            {allocations.map((allocation, index) => (
              ...
              >
                ...
                <TableCell>
                  {getProcurementBadge(allocation.procurementChannel || allocation.procurement_channel || "LOCAL")}
                </TableCell>
      ...
      {isShortfall && (
        ...
        >
          ...
          <div className="flex-1 space-y-1">
            ...
            <p className="text-sm">
              Required: <span className="font-semibold">{formatQty(requiredQty)}</span> | Allocated:{" "}
              <span className="font-semibold">{formatQty(totalAllocated)}</span> | Shortfall:{" "}
              <span className="font-semibold text-red-600 dark:text-red-400">
                {formatQty(requiredQty - totalAllocated)}
              </span>
            </p>
            <p className="text-xs opacity-80">This line item cannot be fully fulfilled with current stock levels.</p>
          </div>
        </div>
      )}
