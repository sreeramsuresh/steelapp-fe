import { AlertCircle, ChevronDown, ChevronRight, History, TrendingDown, TrendingUp } from "lucide-react";
 ...
 */
const ReallocationHistoryPanel = ({ invoiceId, invoiceItemId, collapsed: initialCollapsed = true }) => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      let response;
      if (invoiceItemId) {
        response = await apiClient.get(`/invoices/items/${invoiceItemId}/reallocation-history`);
      } else {
        response = await apiClient.get(`/invoices/${invoiceId}/reallocation-history`);
      }
  ...
  const getReasonBadge = (reasonCode) => {
    ...
    return <Badge className={`text-xs ${colorClasses[reason.color]}`}>{reason.label}</Badge>;
  };
  ...
  return (
    <div
      className={`rounded-lg border ${isDarkMode ? "border-gray-700 bg-gray-800/30" : "border-gray-200 bg-gray-50/50"}`}
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {history.length > 0 && (
            <Badge className={`text-xs ${isDarkMode ? "bg-gray-700 text-gray-300" : "bg-gray-200 text-gray-600"}`}>
      ...
      {!collapsed && (
        <div className={`border-t ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}>
          {loading ? (
            ...
            <div className={`p-4 flex items-center gap-2 text-sm ${isDarkMode ? "text-red-400" : "text-red-600"}`}>
              <AlertCircle size={16} />
              {error}
            </div>
          ) : history.length === 0 ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow className={isDarkMode ? "bg-gray-800" : "bg-gray-100"}>
                    ...
                    <TableHead className="text-right w-[100px]">Cost Impact</TableHead>
                ...
                <TableBody>
                  {history.map((entry, index) => (
                    ...
                    >
                      <TableCell className="text-sm">{formatDateTime(entry.changedAt || entry.changed_at)}</TableCell>
                      <TableCell className="text-sm font-medium">
                        {entry.changedByName || entry.changed_by_name || "System"}
                      </TableCell>
                      <TableCell className="font-mono text-sm">
                        {entry.oldBatchNumber || entry.old_batch_number || "-"}
                        {entry.oldQuantity > 0 && (
                          <span className="text-gray-500 ml-1">({entry.oldQuantity || entry.old_quantity})</span>
                        )}
                      </TableCell>
                      <TableCell className="font-mono text-sm">
                        {entry.newBatchNumber || entry.new_batch_number || "-"}
                        {entry.newQuantity > 0 && (
                          <span className="text-gray-500 ml-1">({entry.newQuantity || entry.new_quantity})</span>
                        )}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        {getCostVarianceDisplay(entry.costVariance || entry.cost_variance)}
                      </TableCell>
                      <TableCell>
          ...
          {history.length > 0 && (
            <div
              className={`p-3 border-t ${isDarkMode ? "border-gray-700 bg-gray-800/50" : "border-gray-200 bg-gray-50"}`}
            >
              <div className="flex items-center justify-between text-sm">
                <span className={isDarkMode ? "text-gray-400" : "text-gray-600"}>Total Cost Impact:</span>
                <span className="font-semibold">
                  {getCostVarianceDisplay(
                    history.reduce((sum, e) => sum + (e.costVariance || e.cost_variance || 0), 0)
                  )}
                </span>
              </div>
            </div>
