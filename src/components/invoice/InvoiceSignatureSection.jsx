...
const InvoiceSignatureSection = ({ company, template = null }) => {
  ...
  if (company?.pdfSealUrl) {
    companySeal = company.pdfSealUrl.startsWith("/") ? `${baseUrl}${company.pdfSealUrl}` : company.pdfSealUrl;
  } else if (company?.sealUrl) {
    companySeal = company.sealUrl.startsWith("/") ? `${baseUrl}${company.sealUrl}` : company.sealUrl;
  }
