    ...
    >
      {hasTermsContent && (
        ...
        >
          ...
          >
            {invoice.terms && <div className="whitespace-pre-wrap">{invoice.terms}</div>}
            {invoice.notes && (
            ...
            {(invoice.warehouseName || invoice.warehouseCode) && (
              <div className="flex items-start">
                <span className="mr-2">•</span>
                <div>
                  <span className="font-semibold">Place of Supply (Warehouse):</span>{" "}
                  {[invoice.warehouseName, invoice.warehouseCode, invoice.warehouseCity].filter(Boolean).join(", ")}
                </div>
              </div>
            )}
          </div>
