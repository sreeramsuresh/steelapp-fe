...
import { AlertCircle, Check, CheckCircle, Copy, ExternalLink, Loader2, ShieldCheck, XCircle } from "lucide-react";
...
const TRNVerifyInput = ({
  ...
  const renderStatusIcon = () => {
    ...
    if (verificationResult?.verified === null && verificationResult?.formatValid) {
  ...
  return (
    <div className={`space-y-1 ${className}`}>
        ...
        >
            ...
            <div className="space-y-1">
              ...
              {verificationResult.businessName && (
                <p className={isDarkMode ? "text-gray-300" : "text-gray-700"}>
                  <span className="font-medium">Business:</span> {verificationResult.businessName}
                </p>
              )}
          ...
          {verificationResult.verified === false && (
            <div className="space-y-1">
              ...
              <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
                This TRN was not found in the FTA database. The entity may not be VAT registered.
              </p>
            </div>
          )}
          ...
          {verificationResult.verified === null && (
            <div className="space-y-2">
              <div className="flex items-center gap-2 font-medium text-yellow-700 dark:text-yellow-400">
                ...
                <span>
                  {verificationResult.apiConfigured === false ? "FTA API Not Configured" : "Verification Unavailable"}
                </span>
              </div>
              <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
      ...
      {formatError && !verificationResult && (
        <p className={`text-xs ${isDarkMode ? "text-orange-400" : "text-orange-600"}`}>{formatError}</p>
      )}

      {/* External error */}
      {externalError && <p className={`text-xs ${isDarkMode ? "text-red-400" : "text-red-600"}`}>{externalError}</p>}
