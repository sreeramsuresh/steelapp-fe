 ...
 */
const LoadingOverlay = ({ show = false, message = "Processing...", detail = null, spinnerSize = "lg" }) => {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[9999] flex items-center justify-center animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="flex justify-center mb-4">
          <Loader2 className={`${spinnerSizes[spinnerSize]} text-teal-600 animate-spin`} />
        </div>

        {/* Message */}
        <p className={`text-center font-semibold text-lg ${isDarkMode ? "text-white" : "text-gray-900"}`}>{message}</p>

        {/* Detail (optional) */}
        {detail && (
          <p className={`text-center text-sm mt-2 ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>{detail}</p>
        )}
