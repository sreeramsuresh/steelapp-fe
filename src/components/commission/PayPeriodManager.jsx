...
const PayPeriodManager = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    const config = {
      OPEN: { bg: 'bg-green-100 text-green-800', darkBg: 'bg-green-900/30 text-green-400', icon: Unlock },
      CLOSED: { bg: 'bg-yellow-100 text-yellow-800', darkBg: 'bg-yellow-900/30 text-yellow-400', icon: Lock },
      PROCESSING: { bg: 'bg-blue-100 text-blue-800', darkBg: 'bg-blue-900/30 text-blue-400', icon: Clock },
      PAID: { bg: 'bg-purple-100 text-purple-800', darkBg: 'bg-purple-900/30 text-purple-400', icon: CheckCircle },
    };
      ...
      {loading ? (
        ...
        <div className={`p-12 rounded-lg border text-center ${
          ...
          <p className={`mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            Pay periods will be created automatically when commissions are generated
          </p>
        </div>
        ...
        <div className="space-y-4">
          {payPeriods.map((period) => {
            ...
            const totalAmount = parseFloat(period.totalAmount || period.total_amount || 0);
            const agentCount = period.agentCount || period.agent_count || 0;
            const commissionCount = period.commissionCount || period.commission_count || 0;
                  ...
                  <div className="flex items-center space-x-4">
                    <div className={`p-3 rounded-lg ${
                      ...
                      <Calendar className={`w-6 h-6 ${
                        isPaid ? 'text-purple-500' : isClosed ? 'text-yellow-500' : 'text-green-500'
                      }`} />
                    </div>
                    <div>
            ...
            <div className={`p-6 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
              <div className="flex items-center space-x-3">
                ...
                <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  {confirmAction.type === 'close' ? 'Close Pay Period?' : 'Process Payments?'}
                </h3>
              </div>
            </div>
            ...
            <div className="p-6">
              {confirmAction.type === 'close' ? (
                <p className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>
                  Closing this pay period will prevent any new commissions from being added. 
                  All approved commissions will be locked and ready for payment processing.
                </p>
              ) : (
                <p className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>
                  This will mark all commissions in this period as paid. 
                  Make sure all actual payments have been processed before confirming.
                </p>
              )}
              ...
              <div className={`mt-4 p-3 rounded-lg ${
                ...
                <p className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  Total: {formatCurrency(parseFloat(confirmAction.period.totalAmount || confirmAction.period.total_amount || 0))}
                </p>
              </div>
            </div>
            ...
            <div className={`p-4 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} flex justify-end space-x-3`}>
              ...
              >
                {processing ? (
                  ...
                  <>
                    {confirmAction.type === 'close' ? <Lock className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                    <span>{confirmAction.type === 'close' ? 'Close Period' : 'Process Payments'}</span>
                  </>
                )}
              </button>
            </div>
