...
const SalesAnalytics = () => {
  ...
  const { data: dashboardData } = useApiData(() => analyticsService.getDashboardData(dateParams), [dateParams]);

  const { data: salesTrends } = useApiData(() => analyticsService.getSalesTrends(dateParams), [dateParams]);
  ...
  const { data: dashboardPrev } = useApiData(() => analyticsService.getDashboardData(prevDateParams), [prevDateParams]);
  const { data: productPerformancePrev } = useApiData(
    () => analyticsService.getProductPerformance(prevDateParams),
    [prevDateParams]
  );

  const { data: customerAnalysis } = useApiData(() => analyticsService.getCustomerAnalysis(dateParams), [dateParams]);
  ...
  const analytics = useMemo(() => {
    ...
    const salesTrendsArray = Array.isArray(salesTrends) ? salesTrends : salesTrends?.data || [];
                ...
                {Object.entries(analytics.categoryPerformance)
                  ...
                  .map(([category, data]) => {
                    const maxRevenue = Math.max(
                      ...Object.values(analytics.categoryPerformance).map((c) => c.revenue || 0)
                    );
                    const percentage =
                      Object.values(analytics.categoryPerformance).length > 0 ? (data.revenue / maxRevenue) * 100 : 0;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
                    {formatCurrency(
                      analytics.topCustomers.filter((c) => c.revenue >= 500000).reduce((sum, c) => sum + c.revenue, 0)
                    )}
                  </span>
                </div>
                  ...
                  <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
                    {formatCurrency(
                      analytics.topCustomers
                        .filter((c) => c.revenue >= 100000 && c.revenue < 500000)
                ...
                <div>
                  ...
                  <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
                    {formatCurrency(
                      analytics.topCustomers.filter((c) => c.revenue < 100000).reduce((sum, c) => sum + c.revenue, 0)
                    )}
                  </span>
                </div>
              </div>
                ...
                {analytics.topCustomers.map((customer, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {index < 3 ? <Award size={20} /> : <span className="text-sm">#{index + 1}</span>}
                      </div>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-4">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className={`text-sm ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>
                              {formatCurrency((customer.revenue || 0) / (customer.orders || 1))} avg
                            </span>
                          </div>
                        </div>
                      </div>
        ...
        <div className="md:col-span-3">
          <div
            className={`border rounded-xl ${isDarkMode ? "border-[#37474F] bg-[#1E2328]" : "border-gray-200 bg-white"}`}
          >
            <div className="p-6">
              <div className="flex items-center gap-2 mb-4">
              ...
              <div className="space-y-3">
                {(() => {
                  const topThreeRevenue =
                    (analytics.topCustomers || []).slice(0, 3).reduce((sum, c) => sum + (c.revenue || 0), 0) || 0;
        ...
        <div>
          ...
          >
            <div className="p-6 text-center">
              ...
              {Object.entries(analytics.categoryPerformance).sort(([, a], [, b]) => b.revenue - a.revenue)[0] && (
                <>
                  ...
                  <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
                    {formatCurrency(
                      Object.entries(analytics.categoryPerformance).sort(([, a], [, b]) => b.revenue - a.revenue)[0][1]
                        .revenue
                    )}{" "}
                    revenue
                  </p>
        ...
        <div>
          ...
          >
            <div className="p-6 text-center">
              ...
              <p className="text-xl font-bold mb-1 text-teal-600">{analytics.topProducts[0]?.product}</p>
              <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
