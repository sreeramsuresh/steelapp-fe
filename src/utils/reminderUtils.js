...
export const calculateDaysUntilDue = (dueDate) => {
  ...
  const now = new Date();
  const todayUAE = new Date(
    now.toLocaleString("en-US", { timeZone: TIMEZONE_CONFIG.UAE_TIMEZONE }),
  );
  todayUAE.setHours(0, 0, 0, 0);

  // Get due date in UAE timezone
  const dueUtc = new Date(dueDate);
  const dueUAE = new Date(
    dueUtc.toLocaleString("en-US", { timeZone: TIMEZONE_CONFIG.UAE_TIMEZONE }),
  );
...
export const getReminderType = (daysUntilDue) => {
  ...
  if (daysUntilDue >= -7 && daysUntilDue < 0)
    return REMINDER_TYPES.POLITE_OVERDUE;
  if (daysUntilDue >= -30 && daysUntilDue < -7)
    return REMINDER_TYPES.URGENT_OVERDUE;
...
export const getInvoiceReminderInfo = (invoice) => {
  ...
  const rawPaymentStatus =
    ...
    calculatePaymentStatus(invoice.total, invoice.payments || []);

  // Normalize payment status to handle 'PAYMENT_STATUS_PAID' format
  const paymentStatus = (rawPaymentStatus || "unpaid")
    .toLowerCase()
 ...
 */
export const getReminderLetterContent = (
  reminderType,
  invoice,
  daysUntilDue,
) => {
...
export const generatePaymentReminder = async (invoice, company) => {
  try {
    ...
    pdf.text(
      `Date: ${formatDateDMY(today.toISOString().split("T")[0])}`,
      pageWidth - margin,
      yPos,
      { align: "right" },
    );
