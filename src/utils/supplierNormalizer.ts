...
export function normalizeSupplier(
  ...
  try {
    ...
    const normalized: any = {
      ...
      contactPerson:
        rawSupplier.contact_person || rawSupplier.contactPerson || undefined,
      ...
      businessLicense:
        rawSupplier.business_license ||
        rawSupplier.businessLicense ||
        undefined,
      taxId: rawSupplier.tax_id || rawSupplier.taxId || undefined,
      certifications: rawSupplier.certifications || undefined, // JSON string
      isDesignatedZone:
        rawSupplier.isDesignatedZone || rawSupplier.is_designated_zone || false,

      // Financial (4 fields)
      paymentTerms:
        rawSupplier.payment_terms || rawSupplier.paymentTerms || undefined,
      creditLimit:
        rawSupplier.credit_limit || rawSupplier.creditLimit || undefined,
      currentCredit:
        rawSupplier.current_credit || rawSupplier.currentCredit || undefined,
      bankDetails:
      ...
      isActive:
        rawSupplier.is_active !== undefined
          ? Boolean(rawSupplier.is_active)
          : true,
...
export function normalizeSuppliers(
  ...
  if (!Array.isArray(rawSuppliers)) {
    console.error(
      `❌ [Supplier Normalizer] Expected array, got ${typeof rawSuppliers}`,
    );
    return [];
  }
  ...
  return rawSuppliers
    .map((supplier, index) =>
      normalizeSupplier(supplier, `${source}[${index}]`),
    )
