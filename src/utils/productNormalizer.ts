...
export function normalizeProduct(
  ...
  try {
    ...
    const normalized: any = {
      ...
      gradeVariant:
        rawProduct.gradeVariant || rawProduct.grade_variant || undefined,
      steelGrade:
        rawProduct.steelGrade ||
        rawProduct.steel_grade ||
        rawProduct.grade ||
        undefined,
      ...
      surfaceFinish:
        rawProduct.surfaceFinish ||
        rawProduct.surface_finish ||
        rawProduct.finish ||
        undefined,
      specifications: rawProduct.specifications || undefined,
      specification:
        rawProduct.specification || rawProduct.specifications || undefined,
      schedule: rawProduct.schedule || undefined,

      // Pricing
      price: parseNumber(
        rawProduct.price || rawProduct.selling_price || rawProduct.sellingPrice,
        undefined,
      ),
      sellingPrice: parseNumber(
        rawProduct.sellingPrice || rawProduct.selling_price || rawProduct.price,
        undefined,
      ),
      costPrice: parseNumber(
        rawProduct.costPrice || rawProduct.cost_price,
        undefined,
      ),
      purchasePrice: parseNumber(
        rawProduct.purchasePrice || rawProduct.purchase_price,
        undefined,
      ),
      unitPrice: parseNumber(
        rawProduct.unitPrice || rawProduct.unit_price,
        undefined,
      ),

      // Inventory & Stock
      currentStock: parseNumber(
        rawProduct.currentStock || rawProduct.current_stock,
        undefined,
      ),
      minStock: parseNumber(
        rawProduct.minStock || rawProduct.min_stock,
        undefined,
      ),
      maxStock: parseNumber(
        rawProduct.maxStock || rawProduct.max_stock,
        undefined,
      ),
      ...
      gstRate: parseNumber(
        rawProduct.gstRate || rawProduct.gst_rate,
        undefined,
      ),

      // Analytics & Metrics
      revenue: parseNumber(rawProduct.revenue, undefined),
      revenueGrowth: parseNumber(
        rawProduct.revenueGrowth || rawProduct.revenue_growth,
        undefined,
      ),
      prevRevenue: parseNumber(
        rawProduct.prevRevenue || rawProduct.prev_revenue,
        undefined,
      ),
      sales: parseNumber(rawProduct.sales, undefined),
      orders: parseNumber(rawProduct.orders, undefined),
      ordersGrowth: parseNumber(
        rawProduct.ordersGrowth || rawProduct.orders_growth,
        undefined,
      ),
      prevOrders: parseNumber(
        rawProduct.prevOrders || rawProduct.prev_orders,
        undefined,
      ),
      quantityGrowth: parseNumber(
        rawProduct.quantityGrowth || rawProduct.quantity_growth,
        undefined,
      ),
      prevQuantity: parseNumber(
        rawProduct.prevQuantity || rawProduct.prev_quantity,
        undefined,
      ),
