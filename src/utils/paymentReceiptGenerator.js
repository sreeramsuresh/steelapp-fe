...
import { formatPaymentDisplay } from "./paymentUtils.js";
import { generateReceiptHTML } from "./receiptTemplateGenerator.js";
...
const addPaymentHistoryTable = (pdf, invoice, currentPayment, yPos, margin, pageWidth) => {
  ...
  allPayments.forEach((pmt, idx) => {
    ...
    const methodText =
      pmtFormatted.modeLabel.length > 10 ? `${pmtFormatted.modeLabel.substring(0, 8)}..` : pmtFormatted.modeLabel;
...
export const generatePaymentReceipt = async (payment, invoice, company, paymentIndex = 1) => {
  try {
    ...
    yPos += 5;
    pdf.text(`Date: ${formatDateDMY(receiptDate.toISOString().split("T")[0])}`, pageWidth / 2, yPos, {
      align: "center",
    });
    yPos += 12;
