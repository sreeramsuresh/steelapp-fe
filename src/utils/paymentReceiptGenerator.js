 ...
 */
export const generateFTACompliantReceiptHTML = (
  payment,
  invoice,
  company,
  customer,
...
const addPaymentHistoryTable = (
  ...
  allPayments.forEach((pmt, idx) => {
    ...
    const methodText =
      pmtFormatted.modeLabel.length > 10
        ? `${pmtFormatted.modeLabel.substring(0, 8)}..`
        : pmtFormatted.modeLabel;
...
export const generatePaymentReceipt = async (
  ...
  try {
    ...
    pdf.text(`Receipt No: ${receiptNumber}`, pageWidth / 2, yPos, {
      align: "center",
    });
    yPos += 5;
    pdf.text(
      `Date: ${formatDateDMY(receiptDate.toISOString().split("T")[0])}`,
      pageWidth / 2,
      yPos,
      { align: "center" },
    );
    ...
    pdf.text("Authorized Signatory", sigLineX + sigLineLength / 2, yPos, {
      align: "center",
    });
    yPos += 4;
    pdf.setFont("helvetica", "bold");
    pdf.text(
      normalizeLLC(company?.name || "ULTIMATE STEELS"),
      sigLineX + sigLineLength / 2,
      yPos,
      { align: "center" },
    );
    yPos += 10;
