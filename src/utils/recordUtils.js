...
export const validateCreditNoteForDownload = (creditNote) => {
  ...
  const hasInvoice = creditNote.invoice_id || creditNote.invoiceId || 
...
export const validateQuotationForDownload = (quotation) => {
  ...
  const hasCustomer = quotation.customer?.name?.trim() || 
    quotation.customer_name?.trim() || quotation.customerName?.trim();
  if (!hasCustomer) {
  ...
  const hasValidItems = hasItems && quotation.items.every(item =>
...
export const validatePurchaseOrderForDownload = (purchaseOrder) => {
  ...
  // Check supplier
  const hasSupplier = purchaseOrder.supplier?.name?.trim() || 
    purchaseOrder.supplier_name?.trim() || purchaseOrder.supplierName?.trim();
  if (!hasSupplier) {
  ...
  const hasValidItems = hasItems && purchaseOrder.items.every(item =>
    (item.name?.trim() || item.product_name?.trim()) &&
    item.quantity > 0 &&
    (item.rate > 0 || item.unit_price > 0),
  );
  ...
  const hasExpectedDelivery = purchaseOrder.expected_delivery_date || 
    purchaseOrder.expectedDeliveryDate || purchaseOrder.expected_delivery;
...
export const validateDeliveryNoteForDownload = (deliveryNote) => {
  ...
  // Check linked invoice
  const hasInvoice = deliveryNote.invoice_id || deliveryNote.invoiceId ||
...
export const validateAccountStatementForDownload = (statement) => {
  ...
  // Check customer
  const hasCustomer = statement.customer_id || statement.customerId ||
    statement.customerName || statement.customer_name;
  if (!hasCustomer) {
    missing.customer = true;
