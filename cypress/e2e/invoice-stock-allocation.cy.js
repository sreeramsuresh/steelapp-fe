...
describe("Invoice Form - Stock Allocation Panel", () => {
  ...
  describe("Customer Selection", () => {
    ...
    it("should show customer details after selection", () => {
      ...
      cy.contains(/name:/i).parent().should("contain", TEST_DATA.customer.name);
  ...
  describe("Product Selection with Stock", () => {
    it("should add product via autocomplete and auto-expand allocation panel", () => {
      ...
      cy.wait("@getBatches", { timeout: 10000 }).then((interception) => {
        ...
        expect(interception.request.url).to.include(
          `productId=${TEST_DATA.product.id}`,
        );
    ...
    it("should display correct warehouse stock numbers", () => {
      ...
      cy.getWarehouseStock(0, TEST_DATA.product.expectedStock.abuDhabi.id)
        ...
        .and(
          "contain",
          TEST_DATA.product.expectedStock.abuDhabi.quantity.toString(),
        );
      ...
      cy.getWarehouseStock(0, TEST_DATA.product.expectedStock.dubai.id)
        ...
        .and(
          "contain",
          TEST_DATA.product.expectedStock.dubai.quantity.toString(),
        );
      ...
      cy.getWarehouseStock(0, TEST_DATA.product.expectedStock.main.id)
        ...
        .and(
          "contain",
          TEST_DATA.product.expectedStock.main.quantity.toString(),
        );

      // Verify NO warehouse shows 0
      cy.get('[data-testid="allocation-stock-warehouses-0"]').should(
        "not.contain",
        /:\s*0(?!\d)/,
      ); // Regex to match ": 0" but not ": 10" etc.

      cy.log("✓ All warehouse stock numbers are CORRECT");
      cy.log(
        `  Abu Dhabi: ${TEST_DATA.product.expectedStock.abuDhabi.quantity}`,
      );
    ...
    it("should display batch allocation table with correct number of batches", () => {
      ...
      cy.get('[data-testid="batch-allocation-table-0"]').should(
        "contain",
        "BTH-",
      ); // Batch number format

      cy.log(
        `✓ Batch table shows ${TEST_DATA.product.expectedBatches} batches`,
      );
    });
  ...
  describe("Zero Stock Product", () => {
    it("should auto-select LOCAL_DROP_SHIP for zero-stock products", () => {
      ...
      cy.get('[data-testid="product-autocomplete-0"]')
        .click()
        .type(TEST_DATA.zeroStockProduct.name);

      // Check if product exists in dropdown
      cy.get("body").then(($body) => {
        const listboxSelector =
          '[data-testid="product-autocomplete-0-listbox"]';
  ...
  describe("Invoice Totals", () => {
    it("should calculate invoice totals correctly", () => {
      ...
      cy.contains(/total/i).parent().should("contain", "1,050");
  ...
  describe("Form Validation", () => {
    ...
    it("should allow save after filling required fields", () => {
      ...
      cy.contains("button", /save.*invoice|create.*invoice/i).should(
        "not.be.disabled",
      );
