 ...
 * payment-related content with proper controls.
 */
...
describe("Payments & Financial Operations", () => {
  ...
  it("should render receivables table or empty state", () => {
    ...
    cy.get("body").should(($body) => {
      ...
      const hasEmptyState = $body.text().toLowerCase().includes("no");
      const hasContent = $body.text().length > 100;
      expect(
        hasTable || hasEmptyState || hasContent,
        "Receivables should show table, empty state, or content",
      ).to.be.true;
    });
  });

  it("should have action buttons for payment operations", () => {
  ...
  it("should have search or filter controls", () => {
    cy.contains(/receivable/i, { timeout: 15000 });
    cy.get(
      'input[placeholder*="Search"], input[type="search"], select, [role="combobox"], [data-testid*="search"], [data-testid*="filter"]',
      { timeout: 10000 },
