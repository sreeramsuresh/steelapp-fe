...
async function testPhase2cForms() {
  ...
  try {
    ...
    // Test 1: SupplierBillForm
    console.log('\n📋 TEST 1: SupplierBillForm');
    console.log('📄 Navigating to /purchases/supplier-bills/new...');
    await page.goto('http://localhost:5173/purchases/supplier-bills/new', {
    ...
    const foundSupplierBillFields = [];
    for (const field of supplierBillFields) {
      const input = await page.$(
        `input[name="${field}"], select[name="${field}"], textarea[name="${field}"]`,
      );
      if (input) {
        foundSupplierBillFields.push(field);
        console.log(`  ✓ ${field}`);
      } else {
    ...
    await page.screenshot({
      path: '/mnt/d/Ultimate Steel/supplierbillform-phase2c.png',
    });
    console.log('📸 Screenshot: supplierbillform-phase2c.png');

    // Check for errors
    if (errors.length > 0) {
      console.error('❌ SupplierBillForm ERRORS:');
      errors.forEach((err) => console.error('  -', err));
      return { success: false, errors, test: 'SupplierBillForm' };
    }

    console.log(
      `✅ SupplierBillForm loaded successfully (${foundSupplierBillFields.length}/${supplierBillFields.length} fields)`,
    );
    ...
    return {
      success: true,
      errors: [],
      supplierBillFieldsFound: foundSupplierBillFields.length,
      supplierBillFieldsTotal: supplierBillFields.length,
...
testPhase2cForms()
  .then((result) => {
    ...
    if (result.supplierBillFieldsFound !== undefined) {
      console.log(
        `  SupplierBillForm: ${result.supplierBillFieldsFound}/${result.supplierBillFieldsTotal} fields`,
      );
      console.log(
